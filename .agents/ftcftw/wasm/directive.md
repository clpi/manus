...
| # | directive |
|---|---|
| 1 | Build the smallest self-hosted semantic execution system that achieves the best lawful physical realization of programs expressed in Idol and admitted foreign source laws. |
| 2 | Wasm/WASI/WASIX are not side runtimes — they are semantic source/foreign-law inputs into the same graph. clpi/wart is the compatibility/performance oracle; do not port it line-for-line. |
...
| # | directive |
|---|---|
| 1 | observation → computation → theorem answer → foreign runtime → component boundary → canonical ABI → process/thread boundary → serialization → copy → aggregate/place → algorithm → fusion → state → compile-time → representation → linear-memory promotion → bounds → ABI specialization → devirtualization → async state → synchronization → layout/placement → schedule/vector/GPU → machine instruction → local superoptimization. |
| 2 | Never optimize rung 24 while a higher rung plausibly dominates. |
...
| # | directive |
|---|---|
| 1 | core Wasm → law → graph add/loop/memory semantics → direct x64/arm64, no interpreter object, compare wart/Winch/Cranelift. |
| 2 | Then sealed component calling (ABI erased vs reference). |
| 3 | Then WASI stdout pipeline (no generic WasiCtx). |
| 4 | Then WASI 0.3 stream (sync collapse + real async). |
...
| # | directive |
|---|---|
| 1 | Compiler lane closes host source-law ingress and GRAMMAR-ONE execution. |
...
| # | directive |
|---|---|
| 1 | tools/wasm/ingest.id is executable research/bootstrap Idol with known canonicality debt. |
| 2 | It checks, compiles under direct-native, and runs as a process, but executable production source is not a canonical teaching surface. |
| 3 | Compound identities, the hex transport, local opcode labels, and the line transport remain migration evidence rather than language authority. |

| # | directive |
|---|---|
| 1 | UPDATE 4 (FINAL for the census): the idol ingest matches ground truth EXACTLY — 29,004 instructions, 1,028 locals across 203 bodies. |
| 2 | Two more grammar bugs closed by per-body trace diff: (a) the walk-ending op (br_table) was excluded from the count |
| 3 | (b) call_indirect's TWO LEB immediates had no handler — padded LEBs were counted as opcodes (body 48: 5 GT ops vs 15). |
| 4 | The measured fixture instruction/local census agrees with its independent oracle; snapshot at evidence/mop/wasm/ ingest.fib.facts. |
| 5 | This does not prove complete Wasm validation. |
| 6 | Open semantic work includes block signatures, stack typing, branch arity, indices, reference types, feature gates, proposal instructions, and publication into the shared semantic graph. |

| # | directive |
|---|---|
| 1 | UPDATE 3: full body walk lands — locals EXACT (1,028 across 203 bodies), instruction census 29,356 vs independent ground truth 29,004 (1.2% divergence). |
| 2 | The big correctness fix was a SIGN ERROR: the sleb class is -2 but its handler tested cl == 2, so every i32.const value byte was counted as an opcode (38,001 before the fix). |
| 3 | The remaining 1.2% is diagnosed by per-body trace-diff (84 bodies differ, both +1 and +40 patterns) with the trace methodology proven; one instrumentation lesson recorded: emit-at-read-time, never post-skip (positions shift). |

| # | directive |
|---|---|
| 1 | UPDATE 2: per-section content counts now parsed IN IDOL and CROSS- VALIDATED against an independent read of the same bytes: types 39, imports 6, functions 203, exports 3, bodies 203 — exact match. |
| 2 | The ingest is the census authority candidate for the module face; the next increment is body walk (local decls + application chains). |

| # | directive |
|---|---|
| 1 | UPDATE: the ingest now RUNS END TO END over a DECLARED hex transport (in-place pair reads, O(1) per byte, zero materialization) — real fib.wasm bytes -> idol -> fact lines: magic, version, and the full section census with byte sizes. |
| 2 | The binary face (H9) later replaces the byteat accessor alone; no fact line changes. |

| # | directive |
|---|---|
| 1 | Idiom lessons paid (fleet-relevant, measured): args() is 1-based over CLI args (args(1) = first arg; args(2) past the end is NULL and propagates SILENTLY through ==, :len, and :byte guards — guard with an explicit range check); `_ = call()` discard-bindings are a no-op shape (call directly as a statement); loop-built str concatenation segfaults on ~73k iterations (read in place instead). |

| # | directive |
|---|---|
| 1 | MEASURED BLOCKER (H9): binary-safe ingress. `path:read()` on a .wasm file yields an EMPTY str — the current read face is text-only and the wasm magic (\0asm) does not survive. |
...
| # | directive |
|---|---|
| 1 | Every emitted record has an application-shaped schema, but its application number is a sequential source occurrence counter, its caller is a Wasm function index, and its relation is a locally assigned string. |
| 2 | Those are provenance and classification coordinates, not exact semantic graph identities. |
...
| # | directive |
|---|---|
| 1 | The full-bounds census found the last unnamed ops (0x46 eq outside the cmp start; conversions 0xA7/0xA9-0xAC inside the arith64 block). |
| 2 | The measured corpus has no opcode left without a local family label. |
...
| # | directive |
|---|---|
| 1 | Track schema coverage, known-fact coverage, trust/proof status, and semantic-id continuity separately. |
| 2 | An unknown card is an honest blocker, not evidence that the corresponding semantic fact has been closed. |
