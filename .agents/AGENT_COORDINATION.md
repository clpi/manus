...
| G-053 | native | P1 | **Branch-scoping fix done (2026-08-02):** first-assignment *inside* an `if`/`elseif`/`else`/loop branch is now hoisted to the enclosing block scope when the value is read later or in another branch — `hoist_control_implicit_locals` in codegen.zig pre-scans control-statement subtrees, unifies branch types (`merge_assigned_types`, mirrors sema `unify_numeric`), and emits a typed default-init declaration before the control statement. Fixes the `scale` undeclared-identifier failure in `quantize_symmetric`. **Still open:** (a) module-scope function-valued assignment (`build = sequential` in `lib/std/ml/nn.duo` emits bare `build`, then reads box as nonexistent `std_ml_nn__build__lua`) — module-scope implicit locals only promote to `module_globals` when referenced by module *functions*, not by module-scope table reads; (b) bitwise ops on float-typed values in `quantize_symmetric` (`val1 & 15`) and `fp16_decode` (`int64 | double mantissa`); (c) line 6665 `void*` from `lua_Value` + 11947 `NN` undeclared. ml_showcase still fails C stage on these pre-existing issues. | opencode | 2026-08-02 |
