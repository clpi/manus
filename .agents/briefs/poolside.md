...
- **P-A — remove an obsolete refusal guard.** Only when the repository
  already contains the exact supporting implementation, the exact guard,
  positive controls, wrong-answer damage controls, and a routed deletion
  instruction. Allowed change: delete guard, delete now-unused helper,
  promote exact fixtures, re-pin exact gate. No other behavior.
- **P-B — physical result-pack extension.** After graph pack facts exist:
  FP result pack, mixed GP/FP pack, branch-result pack moves, foreign sret
  physical adaptation, parallel copy cycle. Poolside does not define pack
  semantics.
- **P-C — target/object portability.** After target and ABI facts exist:
  ELF x86-64 object emission, ELF AArch64 object emission, Win64 calling
  convention, COFF relocation. No target-independent semantic changes.
- **P-D — concrete runtime relation realization.** For an already
  graph-resolved relation (clock, time, filesystem operation, stream
  write/read): implement the physical direct-native behavior. Do not add
  the relation to a roster or infer its world.
- **P-E — mechanical graph-consumer migration.** After Codex provides an
  accessor: replace AST/source-name read with accessor, remove old
  reconstruction, missing fact refuses. One consumer per patch.
