...
pub const entries: []const Entry = &.{
    ...
    .{ .id = "RL-14", .host_path = "scripts/run_benchmark.sh", .subsystem_id = "FC-D", .duo_replacement = "scripts/run_benchmark.duo", .eligibility = .after_m1, .removal_gate = "bench suite via duo run; bash fallback deleted -- HALF MET (measured 2026-08-07): scripts/run_benchmark.duo EXISTS (308 lines vs the shell 335) but is wired NOWHERE; build.zig:103 still runs `bash scripts/run_benchmark.sh`. Finishing is one line plus one verification run, not a port. CORRECTION 2026-08-07: I earlier recorded that the .duo port merely had a wrong failure MESSAGE. It does not. RESULT_FAIL_FLAG is set when C wins, so its 'must beat or tie reference C' text is accurate and the port is checking PERFORMANCE. The shell version fails earlier, on CORRECTNESS (RESULT mismatch for mandel and str_chain), and those mismatches are pre-existing -- verified by reverting tokenizeAuthority to .host_zig and getting byte-identical numbers. So the two versions differ in which gate they reach first, not in message wording, and closing RL-14 means making the .duo port run the correctness comparison before the performance one" },
};
