...
pub const subsystems: []const Subsystem = &.{
    ...
    .{
        ...
        .bootstrap_required = false,
        .migration_blocker = "Remaining for duo_canonical: point the compiler's own tokenize authority at the Duo lexer instead of src/lexer.zig. Everything under that step is now proven rather than assumed. EQUIVALENCE: token-for-token equal to src/lexer.zig on kinds (pass16_lexer_fingerprint_differential) and on TEXT (duo_lexer_text_fingerprint, which mixes length before bytes so concatenation cannot alias). ALLOCATION: lexer.duo's 54 string.sub sites run on lib/std/str.duo, exported as a @comp.c.export primitive, so the backend emits a relocation and needs no writable __DATA arena. DISPATCH (2026-08-07): a C host that knows nothing but the ABI links against the artifact and agrees token-for-token with the in-process lexer on 8 sources including a real 135-line module — examples/pass16_lexer_host_proof.duo, in the selfhost_proofs gate, with both controls firing (the pre-fix artifact shape produces no answer; a host reading the wrong record column fails the differential). The artifact must be built from lib/std/compiler/host.duo, NOT from lexer.duo: --lib never initializes the primary module, so lexer.duo's file-scope `strm` reads NULL. That is GAP-020, filed with a four-line repro; the anchor module is the workaround and the host ABI is unaffected by it. What is left is host-side wiring only: src/lexer.zig consuming the 4-i64-per-token records (kind, line, col, int_val — stride published as duo_lexer_host_stride) and tokenizeAuthority() selecting the Duo path.",
        .removal_gate = "tokenizeAuthority() dispatch through the Duo lexer + pass16-m1-smoke + selfhost_proofs SH-03 row",
    },
    .{
        .id = "SH-04",
        .title = "Parser",
