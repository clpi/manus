...
pub const CodeGen = struct {
    ...
    pub fn can_emit_native_scalar_module(self: *CodeGen, mod: *const ast.Module) bool {
        ...
        if (self.load_chunk or self.lib_mode or self.test_mode or self.bench_mode) {
    ...
    pub fn emit_module(self: *CodeGen, mod: *ast.Module) E!void {
        ...
        if (!self.native_scalar_mode) {
            ...
            self.p("}}\n", .{});
            self.p("static inline v8f32 duo_select_v8f32(v8i32 c, v8f32 a, v8f32 b) {{\n", .{});
            self.p("    v8f32 fc = __builtin_convertvector(c, v8f32);\n", .{});
            self.p("    v8f32 one = (v8f32){{1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f}};\n", .{});
            self.p("    return a * fc + b * (one - fc);\n", .{});
            self.p("}}\n", .{});
    ...
    fn emit_test_runner(self: *CodeGen, mod: *ast.Module) E!void {
        ...
        const structured = self.test_structured_output;
        ...
        for (self.test_entries) |entry| {
            ...
            if (opts.skip) {
                if (structured) {
                    self.pl("fprintf(stderr, \"DUO_EVT\\ttest\\tskip\\tname={s}\\treason=skip\\n\");", .{entry.func_name});
                } else {
            ...
            if (opts.should_panic) {
                self.pl("duo_test_current = \"{s}\";", .{entry.func_name});
                self.pl("duo_test_runner_active = 1;", .{});
                if (structured) {
                    self.pl("fprintf(stderr, \"DUO_EVT\\ttest\\trun\\tname={s}\\treason=expect_panic\\n\");", .{entry.func_name});
                } else {
                    self.pl("fprintf(stderr, \"RUN  %s (expect panic)\\n\", \"{s}\");", .{entry.func_name});
                }
                self.pl("duo_tests_run++;", .{});
                self.pl("if (setjmp(duo_test_jmp) == 0) {{", .{});
                self.pl("  jmp_buf _old_ej; memcpy(_old_ej, error_jmp, sizeof(jmp_buf));", .{});
                self.pl("  bool _old_has = has_error_jmp; has_error_jmp = true;", .{});
                self.pl("  if (setjmp(error_jmp) == 0) {{", .{});
                self.pl("    (void){s}();", .{cname});
                self.pl("    has_error_jmp = _old_has; memcpy(error_jmp, _old_ej, sizeof(jmp_buf));", .{});
                if (structured) {
                    self.pl("    fprintf(stderr, \"DUO_EVT\\ttest\\tfail\\tname={s}\\treason=expected_panic\\n\");", .{entry.func_name});
                } else {
                    self.pl("    fprintf(stderr, \"FAIL %s (expected panic)\\n\", \"{s}\");", .{entry.func_name});
                }
                self.pl("    duo_tests_failed++;", .{});
                self.pl("  }} else {{", .{});
                self.pl("    has_error_jmp = _old_has; memcpy(error_jmp, _old_ej, sizeof(jmp_buf));", .{});
                if (structured) {
                    self.pl("    fprintf(stderr, \"DUO_EVT\\ttest\\tpass\\tname={s}\\treason=panicked\\n\");", .{entry.func_name});
                } else {
                    self.pl("    fprintf(stderr, \"ok   %s (panicked)\\n\", \"{s}\");", .{entry.func_name});
                }
                self.pl("  }}", .{});
                self.pl("}} else {{", .{});
                if (structured) {
                    self.pl("  fprintf(stderr, \"DUO_EVT\\ttest\\tfail\\tname={s}\\treason=assertion\\n\");", .{entry.func_name});
                } else {
                    self.pl("  fprintf(stderr, \"FAIL %s (assertion)\\n\", \"{s}\");", .{entry.func_name});
                }
                self.pl("}}", .{});
                self.pl("duo_test_runner_active = 0;", .{});
                continue;
            }
            ...
            if (structured) {
                self.pl("fprintf(stderr, \"DUO_EVT\\ttest\\trun\\tname={s}\\n\");", .{entry.func_name});
            } else {
            ...
            if (opts.bench or opts.iterations > 1) {
                ...
                if (structured) {
                    self.pl("fprintf(stderr, \"DUO_EVT\\ttest\\tbench\\tname={s}\\twarmup={d}\\titer={d}\\telapsed=%.9f\\tper_us=%.3f\\n\", _elapsed, (_elapsed * 1e6) / (double){d});", .{ entry.func_name, warmup, iters, iters });
                } else {
                ...
                self.pl("double _t1 = duo_time_now();", .{});
                if (structured) {
                    self.pl("fprintf(stderr, \"DUO_EVT\\ttest\\ttime\\tname={s}\\telapsed=%.9f\\n\", _t1 - _t0);", .{entry.func_name});
                } else {
            ...
            if (opts.flaky) {
                if (structured) {
                    self.pl("fprintf(stderr, \"DUO_EVT\\ttest\\tflaky\\tname={s}\\n\");", .{entry.func_name});
                } else {
                    self.pl("fprintf(stderr, \"FLAKY %s (passed)\\n\", \"{s}\");", .{entry.func_name});
                }
            } else if (structured) {
                self.pl("fprintf(stderr, \"DUO_EVT\\ttest\\tpass\\tname={s}\\n\");", .{entry.func_name});
            } else {
        ...
        if (structured) {
            self.pl("fprintf(stderr, \"DUO_EVT\\ttest\\tsummary\\trun=%d\\tpass=%d\\tskipped=%d\\tfailed=%d\\n\", duo_tests_run, duo_tests_run - duo_tests_failed, duo_tests_skipped, duo_tests_failed);", .{});
        } else {
    ...
    pub fn populate_enum_defs(self: *CodeGen, mod: *ast.Module) E!void {
        for (mod.body.stmts) |*stmt| {
    ...
    fn emit_record_initializer(
        ...
        for (rec_fields, 0..) |rf, i| {
            ...
            if (found.get(rf.name)) |val| {
                try self.emit_arg_for_param(val, rf.typ);
            } else if (i < positionals.items.len) {
                try self.emit_arg_for_param(positionals.items[i], rf.typ);
            } else {
                self.p("0", .{});
            }
        }
    ...
    fn emit_arg_for_param(self: *CodeGen, arg: *const ast.Expr, param_type: RT) E!void {
        if (param_type == .any) {
            ...
            self.typ(param_type);
            self.p(")", .{});
            try self.emit_record_initializer(param_type.table_type.fields, arg);
        } else {
            const arg_type = self.expr_type(arg);
            if (arg_type == .any and self.expr_is_raw_c_intrinsic(arg)) {
            ...
            } else if (param_type == .str and arg_type == .str) {
                try self.emit_expr(arg);
            } else if (param_type == .bool and arg_type == .bool) {
                try self.emit_expr(arg);
            } else {
                try self.emit_expr(arg);
            }
    ...
    fn emit_default_for_param(self: *CodeGen, param: *const ast.FuncParam, param_type: RT) E!bool {
        const default_val = param.default_val orelse return false;
        try self.emit_arg_for_param(default_val, param_type);
        return true;
    }
    ...
    fn collect_and_emit_record_decls(self: *CodeGen, mod: *ast.Module) E!void {
        for (mod.body.stmts) |*stmt| {
            switch (stmt.*) {
                ...
                .local_decl => |*ld| {
                    for (ld.names) |*n| try self.collect_records_in_binding(n);
                    for (ld.inits) |e| try self.collect_records_in_expr(e);
                },
                .global_decl => |*gd| {
                    for (gd.names) |*n| try self.collect_records_in_binding(n);
    ...
    fn collect_records_in_binding(self: *CodeGen, lname: *const ast.LocalName) E!void {
        if (lname.typ == .record) {
            const rt = self.resolve_binding_type(lname);
            if (rt == .table_type) try self.ensure_record_decl(rt);
            return;
        }
        try self.collect_records_in_typ(lname.typ);
    }
    ...
    fn collect_records_in_typ(self: *CodeGen, te: ast.TypeExpr) E!void {
        switch (te) {
            .named => |name| {
                if (self.record_aliases.get(name)) |rt| {
                    if (rt == .table_type) try self.ensure_record_decl(rt);
                }
            },
            .record => {
    ...
    fn emit_func_decl_forward(self: *CodeGen, fd: *const ast.FuncDecl) E!void {
        ...
        if (fb.is_typed or self.should_emit_lua_thunk(fb)) try self.emit_lua_thunk_decls(fd);
    }
    ...
    fn emit_func_c_name(self: *CodeGen, fd: *const ast.FuncDecl, buf: []u8) []const u8 {
        ...
        if (self.current_module_cname.len > 0) {
            ...
            if (pos + cm.len <= buf.len) {
                @memcpy(buf[pos..][0..cm.len], cm);
                pos += cm.len;
            }
            if (pos + 2 <= buf.len) {
                buf[pos] = '_';
    ...
    fn func_export_name(fd: *const ast.FuncDecl) ?[]const u8 {
        for (fd.attributes) |attr| {
            if (std.mem.eql(u8, attr.name, "c.export")) {
                ...
                return if (fd.path.len > 0) fd.path[fd.path.len - 1] else "unknown";
            }
        }
    ...
    fn emit_func_storage_and_attrs(self: *CodeGen, fd: *const ast.FuncDecl) E!void {
        ...
        if (fb.device_target != .none) {
            self.p("/* duo-device: {s} */ ", .{directives.deviceTargetName(fb.device_target)});
        }
        if (fb.autodiff) {
            self.p("__attribute__((annotate(\"duo_autodiff\"))) ", .{});
        }
    }
    ...
    fn emit_lua_thunk(self: *CodeGen, fd: *const ast.FuncDecl) E!void {
        ...
        if (nparams == 3) {
            ...
            if (ret == .void) {
                self.p("{s}(_p0, _p1, _p2, _p3);\n    return lua_val_nil();\n", .{cname});
            } else {
                self.p("{s} _r = {s}(_p0, _p1, _p2, _p3);\n    return ", .{ ret_ct, cname });
                try self.emit_lua_value_from_native(ret, "_r");
                self.p(";\n", .{});
            }
            self.p("}}\n\n", .{});
            ...
            if (ret == .void) {
                self.p("{s}(_p0, _p1, _p2, _p3, _p4);\n    return lua_val_nil();\n", .{cname});
            } else {
                self.p("{s} _r = {s}(_p0, _p1, _p2, _p3, _p4);\n    return ", .{ ret_ct, cname });
                try self.emit_lua_value_from_native(ret, "_r");
                self.p(";\n", .{});
            }
            self.p("}}\n\n", .{});
    ...
    fn emit_native_func_name_as_lua_value(self: *CodeGen, raw_name: []const u8, params: []types.ResolvedType, _: bool) E!void {
    ...
    fn emit_stmt(self: *CodeGen, stmt: *const ast.Stmt) E!void {
        switch (stmt.*) {
            ...
            .const_decl => |*cd| {
                ...
                self.typ(rt);
                self.p(" {s} = ", .{cd.ident});
                try self.emit_arg_for_param(cd.val, rt);
                self.p(";\n", .{});
                try self.note_comptime_binding(cd.ident, cd.val);
            },
            .assign => |*as| {
            ...
            .call => |c| {
                ...
                if (c.func.* == .name and std.mem.eql(u8, c.func.name.ident, "__as") and c.args.len == 2) {
                    ...
                    if (self.type_from_c_name(target_name)) |target| {
                        try self.emit_arg_for_param(c.args[1], target);
                    } else {
                ...
                if (try self.maybe_emit_ml_call(c.func, c.args)) return;
                ...
                // Recover the typed signature so we can emit a direct C call.
                if (c.func.* == .name) {
                    var name_buf: [256]u8 = undefined;
                ...
                if (ft == .any) {
                    if (self.should_bind_field_call(c.func, c.args.len)) {
                        try self.emit_bound_field_call(c);
                        return;
                    }
            ...
            .binop => |b| {
                    ...
                    (!self.expr_is_native_cstr(b.lhs) or !self.expr_is_native_cstr(b.rhs)))
                {
                    ...
                    switch (b.op) {
                        ...
                        .@"or", .@"and" => {
                            // For str (const char*), pointer, nil (void*), and any types,
                            // `a or b` = (a ? a : b), `a and b` = (a ? b : a).
                            // C's || and && don't work on pointers.
                            if (lt == .str or lt == .pointer or lt == .nil or lt == .any) {
                                if (b.op == .@"or") {
                                    self.p("((", .{});
                                    try self.emit_expr(b.lhs);
                                    self.p(") ? (", .{});
    ...
    fn maybe_emit_mem_call(self: *CodeGen, func: *const ast.Expr, args: []*ast.Expr, result_rt: RT) E!bool {
        ...
        if (std.mem.eql(u8, fname, "prefetch")) {
            self.p("__builtin_prefetch(", .{});
            try self.emit_mem_ptr_arg(args, 0, true);
            self.p(", ", .{});
            try self.emit_mem_integer_arg(args, 1, "0");
            self.p(", ", .{});
            try self.emit_mem_integer_arg(args, 2, "3");
            self.p(")", .{});
            return true;
        }
        ...
        if (std.mem.eql(u8, fname, "read_u32")) {
            self.p("((uint64_t)(*(uint32_t*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p("))))", .{});
        ...
        if (std.mem.eql(u8, fname, "read_i64")) {
            self.p("((int64_t)(*(int64_t*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p("))))", .{});
        ...
        if (std.mem.eql(u8, fname, "read_u64")) {
            self.p("((uint64_t)(*(uint64_t*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p("))))", .{});
        ...
        if (std.mem.eql(u8, fname, "read_f32")) {
            self.p("((double)(*(float*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p("))))", .{});
        ...
        if (std.mem.eql(u8, fname, "read_f64")) {
        ...
        if (std.mem.eql(u8, fname, "write_byte") or std.mem.eql(u8, fname, "write_i8") or std.mem.eql(u8, fname, "write_u8")) {
            ...
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p(")) = (uint8_t)(", .{});
            if (args.len > 2) try self.emit_mem_integer_arg(args, 2, "0") else self.p("0", .{});
            self.p(")", .{});
            return true;
        }
        if (std.mem.eql(u8, fname, "write_i16")) {
            self.p("*(int16_t*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p(")) = (int16_t)(", .{});
        ...
        if (std.mem.eql(u8, fname, "write_u16")) {
            self.p("*(uint16_t*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p(")) = (uint16_t)(", .{});
        ...
        if (std.mem.eql(u8, fname, "write_i32")) {
            self.p("*(int32_t*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p(")) = (int32_t)(", .{});
        ...
        if (std.mem.eql(u8, fname, "write_u32")) {
            self.p("*(uint32_t*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p(")) = (uint32_t)(", .{});
        ...
        if (std.mem.eql(u8, fname, "write_u64")) {
            self.p("*(uint64_t*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p(")) = (uint64_t)(", .{});
        ...
        if (std.mem.eql(u8, fname, "write_f32")) {
            self.p("*(float*)((uint8_t*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(") + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("0", .{});
            self.p(")) = (float)(", .{});
        ...
        if (std.mem.eql(u8, fname, "bytes_to_f32")) {
            self.p("({{ float _v; memcpy(&_v, ((const char*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(".as.sval)) + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("1", .{});
            self.p(" - 1), sizeof(float)); (double)_v; }})", .{});
            return true;
        }
        if (std.mem.eql(u8, fname, "bytes_to_f64")) {
            self.p("({{ double _v; memcpy(&_v, ((const char*)(", .{});
            if (args.len > 0) try self.emit_expr(args[0]) else self.p("NULL", .{});
            self.p(".as.sval)) + (", .{});
            if (args.len > 1) try self.emit_mem_integer_arg(args, 1, "0") else self.p("1", .{});
            self.p(" - 1), sizeof(double)); _v; }})", .{});
            return true;
        }
        return false;
    }
    ...
    fn maybe_emit_simd_call(self: *CodeGen, func: *const ast.Expr, args: []*ast.Expr) E!bool {
        ...
        if (std.mem.eql(u8, fname, "matmul_f32") and args.len >= 6) {
            self.p("duo_simd_matmul_f32((const float*)(", .{});
            try self.emit_expr(args[0]);
            self.p("), (const float*)(", .{});
            try self.emit_expr(args[1]);
            self.p("), (float*)(", .{});
            ...
            self.p("), (size_t)(", .{});
            try self.emit_expr(args[4]);
            self.p("), (size_t)(", .{});
            try self.emit_expr(args[5]);
            self.p("))", .{});
            return true;
        }
        if (std.mem.eql(u8, fname, "matmul_f64") and args.len >= 6) {
            self.p("duo_simd_matmul_f64((const double*)(", .{});
            try self.emit_expr(args[0]);
            self.p("), (const double*)(", .{});
            try self.emit_expr(args[1]);
            self.p("), (double*)(", .{});
        ...
        if (f.obj.* == .field) {
            ...
            if (inner.obj.* == .name and std.mem.eql(u8, inner.obj.name.ident, "std")) {
                ...
                if (std.mem.eql(u8, submod, "mem")) {
                    ...
                    if (std.mem.eql(u8, fnname, "pool_get")) {
                        self.p("duo_pool_get(", .{});
                        if (args.len >= 1) try self.emit_expr(args[0]);
                        self.p(")", .{});
                        return true;
                    }
                    if (std.mem.eql(u8, fnname, "pool_put")) {
                        self.p("duo_pool_put(", .{});
                        if (args.len >= 1) try self.emit_expr(args[0]);
                        self.p(", ", .{});
                        if (args.len >= 2) try self.emit_expr(args[1]);
                    ...
                    if (std.mem.eql(u8, fnname, "prefetch")) {
    ...
    fn emit_required_modules(self: *CodeGen, mod: *const ast.Module) E!void {
        ...
        while (i < names.items.len) : (i += 1) {
            ...
            const abs_lib_lua = if (self.stdlib_root) |root| try std.fmt.allocPrint(self.alloc, "{s}/{s}.lua", .{ root, mod_path_name }) else @as([]const u8, "");
            const abs_lib_duo = if (self.stdlib_root) |root| try std.fmt.allocPrint(self.alloc, "{s}/{s}.duo", .{ root, mod_path_name }) else @as([]const u8, "");
            ...
            const abs_lib_lua_ok = if (self.stdlib_root != null) if (Io.Dir.access(cwd, self.io, abs_lib_lua, .{})) true else |_| false else false;
            const abs_lib_duo_ok = if (self.stdlib_root != null) if (Io.Dir.access(cwd, self.io, abs_lib_duo, .{})) true else |_| false else false;
            ...
            if (src_lua_ok) {
                ...
                if (self.stdlib_root != null) self.alloc.free(abs_lib_lua);
                if (self.stdlib_root != null) self.alloc.free(abs_lib_duo);
            } else if (src_duo_ok) {
                mod_path = src_duo;
                ...
                if (self.stdlib_root != null) self.alloc.free(abs_lib_lua);
                if (self.stdlib_root != null) self.alloc.free(abs_lib_duo);
            } else if (abs_lib_lua_ok) {
                mod_path = abs_lib_lua;
                self.alloc.free(src_lua);
                self.alloc.free(src_duo);
                self.alloc.free(lib_lua);
                self.alloc.free(lib_duo);
                ...
                if (has_core) self.alloc.free(core_duo);
                if (self.stdlib_root != null) self.alloc.free(abs_lib_duo);
            } else if (abs_lib_duo_ok) {
                mod_path = abs_lib_duo;
                self.alloc.free(src_lua);
                self.alloc.free(src_duo);
                self.alloc.free(lib_lua);
                self.alloc.free(lib_duo);
...
const duo_runtime =
    ...
    \\// ═══════════════════════════════════════════════════════════════
    \\typedef struct { uint8_t* base; size_t cap; size_t pos; } DuoArena;
    \\
    ...
    \\// ═══════════════════════════════════════════════════════════════
    \\typedef struct DuoPoolSlot { struct DuoPoolSlot* next; } DuoPoolSlot;
    \\typedef struct { uint8_t* base; size_t obj_size; size_t cap; DuoPoolSlot* free_list; } DuoPool;
    \\
    ...
    \\typedef float duo_v4f32 __attribute__((vector_size(16)));
    \\typedef float duo_v8f32 __attribute__((vector_size(32)));
    \\typedef double duo_v2f64 __attribute__((vector_size(16)));
    \\typedef double duo_v4f64 __attribute__((vector_size(32)));
    \\typedef int32_t duo_v4i32 __attribute__((vector_size(16)));
    \\typedef int32_t duo_v8i32 __attribute__((vector_size(32)));
    \\typedef int64_t duo_v4i64 __attribute__((vector_size(32)));
    \\#endif
    \\
    ...
    \\typedef struct { const char* ptr; size_t len; } DuoStrView;
    \\
...
test "codegen: generic specialization calls use typed argument coercion" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    const Sema = @import("sema.zig").Sema;
...
test "codegen: explicit @specialize emits generic specialization without call site" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    const Sema = @import("sema.zig").Sema;
    ...
    try testing.expect(std.mem.indexOf(u8, output, "static inline int64_t duo_id_i64(int64_t x);") != null);
    try testing.expect(std.mem.indexOf(u8, output, "static inline int64_t duo_id_i64(int64_t x) {") != null);
}
...
test "codegen: @sizeof and @alignof emit layout intrinsics" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    ...
    var lex = Lexer.init(
        \\local value: i64 = 0
        \\local sz: i64 = @sizeof(i64)
        \\local align: i64 = @alignof(i64)
        \\local value_sz: i64 = @sizeof(value)
        \\print(sz + align + value_sz)
    , "test");
    var parser = Parser.init(&lex, alloc);
    var module = try parser.parse_module();
    var semantic = sema.Sema.init(alloc);
    ...
    try testing.expect(std.mem.indexOf(u8, output, "int64_t align = ((int64_t)_Alignof(int64_t));") != null);
    try testing.expect(std.mem.indexOf(u8, output, "int64_t value_sz = ((int64_t)sizeof(value));") != null);
    try testing.expect(std.mem.indexOf(u8, output, "@sizeof") == null);
    try testing.expect(std.mem.indexOf(u8, output, "__sizeof") == null);
}

test "codegen: @as unboxes dynamic values into explicit native type" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
...
test "codegen: typed table module calls unbox boxed runtime results" {
    ...
    const alloc = arena.allocator();
    var lex = Lexer.init(
        \\local joined: str = table.concat({"a", "b"}, ",")
        \\local frozen: bool = table.isfrozen({})
        \\print(joined, frozen)
    , "test");
    var parser = Parser.init(&lex, alloc);
    var module = try parser.parse_module();
...
test "codegen: typed extended math module calls lower to native f64" {
    ...
    var lex = Lexer.init(
        \\local x: f64 = 1.5
        \\local deg: f64 = math.deg(x)
        \\local rad: f64 = math.rad(90.0)
        \\local logv: f64 = math.log10(100.0)
        \\local h: f64 = math.sinh(x) + math.cosh(x) + math.tanh(x)
        \\print(deg, rad, logv, h)
    , "test");
    var parser = Parser.init(&lex, alloc);
    var module = try parser.parse_module();
    var semantic = sema.Sema.init(alloc);
    ...
    try testing.expect(std.mem.indexOf(u8, output, "180.0 / 3.14159265358979323846") != null);
    try testing.expect(std.mem.indexOf(u8, output, "double rad = ") != null);
    try testing.expect(std.mem.indexOf(u8, output, "3.14159265358979323846 / 180.0") != null);
    try testing.expect(std.mem.indexOf(u8, output, "double logv = log10(") != null);
    try testing.expect(std.mem.indexOf(u8, output, "sinh(x)") != null);
    try testing.expect(std.mem.indexOf(u8, output, "cosh(x)") != null);
    try testing.expect(std.mem.indexOf(u8, output, "tanh(x)") != null);
    try testing.expect(std.mem.indexOf(u8, output, "double deg = lua_to_num(lua_math_deg(") == null);
    try testing.expect(std.mem.indexOf(u8, output, "double rad = lua_to_num(lua_math_rad(") == null);
    try testing.expect(std.mem.indexOf(u8, output, "double logv = lua_to_num(lua_math_log10(") == null);
    try testing.expect(std.mem.indexOf(u8, output, "lua_Value deg = ") == null);
    try testing.expect(std.mem.indexOf(u8, output, "lua_Value rad = ") == null);
}

test "codegen: typed boxed math module calls unbox runtime results" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
...
test "codegen: typed ffi module calls unbox boxed runtime results" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    var lex = Lexer.init(
        \\local sz: i64 = ffi.sizeof("int")
        \\local al: i64 = ffi.alignof("int")
        \\local off: i64 = ffi.offsetof("S", "field", 0)
        \\local ok: bool = ffi.istype("int", 0)
        \\local err: i64 = ffi.errno(0)
        \\local s: str = ffi.string(0, 0)
        \\print(sz, al, off, ok, err, s)
    , "test");
    var parser = Parser.init(&lex, alloc);
    var module = try parser.parse_module();
    var semantic = sema.Sema.init(alloc);
...
test "codegen: typed os module calls unbox boxed runtime results" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
...
test "codegen: typed coroutine and debug module calls unbox boxed runtime results" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    var lex = Lexer.init(
        \\local st: str = coroutine.status({})
        \\local y: bool = coroutine.isyieldable()
        \\local closed: bool = coroutine.close({})
        \\local tb: str = debug.traceback()
        \\print(st, y, closed, tb)
    , "test");
    var parser = Parser.init(&lex, alloc);
    var module = try parser.parse_module();
    var semantic = sema.Sema.init(alloc);
    ...
    try testing.expect(std.mem.indexOf(u8, output, "lua_Value tb = ") == null);
}

test "codegen: typed jit module calls unbox boxed runtime results" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    var lex = Lexer.init(
        \\local enabled: bool = jit.status()
        \\local version: f64 = jit.version_num()
        \\print(enabled, version)
    , "test");
    var parser = Parser.init(&lex, alloc);
    var module = try parser.parse_module();
    var semantic = sema.Sema.init(alloc);
...
test "codegen: typed net.send lowers native fd and string without boxing" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    ...
    var lex = Lexer.init(
        \\local fd: i64 = 3
        \\local msg: str = "ping"
        \\local sent: i64 = net.send(fd, msg)
        \\print(sent)
    , "test");
    var parser = Parser.init(&lex, alloc);
    var module = try parser.parse_module();
    ...
    try testing.expect(std.mem.indexOf(u8, output, "duo_net_tcp_send(lua_val_from_int((int64_t)(fd)), lua_val_from_str(msg))") == null);
}

test "codegen: typed net.send fallback unboxes boxed runtime result" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    ...
    var lex = Lexer.init(
        \\local sock = { fd = 3 }
        \\local sent: i64 = net.send(sock.fd, "x")
        \\print(sent)
    , "test");
    var parser = Parser.init(&lex, alloc);
    var module = try parser.parse_module();
...
test "codegen: typed net.close lowers native fd without boxing" {
    ...
    try testing.expect(std.mem.indexOf(u8, output, "duo_net_tcp_close(lua_val_from_int((int64_t)(fd)))") == null);
}

test "codegen: dynamic net.close keeps boxed runtime path" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
...
test "codegen: @c.call emits direct C calls in typed contexts" {
    ...
    try testing.expect(std.mem.indexOf(u8, output, "int64_t n = llabs(x);") != null);
    try testing.expect(std.mem.indexOf(u8, output, "take(llabs(x))") != null);
    try testing.expect(std.mem.indexOf(u8, output, "lua_to_num(llabs(x))") == null);
    try testing.expect(std.mem.indexOf(u8, output, "__c_call") == null);
}

test "codegen: @c.import emits header include for direct C calls" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    ...
    try testing.expect(std.mem.indexOf(u8, output, "return fabs(x);") != null);
    try testing.expect(std.mem.indexOf(u8, output, "__c_call") == null);
}

test "codegen: @c.type emits external C type names" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    ...
    var lex = Lexer.init(
        \\@c.include("stdio.h")
        \\fun demo(): i64
        \\  local p: *@c.type("FILE") = @c.call("tmpfile")
        \\  local closed: i64 = @c.call("fclose", p)
        \\  return closed
        \\end
        \\print(demo())
    , "test");
    ...
    try testing.expect(std.mem.indexOf(u8, output, "FILE* p = tmpfile();") != null);
    try testing.expect(std.mem.indexOf(u8, output, "int64_t closed = fclose(p);") != null);
    try testing.expect(std.mem.indexOf(u8, output, "lua_Value p = tmpfile()") == null);
    try testing.expect(std.mem.indexOf(u8, output, "__c_type") == null);
}

test "codegen: @c.export emits exported native symbol name" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
...
test "codegen: @ builtin aliases emit existing intrinsic paths" {
    ...
    const alloc = arena.allocator();
    var lex = Lexer.init(
        \\local folded: i64 = @constexpr(10 + 5)
        \\local branch: i64 = @comptime_if(true, folded, 0)
        \\local ty: str = @type_name(branch)
        \\local bits: i64 = @popcount(0xff)
        \\@static_assert("sizeof(int64_t) == 8", "i64 size")
        \\print(branch + bits)
        \\print(ty)
    , "test");
    var parser = Parser.init(&lex, alloc);
    ...
    const output = aw.written();
    try testing.expect(std.mem.indexOf(u8, output, "int64_t folded = 15;") != null);
    try testing.expect(std.mem.indexOf(u8, output, "int64_t branch = folded;") != null);
    try testing.expect(std.mem.indexOf(u8, output, "int64_t bits =") != null);
    try testing.expect(std.mem.indexOf(u8, output, "__builtin_popcountll") != null);
    try testing.expect(std.mem.indexOf(u8, output, "_Static_assert(sizeof(int64_t) == 8, \"i64 size\")") != null);
    try testing.expect(std.mem.indexOf(u8, output, "@constexpr") == null);
...
test "codegen: record literal fields unbox into typed record params" {
    ...
    const alloc = arena.allocator();
    var lex = Lexer.init(
        \\type Pair = { x: i64, y: i64 }
        \\fun sum_pair(p: Pair): i64
        \\  return p.x + p.y
        \\end
        \\local boxed = { x = 41, y = 1 }
        \\local named: i64 = sum_pair({ x = boxed.x, y = boxed.y })
        \\local positional: i64 = sum_pair({ boxed.x, boxed.y })
...
test "codegen: dynamic locals unbox into typed call params" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
...
test "codegen: typed multi-return locals unbox from lua result buffer" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
...
test "codegen: typed __emit bypasses lua_to_num on return and locals" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    ...
    var lex = Lexer.init(
        \\fun raw_i64(): i64
        \\  local x: i64 = __emit("(int64_t)(40 + 2)")
        \\  return __emit("(int64_t)(x + 1)")
        \\end
    , "test.duo");
    var parser = Parser.init(&lex, alloc);
...
test "wasm: validateTarget blocks threaded scheduler on wasm32-wasi" {
    ...
    try testing.expectError(
        error.WasmThreadsUnsupported,
        AsyncLower.validateTarget(true, true),
    );
}

test "wasm: validateTarget allows async-only (no threads) on wasm32-wasi" {
    const AsyncLower = @import("async_lower.zig");
    try AsyncLower.validateTarget(true, false);
}
...
test "expr_type: named record alias field fallback recovers declared field type" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    ...
    fields[0] = .{ .name = "x", .typ = .i64 };
    fields[1] = .{ .name = "label", .typ = .str };
    try cg.record_aliases.put(testing.allocator, "Point", .{ .table_type = .{ .fields = fields } });
    try cg.note_local_type("p", .{ .@"struct" = .{ .name = "Point" } });

    const loc = ast.Loc{ .file = "test", .line = 1, .col = 1 };
    var p_name = ast.Expr{ .name = .{ .loc = loc, .ident = "p" } };
    var x_field = ast.Expr{ .field = .{ .loc = loc, .obj = &p_name, .field = "x" } };
    var label_field = ast.Expr{ .field = .{ .loc = loc, .obj = &p_name, .field = "label" } };

    try testing.expectEqual(RT.i64, cg.expr_type(&x_field));
    try testing.expectEqual(RT.str, cg.expr_type(&label_field));
}

test "codegen: generic type alias resolves through normal type syntax" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    ...
    const tparams = try alloc.alloc(ast.TypeExpr, 1);
    tparams[0] = .{ .named = "T" };
    const list_base = try alloc.create(ast.TypeExpr);
    list_base.* = .{ .named = "List" };
    const list_args = try alloc.alloc(ast.TypeExpr, 1);
    list_args[0] = .{ .named = "T" };
    const alias = try alloc.create(ast.AliasDef);
    alias.* = .{
        .loc = .{ .file = "test", .line = 1, .col = 1 },
        .name = "Vec",
        .type_params = tparams,
        .target = .{ .generic = .{ .base = list_base, .params = list_args } },
        .fields = &.{},
        .methods = &.{},
    };
    try cg.alias_defs.put(alloc, "Vec", alias);

    const vec_base = try alloc.create(ast.TypeExpr);
    vec_base.* = .{ .named = "Vec" };
    const vec_args = try alloc.alloc(ast.TypeExpr, 1);
    vec_args[0] = .{ .named = "i64" };
    const resolved = cg.resolve_type(.{ .generic = .{ .base = vec_base, .params = vec_args } });

    try testing.expect(resolved == .array);
    try testing.expect(resolved.array.elem.* == .i64);
}
...
test "expr_type: structural fallback recovers literal and function expression types" {
    ...
    const func_type = cg.expr_type(&func_expr);
    try testing.expect(func_type == .func);
    try testing.expectEqual(@as(usize, 1), func_type.func.params.len);
    try testing.expectEqual(RT.i64, func_type.func.params[0]);
    try testing.expectEqual(RT.str, func_type.func.ret.*);
    try testing.expect(!func_type.func.has_vararg);
}

test "codegen: closure programs emit runtime JIT tables" {
    const Lexer = @import("lexer.zig").Lexer;
    const Parser = @import("parser.zig").Parser;
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    ...
    try testing.expect(std.mem.indexOf(u8, output, "duo_jit_dispatch") != null);
    try testing.expect(std.mem.indexOf(u8, output, "duo_jit_pack_") != null);
    try testing.expect(std.mem.indexOf(u8, output, "lua_jit_init") != null);
}
...
test "arc: enum-typed locals do not retain or release whole enum values" {
    ...
    try testing.expect(std.mem.indexOf(u8, output, "duo_retain((void*)(circ))") == null);
...
test "sieve native specialization uses wheel-6 byte flags" {
    ...
    try testing.expect(std.mem.indexOf(u8, output, "if (n < 5) return 2;") != null);
    try testing.expect(std.mem.indexOf(u8, output, "__sieve_q = n / 6") != null);
    try testing.expect(std.mem.indexOf(u8, output, "__sieve_len = 2 * __sieve_q") != null);
    try testing.expect(std.mem.indexOf(u8, output, "uint8_t* __restrict __sieve_alloc") != null);
    try testing.expect(std.mem.indexOf(u8, output, "__builtin_assume_aligned(__sieve_alloc, 16)") != null);
    try testing.expect(std.mem.indexOf(u8, output, "memset(__sieve, 1, (size_t)__sieve_len)") != null);
    try testing.expect(std.mem.indexOf(u8, output, "for (int64_t i = 4; i <= n; i += 2)") == null);
    try testing.expect(std.mem.indexOf(u8, output, "i += __sieve_istep, __sieve_istep = 6 - __sieve_istep") != null);
    try testing.expect(std.mem.indexOf(u8, output, "__sieve[i / 3 - 1]") != null);
    try testing.expect(std.mem.indexOf(u8, output, "__sieve_mark_idx = (i * i) / 3 - 1") != null);
    try testing.expect(std.mem.indexOf(u8, output, "if (__sieve_istep == 4)") != null);
    try testing.expect(std.mem.indexOf(u8, output, "if (i % 6 == 1)") == null);
    try testing.expect(std.mem.indexOf(u8, output, "__sieve_delta_a = ((i << 2) - 1) / 3") != null);
    try testing.expect(std.mem.indexOf(u8, output, "__sieve_delta_b = ((i << 1) + 1) / 3") != null);
