...
pub const AliasRegistry = struct {
    ...
    pub fn getRecord(self: *const AliasRegistry, record: *const ast.TypeExpr.RecordType) ?*const ast.AliasDef {
        var aliases = self.map.valueIterator();
        while (aliases.next()) |entry| {
            const target = entry.*.target orelse continue;
            if (target == .record and target.record == record) return entry.*;
        }
        return null;
    }
...
/// `field == null` is the subject value itself; otherwise it is the exact
/// descriptor member ordinal. One fact stream keeps those two uses from
/// acquiring parallel indexes.
...
pub const Sema = struct {
    ...
    relation_subjects: std.ArrayListUnmanaged(RelationSubjectFact) = .empty,
    ...
    /// WHICH WORLDS REACH THIS COMPILATION, supplied by the launcher as exact
    /// identities. Source provenance cannot answer this question.
    fn inhabitsWorld(self: *const Sema, world: subject_home.Home) bool {
        return subject_home.worldReached(self.worlds.slice(), world);
    }
    ...
    fn is_builtin_global(self: *const Sema, name: []const u8) bool {
        ...
        // hardcoded five-name disjunction below; `test` requires a launcher
        // witness and was a hardcoded special case above it. One question, asked of
        // the declaration, so a world added to the table is nameable with no
        // edit here — and a world that does not reach this launch is an ordinary
        // word in it.
        ...
        //
        // AND IT IS ASKED OF THE LAUNCH-WORLD SET. Which member edges are bare
        // follows the exact grants the launcher supplied; source provenance is
        // not consulted here.
        //
        ...
        if (subject_home.injectedWorldProvidingFor(self.worlds.slice(), name) != null)
            return self.userBinding(name) == null;
    ...
    pub fn init(alloc: Allocator) Sema {
        return .{
            ...
            .worlds = subject_home.injectedWorlds(),
        };
    }

    pub fn grantWorld(self: *Sema, world: subject_home.Home) void {
    ...
    pub fn relationSubjectFact(self: *const Sema, relation: *const ast.FuncDecl) ?RelationSubjectFact {
        for (self.relation_subjects.items) |fact| if (fact.relation == relation) return fact;
        return null;
    }

    pub fn relationSubjectFacts(self: *const Sema) []const RelationSubjectFact {
        return self.relation_subjects.items;
    }
    ...
    /// publication uses this accessor only to project that home's descriptor
    /// declarations into graph facts. Two different modules claiming one home
    /// make the answer ambiguous and therefore absent.
    pub fn resolvedHome(self: *const Sema, home: []const u8) ?ForeignHome {
        var found: ?ForeignHome = null;
        var it = self.foreign_homes.valueIterator();
        while (it.next()) |entry| {
            const candidate = entry.* orelse continue;
            if (!std.mem.eql(u8, candidate.home, home)) continue;
            if (found) |prior| {
                if (prior.module != candidate.module) return null;
            } else {
                found = candidate;
            }
        }
    ...
    fn worldSubject(self: *const Sema, expr: *const ast.Expr) ?subject_home.Home {
        ...
        return subject_home.worldNamedFor(self.worlds.slice(), ident);
    }
    ...
    fn refuseOsDotFace(self: *Sema, loc: ast.Loc, obj: *const ast.Expr, field: []const u8) bool {
        ...
        const world = subject_home.worldNamedFor(self.worlds.slice(), "os") orelse return false;
    ...
    fn refuseBareStreamRelation(self: *Sema, loc: ast.Loc, name: []const u8) bool {
        ...
        switch (subject_home.bareReachFor(self.worlds.slice(), name)) {
            .ambiguous => |pair| {
                self.err(
                    loc,
    ...
    fn methodCallResolved(
        ...
        if (obj.* == .name and self.userBinding(obj.name.ident) == null) {
            if (subject_home.suppliedInstanceFor(self.worlds.slice(), obj.name.ident)) |supplied| {
        ...
        return switch (expr.*) {
            ...
            .name => |n| {
                if (n.role == .subject) {
                    ...
                    return .{ .@"struct" = .{ .name = subject.home.name } };
                }
                if (self.current_subject) |subject_index| {
                    const subject = &self.relation_subjects.items[subject_index];
                ...
                if (subject_home.injectedWorldProvidingFor(self.worlds.slice(), n.ident) != null and
    ...
    fn check_func_decl(self: *Sema, fd: *ast.FuncDecl) SemaError!void {
        ...
        if (fd.subject != null or (fd.path.len >= 2 and fd.method)) {
            ...
            try self.relation_subjects.append(self.alloc, fact);
            subject_index = @intCast(self.relation_subjects.items.len - 1);
        }
        const has_vararg = fb.vararg or fb.vararg_name != null;
        var all_typed = true;
        for (fb.params) |*p| {
        ...
        if ((fd.path.len >= 2 and fd.method) or subject_index != null) {
            const table_name = if (subject_index) |index| self.relation_subjects.items[index].home.name else fd.path[0];
        ...
        self.current_subject = subject_index;
        defer {
            self.current_ret = prev_ret;
            self.current_ret_fallible = prev_fallible;
            self.current_nopanic = prev_nopanic;
    ...
    fn subjectField(self: *Sema, subject: *const RelationSubjectFact, field_name: []const u8) SemaError!?SubjectField {
