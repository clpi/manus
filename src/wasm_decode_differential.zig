...
test "wasm_decode_differential: known opcode count is 64" {
    try std.testing.expect(wasm_semantic.knownOpcodeCount() == 64);
    try std.testing.expect(wasm_semantic.mvpCount() == 64);
}
