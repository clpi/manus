...
pub const items: []const MigrationItem = &.{
    ...
    .{ .id = "MP-06", .title = "Duo-native parser kernel (bounded subset)", .workstream = "P16-WS6", .subsystem_id = "SH-04", .status = .partial, .depends_on = &.{"MP-04"}, .owner = "lib/std/compiler/parser.duo", .validation = "Positive + compile-fail corpus differential", .removal_implication = null },
