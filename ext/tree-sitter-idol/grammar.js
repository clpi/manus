 ...
 *                                 ext/tree-sitter-idol/src. That hole is why
...
module.exports = grammar({
  ...
  rules: {
    ...
    // `#!/usr/bin/env idol`. src/lexer.zig recognises `#!` only at index 0 and
    ...
    // Idol has no statement terminator, but `;` is a token (`semi`) and 16
    ...
    // Canonical `#` comment. Prec below `##` so hash_hash stays an operator.
    hash_comment: $ => token(prec(-1, seq('#', /[^#!\n][^\n]*/))),
    compat_comment: $ => token(seq('--', /[^\n]*/)),
    compat_long_comment: $ => token(seq('--', '[', repeat('='), '[',
      repeat(choice(/[^\]]/, seq(']', /[^\]]/))),
      ']', repeat('='), ']')),
  },
});
