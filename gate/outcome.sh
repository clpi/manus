...
for f in zero seven trap arg; do
    [ -f "$fixture/$f.id" ] || fail "fixture $fixture/$f.id is missing"
done

cd "$work" || fail "cannot enter $work"

# The holding guest lives HERE and not in `examples/`: it never terminates by
...
outcome() {
    ...
    _status=0
    if [ "$#" -gt 0 ]; then
        "$idol" run "$_src" -- "$@" >"$_name.out" 2>"$_name.err" || _status=$?
    else
        "$idol" run "$_src" >"$_name.out" 2>"$_name.err" || _status=$?
    fi
    printf '%s\n' "$_status"
...
got=$(outcome zero "$fixture/zero.id")
answers "$got" 0 || fail "arm 1: zero.id reported $got, its outcome is 0"
got=$(outcome seven "$fixture/seven.id")
answers "$got" 7 || fail "arm 1: seven.id reported $got, its outcome is 7"
if named seven RUN; then
    fail "arm 1: a program that exited named a refusal identity"
fi

printf 'outcome gate: arm 1 exit 0 and exit 7 discriminated through idol run\n'
...
#
# hold.id prints one line and then never stops. The line is the proof that a
# guest is executing; the signal goes to the runner process that carries it.

"$idol" run "$work/hold.id" >live.out 2>live.err &
held=$!
alive=0
i=0
while [ "$i" -lt 80 ]; do
...
"$idol" compile --backend=wasm "$fixture/seven.id" -o "$work/seven.artifact" \
    >compile.log 2>&1 ||
...
got=0
WASMTIME_BIN=/nonexistent/wasm-runner "$idol" run "$fixture/zero.id" \
...
got=0
WASMTIME_BIN="$work/reject" "$idol" run "$fixture/zero.id" \
...
got=0
WASMTIME_BIN="$work/carry" "$idol" run "$fixture/zero.id" \
