#!/bin/sh
# thin driver: the gate logic lives in gate/gap-111-map-ambiguity.id
...
if [ ! -x "$idol" ]; then
    printf 'gap-111-map-ambiguity gate: FAIL compiler is not executable: %s\n' "$idol" >&2
    exit 1
fi
CDPATH='' cd -- "$root" || exit 2
exec "$idol" run --backend native gate/gap-111-map-ambiguity.id
