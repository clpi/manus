| field | value |
|---|---|
| title | GAP-145 collapse-site classification — sema.zig (measured, read-only) |
...
| # | directive |
|---|---|
| 1 | `.quoted` in switch arms (selection — correct) |
| 2 | `.quoted.val` in directive/error contexts reading a name (e.g., `mem_type_from_name(args[index].quoted.val)`) — these consume the text as a NAME, not as a string value; the quote fact is semantically irrelevant for names |
