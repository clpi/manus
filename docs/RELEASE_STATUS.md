...
`71e397f` (stated per section — three other sessions were landing work while
this ran, so each number below names the commit it was taken at). Host:
...
| Pass 100 deny table | `zig build audit100` | **PASS**, 18 rows at budget |
...
| Duo-vs-C benchmark | `zig build bench` | **RED by its own criterion** — see §4 |
...
## 4. Benchmark position — 24 W / 16 L, and what that number is worth

`zig build bench` at `cafa149`, in an isolated worktree.
`BENCH_MANIFEST backend=c-specialized representation=specialized
...
**Speed: Duo wins 24 rows, reference C wins 16.** `zig build bench` therefore
**exits 1**, because the gate's criterion is "Duo must beat or tie C on *every*
test". The suite is red, on purpose, and it is red honestly.
...
### The caveat that must be read with the 24

**13 of the 24 Duo "wins" are at or below timer resolution** — the `.duo`
column reads `0`, `1e-06` or `6e-06` while reference C reads `1e-04` to
`3.5e-02`: Fibonacci(40), Table array, Filter count, Clamp sum, Bucket hash,
EMA smooth, Table lookup, Table churn, XOR fold, Fenwick tree, CORDIC sin,
Bitcount, and String bytes. Their *values* agree with C, so nothing is
fabricated — but **a row that reports zero seconds is not a measured win**, and
the honest reading of this suite is "11 measured wins, 13 rows where the
compiler evaluated the benchmark rather than ran it, 16 losses".
...
2. **The benchmark gate is red**, and 13 of the 24 winning rows are below timer
   resolution. The suite proves **correctness** (40/40 against reference C). It
   does not currently prove a speed claim, and no speed claim should be
   published from it without the §4 caveat attached.
...
8. **`ext/tree-sitter-duo/grammar.js` is 804 lines of hand-authored
   JavaScript** — the only non-Duo, non-generated source in the toolchain, and a
   live monoglot violation. Pass 101 requires it to be a generated projection.
9. **The shared working tree is not a measurement surface.** Concurrent sessions
   caused one transient gate failure during this very pass. Any release
   measurement must be taken in a detached worktree at a pushed commit — this
   document's were.
...
clean-directory project loop and both lexer differentials are green. `duo init`
→ `check` → `build` → `run` works on an empty directory and is gated. All 40
benchmark correctness rows agree with reference C. Ward executes 43 of 44 WASM
