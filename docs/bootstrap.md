...
| Lexer ABI schema | IDOL OWNED, GENERATED HOST BRIDGE | `RECORD_SLOTS` / `lexErrorFromCode` / `tokenKindFromOrdinal`, the host-authored `TokenKind` enum, and `kindFromRecord` are deleted. Record decode validates directly through the owner-generated sparse enum, without a runtime grammar-row lookup. Three parser decisions consume compact producer facts without Zig role logic. Remaining bridge debt: almost every parser token still carrying the generated enum, and the `duo_lexer_*` / `useDuoTokens` slice bridge (`law.bridge.death`, GAP-107). |
...
- **NEXT:** `GAP-145` remaining is Tree-sitter `grammar.json` and semantic
  ...
  and deleting the generated-enum token slice → `useDuoTokens` bridge without
