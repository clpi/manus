...
| Lexer ABI schema | IDOL OWNED, GENERATED HOST BRIDGE | `RECORD_SLOTS` / `lexErrorFromCode` / `tokenKindFromOrdinal` and the host-authored `TokenKind` enum are deleted. Producer ordinals index `src/grammar_role_table.zig`, generated by `lib/compiler/token.id`; `src/lexer.zig` aliases that physical enum. Remaining bridge debt: parser tokens still carry the generated enum instead of the producer pack's token-role identity directly, plus `duo_lexer_*` / `useDuoTokens` names (`law.bridge.death`, GAP-107). |
...
| Grammar roles | IDOL OWNED, GAP-134 partial | `lib/compiler/token.id` is the **one executable grammar-fact owner** (`law.grammar.one`): token identities; roles; precedence and associativity; relation and prefix identity; and infix, update, and glued source faces. It emits `src/grammar_role_table.zig` (host bridge the production parser reads, `law.bridge.death`) and `lib/token/grammarrole.id` (Idol projection). `src/grammar_roles.zig` is accessors only and holds no fact; `src/ast.zig` aliases the generated relation ontology. The former parser infix/update/glued/unary maps and the pretty-printer's authored inverse are deleted; totality and injectivity of the generated inverse are compile-time checked. `src/grammar_role_gen.zig` and `src/emit_grammar_role.zig` are deleted. `zig build grammar-projection` (on `test`) fails unless both artifacts regenerate byte-identically. Canonical `return` observes `begin_expr`, match-arm discovery observes `pattern`, and the untyped-header body decision observes `body_start` through the immutable production token view — each a bounded decision-class transfer, not parser recognition. The immutable prerequisite `lib/compiler/token_view.id` now checks clean and `dump-c --lib` emits a `cc -fsyntax-only` clean artifact on this DNB004 host; `lib/compiler/parser.id` still does not. The first consumer exposed and repaired an incomplete producer domain rather than adding a parser-local exception. Independent one-row damage makes `return 99`, a consecutive integer match arm, or an untyped header with a `return` body refuse. NOT closure: only bounded recognition decisions consume these facts; `docs/spec/grammar.md` still does not generate the parser; Tree-sitter still contains authored literal/comment recognition; the host `TokenKind` bridge remains; and parser recognition is still host-executed. |
...
- **NEXT:** `GAP-145` remaining is Tree-sitter `grammar.json` and semantic
  ...
  grammar.md as the generatable owner and crossing parser-side linkability after
  GAP-145 reaches zero observers. Header recognition is one
  ...
  recognizer that consumes those facts remains host-executed.
