...
The production front end nevertheless has two bounded executed Idol-owned
boundaries: the lexer producer and one callable-header recognizer. The executed
...
also role-owned. Callable-header classification now executes
`lib/compiler/parser.id` `header_signal_lx` over a compact projection of that
producer pack; the 209-line Zig recognizer is deleted. The gap is not closed. Porting the
...
| Grammar roles | IDOL OWNED, GAP-134 partial | `lib/compiler/token.id` is the **one executable grammar-fact owner** (`law.grammar.one`): token identities; roles; precedence and associativity; relation and prefix identity; and infix, update, and glued source faces. It emits `src/grammar_role_table.zig` (host bridge the production parser reads, `law.bridge.death`) and `lib/token/grammarrole.id` (Idol projection). `src/grammar_roles.zig` is accessors only and holds no fact; `src/ast.zig` aliases the generated relation ontology. The former parser infix/update/glued/unary maps and the pretty-printer's authored inverse are deleted; totality and injectivity of the generated inverse are compile-time checked. `zig build grammar-projection` fails unless both artifacts regenerate byte-identically. Canonical `return`, match-arm discovery, and the untyped-header body decision consume owner facts through the immutable view. `lib/compiler/parser.id` `header_signal_lx` now executes the header decision in production from the same facts; its tracked C projection is byte-gated and semantically probed. NOT closure: `docs/spec/grammar.md` still does not generate the parser, the generated `TokenKind` bridge remains, and nearly all parser recognition is host-executed. |
| Parser recognition | IDOL OWNED SLICE, HOST MAJORITY | `lib/compiler/parser.id` `header_signal_lx` classifies callable headers from compact producer ordinal/line/column facts; `src/parser.zig` supplies the immutable physical view and no header rule. The former 209-line Zig recognizer is deleted. `src/parser.zig` still decides every other expression, binding, source-structure, and AST-construction edge. This is one production relation, not a complete parser stage or Compiler B. |
...
- **NEXT:** `GAP-145` remaining is Tree-sitter `grammar.json` and semantic
  ...
  enum binding remain explicit bootstrap bridges. `GAP-134` now executes one
  parser relation; remaining work is closing grammar.md as the generatable owner
  and moving another bounded decision over the same pack without outrunning
  GAP-145. Callable-header recognition is `parser.id` `header_signal_lx`; Pratt
