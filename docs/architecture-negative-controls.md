...
| **RESOLUTION-ORDER-INDEPENDENT** | Reordering home declaration must not change meaning | Sema collects all matches; no first-wins | **partial** — ambiguity when multiple |
...
| **RESOLUTION-PERMUTATION** | Reorder home/file enumeration → semantics unchanged | Companion probe (multi-home fixture) | **planned** |
| **HOME-MOVE** | Move relation physically → semantic application unchanged | Companion probe | **planned** |
| **REPRESENTATION-HISTORY** | Exploded locals vs aggregate → same realization set | Companion probe | **planned** |
