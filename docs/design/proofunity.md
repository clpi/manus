...
Kernel vocabulary census (23 words, all LAW-16 clean): `span`, `first`,
`rest`, `nth`, `push`, `num`, `of`, `check`, `term`, `holes`, `ctx`,
`prop`, `head`, `tail`, `plug`, `goal`, `intro`, `exact`, `apply`,
`split`, `left`, `right`, `qed`. (Each resolves to an
...
**Frontier 1 — tactic scripts as ordinary relation chains.** In Coq,
tactics are an imperative sublanguage (Ltac) operating on an unnamed,
ambient goal. In Lean, `by` blocks are metaprograms over an elaborator
monad. In Idol the goal state is a *named value* and each tactic is a
relation: `p:goal():intro():apply("2"):exact("1"):qed()` is one
expression in the same language as the program it proves things about.
There is no proof mode to enter and no second grammar to learn — the
term face (`"LLLA2A31"`) and the tactic face are the same values. No
other proof assistant makes the proof state a first-class subject of
`subject:edge(rest)` composition.
...
- **Phase 1 — kernel (done).** String-encoded propositions, proof terms
  ...
  tactic set. Files: `lib/proof/prop.id` (scanner), `lib/proof/check.id`
  (checker), `lib/proof/tac.id` (tactics). Tests: `test/proof/main.id`
