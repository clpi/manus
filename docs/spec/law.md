...
Syntax never encodes the answer. `f(x)`, `descriptor(value)`, `world(key)`,
`closure(x)`, and `foreign(x)` all begin as the same question; computed access
`table[key]` is projection and converges to the same graph machinery after
recognition.
...
Computed access is projection: `table[key]`. Not `table(key)`, not
...
uniformity — the projection itself is enough.

Static `.` and computed `[]` assert different author knowledge and stay
distinct source faces: `user.name` asserts one statically known structural
projection; `user[key]` is computed projection with a runtime key. If the
compiler proves both denote the same member they may converge to one semantic
