...
Epoch 2. This file is **machine-read** by `scripts/foreign_census.id`
...
ledger   ext/vscode-duo/extension.js   until:the VS Code host can consume a duon projection directly; generated by scripts/vscodeemit.id and checked by scripts/vscodeproj.id
ledger   ext/tree-sitter-duo/grammar.js until:the tree-sitter host can consume a duon projection directly; generated by scripts/treesitter_emit.id and checked with TSEMIT_CHECK=1
...
No tracked foreign file is currently unclassified. `ext/vscode-duo/extension.js`
and `ext/tree-sitter-duo/grammar.js` are ledger rows because their Duon
generators are byte-identity checked against the tracked outputs.
