...
law: {
    ...
    binds: seq(str)
    fails: str
    -- Widened 2026-08-09. APPLY-ONE began VALIDATING this file against this
    -- descriptor — a braced application now checks its labels against the
    -- subject's fields — and it rejected twelve labels the laws were already
    -- using. The DESCRIPTOR was the stale half: `why`, `deny`, `canon` and the
    -- rest were in the corpus long before a checker could see them. Widened to
    -- match measured use rather than narrowing the laws to match a guess; the
    -- corpus is the evidence of what a law actually needs to say.
    --
    -- The constitution now type-checks against its own descriptor, which is
    -- law.doc.corpus arriving mechanically instead of by discipline.
    why: any
    deny: any
    canon: any
    keep: any
...
--
-- Recorded 2026-08-08 after a concrete failure that this file exists to make
-- impossible. `ward@dominates(wart)` was retired by owner directive; a later
-- session RESTORED it citing "higher pass wins", because the epoch protocol
-- ranked pass documents without ranking a directive against one. A pass
-- document is GENERATED BY AN AGENT. A directive is not. So "higher pass wins"
-- silently ranked agent output above the person the passes are written for.
--
-- Any pass text contradicting a recorded directive is VOID ON ITS FACE and
-- needs no adjudication. A ruling that cannot cite a directive may not
-- overturn one.
...
entity = law{
    ...
    holds = .graph
    binds = {
        "the exact graph entity or its owner-defined compact handle is semantic identity within one graph incarnation",
...
selfhost = {
    slice{ 1, "lexer", "substantially demonstrated by the native corpus" },
    slice{ 2, "grammar", "must consume the GENERATED constitutional grammar" },
    slice{ 3, "identity", "binding + stable semantic identity — BEFORE any optimizer migration" },
...
-- Measured: `addChild` at src/semantic_graph.zig:297 sets `n.scope = parent`,
-- then calls `addNode`, then `addEdge(.contains)`. The source itself records
-- that the two used to drift apart, which is the tell. `parent contains child`
-- is the FACT; `child.scope`, `parent.children` and ancestry are PROJECTIONS.
-- So `addChild` has no semantic existence at all: `parent:add(contains)(child)`.
...
-- descriptor gate specialized emitters, 28 carry a checking companion, 42
-- recognizers feed them, and 51 further sites spell assume, noalias, pure or
-- likely. Six ad-hoc trust mechanisms grown by accretion rather than decision —
-- not one of them a fact, not one witnessed, not one composable.
--
-- `asserted` is the level the tree lacks entirely, and its absence is precisely
-- why nobody could enumerate the unchecked flags: there was no way to say "I
-- know this and you cannot prove it" and have the claim RECORDED, COUNTED,
-- INVALIDATABLE and ATTRIBUTABLE when it turns out false. An assumption wearing
-- no label is indistinguishable from a proof right up until it answers 200000
-- where the reference answers 100000, which this repository has already paid
-- for once.
trust = @{
...
-- tell them apart.
--
-- One correction to GAP-085, which proposes `x@{ alias = false }` by name:
-- `alias` is a RESERVED WORD and cannot name a field — measured, the parser
-- rejects it where `sharing`, `reentry`, `failure`, `observer` and `effect` all
-- pass. The negative fact is `sharing`, and this is the ordinary case of a
-- proposal being a proposal and the measurement being the law.
absence = @{
...
-- Package composition is a SEMANTIC exercise owed now, not package-system
-- detail owed later — the registry gate is currently a gate with no referent.
...
--
-- Four distinctions that follow, each of which the tree currently violates in a
-- measurable way (2026-08-08):
--
--   1. A function does not become conceptually different because it arrived
--      from another package.
--   2. A standard operation does not need `std.` because of where its
--      implementation FILE lives.       -- 3549 `std.*` sites, 126 homes
--   3. A package namespace is not a substitute for relation identity.
--   4. IMPORTING CODE IS NOT GRANTING CAPABILITY.  -- 278 ambient sites
...
authority.grant = law{
    ...
    fails = "import as permission — 278 ambient sites measured"
}
...
-- MEASURED BASELINE 2026-08-08, so the ratios below start honest.
--
--   CLAUDE.md states the invariant "zero Lua boxed values in the hot path".
--   Measured in the bootstrap: 1272 `lua_Value` sites, 4399 `lua_*` emission
--   sites, 353 DISTINCT `lua_*` helpers. The invariant is aspirational and the
...
--   and `why(skip)` can be designed BEFORE the explosion rather than retrofitted
--   onto it, which is the opposite of how every incumbent arrived.
perf.ratios = {
...
-- SELF-AUDIT 2026-08-08, applying the final test to `zig build bootstrap-scan`,
-- landed hours before this section. It RATCHETS 45 `add*`/`register*`/`emit*`
-- host functions, so a 48th turns it red — the class is closed against that
-- spelling.
--
-- IT FAILS THE FINAL TEST ANYWAY. The row matches by SPELLING, so `insertThing`,
-- `internFoo`, `pushEntry`, `attachRecord` and `defineBar` all pass it while
-- being the same architectural mistake. Worse, a PERFECTLY NAMED `add` whose
-- signature still forces `add(graph, node)` is invisible to it — the orientation
-- defect the scan cannot express at all.
--
...
--
-- OWNER DIRECTIVE 2026-08-08, and it SUPERSEDES the leading-dot readings in
-- §0.2, §0.3 and §0a. Recorded under law.owner.precedence: a directive outranks
-- any generated ruling, and this one narrows laws those passes stated too
-- generally.
--
...
--
-- AMENDED 2026-08-08 by gap[087], and the amendment is the whole lesson:
-- obligation 1 is NOT satisfied by the replacement PARSING. Measured, `kind =
-- invariant` parses and does NOT resolve — it lowers to an undeclared
-- identifier — while the retirement-debt form `kind = .invariant` compiles. I
-- recorded obligation 1 satisfied on `duo check` alone and was wrong. This tree
-- already had that trap on record twice ("a resolving ambient path is not a
-- working one"), and the third time was in the file legislating against it.
subtract.proves = {
...
-- MEASURED 2026-08-08, and the surface is small enough to retire:
--
--   ordinary postfix `x.y`            24031   <- canonical, dominates
--   case in construction `= .x`         123
--   lens in argument `map(.f)`           96
--   leading `.` in arithmetic `.x * k`   36    <- SELF-ZERO's field read
--   bare `.` as value                    15
--   inferred case `== .name`             12
--   leading `:name()` invoke              6
--   `{ .field = value }`                  0    <- ALREADY ABSENT
--
-- ~290 leading-dot sites against 24031 canonical postfix. One of the four forms
-- the directive names does not exist in the tree at all.

-- THE HARD CASE, stated rather than glossed: SELF-ZERO (§0a) rests ENTIRELY on
-- leading `.`. `scale = (k) @{ .x * k, .y * k }` has no subject written down —
-- that is the whole point of there being no `self`. Retiring leading `.` and
-- keeping SELF-ZERO requires the directive's own suggestion to hold: that
-- implicit-subject fields use ORDINARY BARE IDENTITIES.
--
--   scale = (k) @{ x * k, y * k }
--
...
-- yet proven either way. 36 arithmetic sites, 15 bare `.`, 6 leading invokes:
-- 57 sites, all SELF-ZERO, all blocked on the same question.
--
-- This is recorded as OPEN rather than resolved. The other forms — 123 case
-- constructions, 96 argument lenses, 12 inferred cases — do not depend on it and
-- retire independently, which is the order to work in.
...
--
-- OWNER DIRECTIVE 2026-08-08. This RESOLVES §40's open question and SUPERSEDES
-- §0a's definition of SELF-ZERO.
--
...
-- THE COMPLETE RECEIVER MODEL. Eight rows, and every one of them earns its
-- spelling:
--
--   explicit field            p.x
--   IMPLICIT field            x
...
--
-- Bare `.` IS KEPT for the subject VALUE ITSELF — `cache:put(.)` — and now
-- means exactly ONE thing. Not case. Not lens. Not field shorthand.
punctuation = law{
    ...
    binds = {
        ...
        "except the subject VALUE `.` or the invocation boundary `:`",
    }
}
...
-- MEASURED 2026-08-08: all four forms — `kind = .invariant`, `kind = invariant`,
-- `.x * k` and `x * k` — already PARSE. So the retirement is a
-- CANONICALIZATION, not a language change, and §40's obligation 1 (a strictly
-- simpler existing spelling) is satisfied by construction for both.
--
-- §40's blocked count RESOLVES: of the 57 SELF-ZERO sites, the 36 `.x`
-- arithmetic reads retire to bare identity; the 15 bare `.` and 6 leading `:`
-- STAY, because both earn their syntax under law.subject.punctuation.
-- BLOCKED by gap[087]: the bare form does not resolve, so nothing retires yet.
subtract.resolved = "36 retire, 21 stay — BLOCKED until bare identity resolves"
...
-- THE CANONICAL-PERFORMANCE LAW, and it is the one this session measured twice.
...
--
--   gap[081]  canonical `x:to(i64)` fails both faces; noncanonical
--             `to(i64)(x)` works. 1647 sites chose the working form.
--   gap[087]  canonical bare case does not resolve; dotted does. Every site
--             in the tree uses dotted.
--
...
-- CONSTITUTION-FIRST MIGRATION (§46). THIS FILE currently teaches the forms it
-- retires — `kind = .invariant`, `holds = .facts`, `close = .compatibility`.
-- It is the FIRST canonical migration target, and **the constitution must never
-- teach deprecated syntax**.
--
-- BLOCKED, and honestly: gap[087] measured that the replacement does not
-- resolve. Migrating now would make C0 unparseable by the compiler it governs,
-- which §45 forbids by name. The migration lands the moment bare identity
-- resolves, and not before.
migrate.first = "the constitution"
migrate.blocked = "gap[087] — bare identity parses, does not resolve"

-- Grammar reconciliation (§47): `primary` eventually drops `.name`; postfix
-- `expr.name` stays; bare `.` survives ONLY as the ambient subject value. The
-- formal grammar and this file must agree, and today they do not.
grammar.owed = "primary drops .name; docs/spec/grammar.md and c0 must agree"

-- ═══ §43 · DELIMITER CLOSURE — useful source distinctions survive ═══════
--
-- OWNER DIRECTIVE 2026-08-10: brace-zero and bracket-zero are closed.
--
...
-- MEASURED 2026-08-08. 147 `name{` sites over 40 distinct names, 80 `@{`, and
-- 29 typed bindings. `law{` alone is 57 of the 147 — THIS FILE — and it only
-- ever passes `duo check`, because a constitution is data and is never run. So
-- the dominant use of the form in the tree has never exercised its lowering,
-- which is how a canonical form stayed broken while looking well-used.
brace.sites = 147
brace.names = 40
...
--
-- Appended to §44 rather than restated: a concurrent session landed
-- law.apply.one, law.construct.zero and law.pack.shape while I was writing the
-- same three. My duplicates are DELETED — two facts with one id is worse than
-- two facts that disagree, because law.fact.one cannot even name which is
-- authoritative. What follows is only what was NOT already there.
...
--
-- `src/relation.zig` is the bootstrap realization of ordinary graph relation
-- facts. **None of its object taxonomy is semantic authority.**
--
-- It currently carries `Class`, `Edge`, `Path`, `Relation`, flat edge storage,
-- STRING descriptor identities, a recognizer for the declaration shape, and an
-- explicit `derive()`. That is acceptable ONLY as deletion-gated host
-- projection, and it is named here because implementation pressure is exactly
-- how gap[084]'s 64 nouns arrived: nobody decided to build a registry, one
-- accreted. `ConversionRelation`, `ConversionEdge`, `ConversionClass` and
-- `ConversionPath` as permanent object types would be gap[084] returning under
-- cleaner names.
...
--                       descriptor names once stable semantic ids exist —
...
--
-- Measured (gap[082]): `d: feet = 3.0` is REJECTED because the initializer is
-- f64. So the conversion graph derives `feet -> micron -> inch` while the
-- compiler cannot know a runtime value IS feet.
--
...
-- MEASURED 2026-08-09: relation families with a real algebra = 1 (`to`).
-- Authored std conversion edges = essentially 0. Projection-algebra breadth is
-- roughly 10-15% realized against an architecture that is far more complete.
-- That distance is the frontier, and it is not closed by more syntax.
algebra.families = 1
...
--
-- Owner ruling, and it CORRECTS gap[100] before its 620-site migration runs.
-- gap[100] is right that `os.remove(path)` is authority masquerading as an
-- inert table. Its proposed repair — make the capability the receiver —
-- would fix reach and break orientation.
...
lift = law{
    ...
    canon = "str = view(slice(byte)) + encoding(utf8) + valid(utf8): slice lifts, put(byte) does not"
...
-- forms are genuinely one idea: OMIT AN IDENTITY THE SEMANTIC ENVIRONMENT
-- ALREADY KNOWS.
--
...
anchor = law{
    ...
    binds = {
        ...
        "@ supplies the ambient descriptor; . supplies the ambient subject",
...
anchor.brace = "@ + pack = apply(enclosing descriptor)(pack)"
...
-- §4 owns the project and source identity. This projection exists only so the
-- later authority laws refer to that one owner rather than redeclaring it.
identity = language
identity.emit = language.file
...
--
-- MEASURED 2026-08-09, `src/duo_native_ir.zig`: **45 opcodes, 26 of them
-- carrying an underscore**, and a 20-FIELD instruction record. A `mul` reserves
-- storage for callee, req alias, field, record, third result, value array,
-- branch target and hardware intrinsic — none of which apply. That is a second
-- ontology with a second naming law and a second cost model.
...
--
-- MEASURED: every compile writes `/tmp/duo_<basename>.c` with NO RUN IDENTITY.
-- With several sessions building one tree, `agent-smoke` returns 0/1/0 across
-- three identical runs and the failing row MOVES between them. Both rows pass
-- standalone.
...
evidence = law{
    ...
    why   = "the same law as everywhere else — one fact, one authority, explicit provenance — applied to the artifacts a gate reads. Three sessions today each reported a red they could not attribute, and each had to rebuild from `git archive` to find out whose it was."
}
...
-- Measured and CORRECTED already: `module_consts.ints` and function-local
-- `const_ints` encode genuinely different SCOPE facts and must not be fused.
...
--
-- Measured together they read as contradiction, and both are true: the zig
-- ledger went 237 -> 115 and sh/py to 0, while gap[062] reports the stage table
-- UNMOVED — realize and flow absent, linking still shelling to `xcrun cc` — and
-- `codegen.zig` GREW 34,780 -> 35,704.
...
gatelive = law{
    ...
    why   = "four instruments were found this week that existed and never ran, each recorded as delivered — one of them cited by TWO gaps as already existing when it had never been written. The pattern is structural, not careless."
}
...
--
-- Owner directive 2026-08-09. The pressure test was the conventional shape
...
budgetreduction = @{
    source = {
        "budget = (name: str, fallback: i64): i64",
        "    value = std.os.getenv(name)",
...
touchzero = law{
    ...
    binds = {
        ...
        "remaining debt records vocabularyblocked implementationblocked or migrationblocked with stable identity and rationale",
