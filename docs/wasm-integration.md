...
```
ext/ward/            28 tracked .id · 11,350 lines · 51 tracked non-.id (47 .wasm, 2 .wat, .json, .gitignore)
  src/ward.id       6,312 lines — THE RUNTIME. decoder + interpreter + ARM64 JIT, one file.
  src/wasm/*.id     1,802 lines — a parallel modular tree. DEAD: nothing reqs it (§1.2).
  src/nn*.id        370 lines — a tensor/NN library. Nothing to do with wasm.
  bench/             9 .id harnesses + 47 .wasm fixtures
  test/conform.id   480 lines — the conformance differential (`zig build wasm-test`)
  test/main.id      108 lines — BROKEN: reqs src.wasm, src.edge, src.nn; two do not exist
  tools/opcodes.id  551 lines — the opcode-table projector
  examples/          showcase.id — BROKEN: reqs src.lib, which does not exist
  my-ward-app/       one scaffold .wat, no consumer

lib/std/wasm/        6 .id · 1,433 lines — the LIBRARY face, `req "std.wasm.*"`
  decode.id · instruction.id · opcodes.id · wasi.id
  opcode_lookup.id        (generated — `duo wasm-tables emit`)
  ward_mvp_opcodes.id     (generated — same)
```
...
`src/ward.id`'s header records six codegen constraints that forced the
...
### 1.2 `src/wasm/*.id` is dead, and one of its files is the foreign waist by name

`src/ward.id` reqs exactly two modules: `std.jit` and `std.io`. Nothing in the
repository reqs `src.wasm.op`, `.simd`, `.stack`, `.table`, `.value`, `.memory`,
`.jit`, `.jit_arm64` or `.aot` except `test/main.id`, which does not parse.

```
$ grep -rn 'req "' ext/ward --include=*.id | grep -v src/ward.id
```

`src/wasm/jit.id:2` reads *"Strategy: emit C and invoke clang -O2 to produce a
shared library."* That is the C-as-intermediary waist Pass 103 §0b names as
banned, sitting in dead code. `src/wasm/aot.id` is 10 lines and its `compile()`
returns `""`.

`src/wasm/jit_arm64.id` (701 lines) was dead as a *module* but served as donor
provenance for `lib/std/compiler/arm64.id` and
`lib/std/compiler/arm64check.id`. That dated relationship grants no semantic
authority and no permanent retention. Every tracked project-owned `.id` file
...
`scripts/capability_scan.id` (Pass 105 U1, `zig build capability-scan`) counts
ambient `fs`/`net`/`clock`/`rand`/`proc`/`fault` reach across
`git ls-files 'lib/std/*.id'` — a pathspec that recurses. Measured at baseline:

```
capability_scan: PASS — 278 sites (budget 278, slack 0), 53 modules (budget 53, slack 0)
  modules   258 tracked lib/std/**/*.id
```

**Slack zero on both ratchets.** The engine is a program: `src/ward.id` alone
...
`scripts/stdlib_embed_gate.id` says the same thing from the other side: it
does `find lib/std -name '*.id'` and asserts every hit is `req`-able from a
...
`~/x/duo-mcp`) that became parts of the toolchain. `tools/lsp/src/server.id`
...
`std.wasm.opcodes`, `std.wasm.wasi`. `lib/std.id` registers them; the ambient
`std.wasm.SECTION.CODE` path is checked by
`scripts/stdlib_correctness_gate.id`. Nothing here changes except one filename
(§4).
...
two benchmark sweeps (`bench/run.id` header). The target:
...
2. `src/main.zig` ~line 846 (next to the `wasm-tables` handler): add a `wasm`
   handler that resolves `tools/wasm/src/engine.id`, compiles-or-caches it, and
...
Until (1)–(3) land the engine is reachable as `duo run tools/wasm/src/engine.id`
and as the built binary, and the env contract remains the interface. **Do not
delete the env contract when the subcommand lands** — `test/conform.id` and
...
| `ext/ward/src/ward.id` | `tools/wasm/src/engine.id` | the runtime |
...
| `-- derived(ward.opcodes.*)` regions | `-- derived(wasm.opcodes.*)` | in `engine.id` + `tools/opcodes.id` |
...
|---|---|---|
| `lib/std/wasm/ward_mvp_opcodes.id` | `lib/std/wasm/mvp.id` (`std.wasm.mvp`) | **Four-file atomic rename.** The file is `generated` in `docs/spec/corpus.md`, which exempts it from the audit100 deny table; rename it without moving that row and its 73 lines of `OP_i32_add = 106` land in the `canonical` partition and pay `snake` + `upper` in full on a table whose `upper` row is already 32 over. Touches `src/wasm_semantic_gen.zig:239`, `src/main.zig:{398,856,858}`, `src/semantic_compression_report.zig:39`, `src/ward_readiness.zig:70`, `docs/spec/corpus.md`, `lib/std.id:213`, `examples/pass9/ward_mvp_opcodes_smoke.id`. LAW-ONE violation twice over today. |
...
| `scripts/pass11_ward_no_boxing_smoke.id` | `scripts/pass11_wasm_no_boxing_smoke.id` | `scripts/` — not in this ownership. |
| `duon wasm` subcommand | §3 (1)–(3) | `src/main.zig`. |

### Deliberately NOT renamed

- `wart`, `wart_*.wasm` fixtures, `bench/wart.id` — wart is a different
  project and keeps its name.
- `forward` / `backward` / `toward` / `transformer_forward` in `src/nn/init.id`
...
- `bench/run.id:87` special-cases exit 132 (`128+SIGILL`) as wart's expected
  failure. wart does not SIGILL — measured 2026-08-08, it builds and JITs
  correctly on this machine. **Dead arm, removed.**
- `bench/run.id:103` resolves wart through `$WART` / `$HOME/x/wart/...` — an
...
**`examples/` — `showcase.id` is deleted.** It reqs `src.lib`, `std.ml.tensor`,
`std.ml.nn` and `std.ml.device`; `src/lib.id` does not exist and has not for
...
**`test/main.id` is deleted.** `build.zig:221` already records that it "required
four modules that had been deleted and did not parse", which is why
`test/conform.id` was written. Keeping a second, non-parsing test file next to
the real one is how a suite gets skipped.

**`src/nn*.id` moves to `tools/wasm/…` for now and should not stay.** 370 lines
...
requires them except the deleted `test/main.id`. **Proposal: they belong in
...
**`src/wasm/aot.id` and `src/wasm/jit.id` should be deleted.** Both are dead
(§1.2). `jit.id` is a C-emitting JIT — the foreign waist Pass 103 §0b bans by
name — and `aot.id` is a 10-line stub returning `""`. They are cited by
...
**`src/wasm/jit_arm64.id` stays** — dead as a module, live as the source of
truth for `lib/std/compiler/arm64.id` and `arm64check.id`, and a named donor
for SH-11.
...
| `audit100` `nonduo` | 537 / 537, **slack 0** | **None from a move** — every file stays tracked and stays non-`.id`. A single *added* non-`.id` file fails it. Deleting `my-ward-app/main.wat` takes it to 536. |
...
- `audit100`: `upper` 32 over (traced to `lib/std/crypto/sha.id`), `trailret`
...
3. `git mv src/ward.id src/engine.id`; rename the step to `wasm-test` and the
...
5. `bench/run.id`: dead SIGILL arm out, wart provenance in.
6. Prose: README/CLAUDE/HANDOFF rewritten; delete `showcase.id` and
   `test/main.id`.
...
| 3 · `src/ward.id` → `src/engine.id`, step → `wasm-test`, artifact → `wasm-conform` | ✅ | |
| 4 · `WARD_*` → `DUO_WASM_*`, producer + consumers together | ✅ | see the controls below |
| 5 · `bench/run.id` SIGILL arm out, wart provenance in | ✅ | prints `wart at ad10076+3 uncommitted — NOT REPRODUCIBLE` |
| 6 · README rewritten, `showcase.id` deleted, `my-ward-app/` deleted | ✅ | |
| 6b · `test/main.id` deleted | ❌ **BLOCKED** | it carries another session's uncommitted edits; deleting it would destroy work this session does not own. Left in place. |
...
tail, and `build.id`'s `deps = {}` was deleted rather than left in the shape
the `M = {}` row matches.
...
`scripts/runtime_bench.id` is untracked, belongs to a concurrent session, and
hard-codes seven `ext/ward/...` paths (`ext/ward/bench/*.wasm`,
`ext/ward/src/*.id`, `ext/ward/src/wasm/*.id`). **Those paths no longer
...
comments: `lib/std/compiler/arm64.id`, `lib/std/compiler/arm64check.id`,
`lib/std/io.id`, `scripts/audit100.id`, `scripts/capability_scan.id`,
