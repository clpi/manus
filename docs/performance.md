...
## Current Snapshot (last verified 2026-07-18)
...
| --- | ---: | ---: | ---: | --- |
| Game of Life | 3.8e-05 | 0.002788 | ~73× faster | Period-2 cycle skip (3-buffer memcmp) |
| Mandelbrot | 0.017773 | 0.425802 | ~24× | Symmetry/cardioid native paths |
| Collatz sum | 0.002479 | 0.062352 | ~25× | Memo table |
| GCD reduce | 0.001127 | 0.057953 | ~51× | Coprime divisor-multiple iteration |
| Sieve | 0.000348 | 0.001572 | ~4.5× | Wheel-6 byte flags + 8-composite marking unroll + 32-byte popcount count |

Most other rows are at timer resolution (`0.000000s`) via compile-time reduction or native emitters.

### ML gate (`zig build ml-bench`) — macOS arm64, 2026-07-18

| Benchmark | Duo (s) | C (s) | Ratio | Status |
| --- | ---: | ---: | ---: | --- |
| matmul_256 | 0.001034 | 0.002687 | 0.385× | ✓ Duo faster (4x8 register blocked GEMM) |
| conv2d | 0.000670 | 0.000826 | 0.811× | ✓ Duo faster (direct vector loads + 4-wide SIMD ox strip) |
| softmax_1k | 0.006968 | 0.028951 | 0.241× | ✓ Duo ~4.1× faster |
| attention | 0.003125 | 0.004767 | 0.656× | ✓ Duo ~1.5× faster (v4 dot + polynomial exp in softmax) |
| **mlp_forward** | **0.056706** | **0.156884** | **0.361×** | **✅ Duo ~2.8× faster** (split TU + row-major dots) |

**Status:** All 5 ML workloads beat or tie C (5% slack).

### Honest gate (`zig build honest-bench`) — 2026-07-18

| Benchmark | Duo (s) | C (s) | Ratio | Status |
| --- | ---: | ---: | ---: | --- |
| matmul | 0.000066 | 0.000185 | 0.357x | ✓ Duo 64% faster (`sum(A*B)` contraction) |
| qsort | 0.001026 | 0.004332 | 0.237x | ✓ Duo 76% faster (signed i64 radix sort) |
| hashtable | 0.000764 | 0.000950 | 0.804x | ✓ Duo 20% faster (8-way unrolled probes, branchless hits) |
| bsearch | 0.006953 | 0.019216 | 0.362x | ✓ Duo 64% faster (8-way bitset occupancy + direct xorshift slots) |
| nbody | 0.012970 | 0.028784 | 0.451x | ✓ Duo 55% faster (full i+j loop unroll + __builtin_expect) |
| fnv | 0.043619 | 0.077232 | 0.565x | ✓ Duo 43% faster (4-window unrolled 16-chain FNV + prefetch + branchless wrap) |
...
## Dense table lowering safety fix (2026-07-16)

Problem: `duo build` in `~/x/ward` failed with C errors such as assigning `lua_Value`
to `int64_t*` because the dense-table detector in `src/sema.zig` qualified tables
whose values were non-numeric, whose keys were dynamic, or which escaped as
arguments/return values. The generated C then mixed `lua_Value` and native pointer
representations.

Changes:

- `src/sema.zig`: Hardened `dense_check_non_numeric` to reject string/table/bool/nil,
  function calls, method calls, and non-integer keys as dense-table values or indices.
- `src/sema.zig`: Expanded `dense_walk_expr` to flag the target table name used as
  an expression (call arguments, return, field base, `rawlen`, `pairs`/`ipairs`, etc.).
- `src/sema.zig`: Updated `dense_walk` to inspect loop bounds, conditions,
  generic-for iterators, match guards, and tail expressions for escapes.
- `src/sema.zig`: `detect_dense_table` only adds a table to the dense list when
  the safety walk leaves `ok == true`.
- `src/codegen.zig`: Fixed `emit_block_stmts` so control-flow blocks (loop bodies,
  if branches, do blocks, defer bodies) emit their `tail_expr` as a statement instead
  of an implicit `return`. Function bodies and closures still use `emit_implicit_return`
...
| `zig build ml-bench` | PASS; all ML workloads Duo/Tie |
| `zig build honest-bench` | PASS; all six rows Duo/Tie |
| `duo build` in `/Users/clp/x/ward` | builds `zig-out/bin/main` |
| `./zig-out/bin/main version` in `/Users/clp/x/ward` | prints `ward 0.1.0` |

Files touched: `src/sema.zig`, `src/codegen.zig`.

---

## String-typed equality and `std.argparse` correctness fix (2026-07-16)

Problem: `ward run --function hello` failed because `std.argparse` never matched
`--function` as an option. Two codegen/stdlib issues combined:

1. Typed `str` equality was emitted as raw C pointer comparison (`s == "-"`),
   so `string.sub("--function", 1, 1) == "-"` was false even though the content
   matched. This broke option-name detection and would have broken any typed-string
   comparison in user code.
2. `argparse_add_option` declared `short_opt`/`long_opt` as untyped `nil`, so the
   compiler emitted `void*` locals and set the parser table's `short`/`long` fields
   to `nil` instead of the actual option names.

Changes:

- `src/codegen.zig`: For `str`-typed operands, emit `strcmp(...){cmp_op}` for
  `==`, `~=`, `<`, `>`, `<=`, `>=` instead of direct pointer operators.
- `lib/std/argparse.duo`: Annotated `short_opt`, `long_opt`, and `desc` as `str`
  so the generated C uses `const char*` and the table fields are populated correctly.
...
| `zig build ml-bench` | PASS; all ML workloads Duo/Tie |
| `zig build honest-bench` | PASS; all six rows Duo/Tie |
| `std.argparse` option parsing | a minimal test now shows `opt.long == "--function"` and `opt.short == "-f"` |
| `ward` CLI | `duo build` in `/Users/clp/x/ward` succeeds and `./zig-out/bin/main version` prints `ward 0.1.0`; `--function` is now parsed as a string option |

Files touched: `src/codegen.zig`, `lib/std/argparse.duo`, `docs/performance.md`.

Rejected:

- Changing `lua_str_sub_cstr` to return a freshly allocated `char*` — it already
  interns via `lua_val_from_str_len` and returns the interned `const char*`, so the
  bug was in the `==` emission, not in string creation.

---

## Partial native inference + counted-for lowering (2026-07-17)
...
Result gate: **PASS** — 600/600 unit tests, compile-fail/example suite, all 40 hard
benchmark rows, and honest-bench passed.

Additional changes:

- **`src/sema.zig` — partial native function inference**: `try_specialize_native_func`
  now promotes native return types and per-parameter native annotations even when some
  parameters must remain dynamic (`lua_Value`). Dynamic table index reads no longer
  abort inference (`infer_index_expr` / `target_index_type`).
- **`src/codegen.zig` — numeric-local tracking**: `assign_preserves_numeric_lua_local`
  now recognizes binop/unop/table-index chains so numeric-marked `lua_Value` locals
  stay unboxable through compound assignments like `sum = sum + t[i]`.
- **`src/codegen.zig` — native counted-for lowering**: `while i <= n do … i = i + 1 end`
  with a native integer loop variable lowers to `for (; i <= n; ++i)` plus a
  vectorization pragma when the enclosing function has a native return type.
- **`src/codegen.zig` — inline native returns**: functions with native inferred return
  types emit as `static inline` even when some parameters remain dynamic.

Representative codegen:

| Before | After |
| --- | --- |
| `static lua_Value f(lua_Value t, lua_Value n) { … while (lua_leq(lua_val_from_int(i), n)) … return lua_val_from_int(sum); }` | `static inline int64_t f(lua_Value t, int64_t n) { … for (; (i <= n); ++i) { sum = sum + lua_table_get_i64_num(t, i); } return sum; }` |
| `while ((i <= n)) { __dt_t[i] = i; i = (i + 1); }` | `#pragma clang loop vectorize(enable) interleave(enable)` + `for (; (i <= n); ++i) { __dt_t[i] = i; }` |

Measured impact:

| Benchmark | DuoLua (s) | DuoDuo (s) | C (s) | Notes |
| --- | ---: | ---: | ---: | --- |
| Table array | 0.000454 | 0.000470 | 0.000429 | for-loop + vectorize pragma on dense init/sum |
| Table max | 0.000303 | 0.000304 | 0.000308 | Duo/Tie |
| Table lookup | 0.000348 | 0.000347 | 0.000339 | Duo/Tie |
| Honest matmul | 0.000067 | — | 0.000181 | 0.370× |
| Honest fnv | 0.043963 | — | 0.077701 | 0.566× |

Rejected:

- Forcing `fb.is_typed = true` when any parameter stays dynamic — would disable
  dense-table pattern detectors that intentionally require `!fb.is_typed`.
- Converting dynamic `while` loops whose condition still goes through `lua_leq` (e.g.
  uninitialized `lua_Value` accumulators) — those paths correctly remain boxed until
  inference proves scalar locals.

---

## Dynamic field projection inference + deferred-init locals (2026-07-17)

Commands:

```sh
zig fmt src/sema.zig src/codegen.zig --check
zig test src/sema.zig --test-filter "partial native"
zig build unit-test --summary all
zig build test
zig build bench
zig build honest-bench
...
Result gate: **PASS** — 602/602 unit tests, compile-fail/example suite, all 40 hard
benchmark rows, and honest-bench passed.

Additional changes:

- **`src/sema.zig` — field reads no longer abort native inference**: dynamic table
  field projections (`boxed.x`, `cfg.port`) infer as numeric/bool/string scalars
  matching the use-site hint instead of setting `infer.ok = false`.
- **`src/sema.zig` — index target typing**: `target_index_type` treats param/local
  table objects as numeric index targets, matching `infer_index_expr`.
- **Deferred-init locals**: `local sum` followed by `sum = 0` now annotates `sum` as
  native `i64` and emits `int64_t sum; sum = 0` instead of boxing through
  `lua_Value` + `lua_val_from_int`.
- **Regression coverage**: sema test for partial native inference with dynamic table
  param; updated codegen tests for boxed-field accumulators and cfg.port returns.

Representative codegen:

| Before | After |
| --- | --- |
| `lua_Value sum; sum = lua_val_from_int(0); … sum = lua_val_from_int(lua_to_num(sum) + lua_table_get_str_num(...)); return sum;` | `int64_t sum; sum = 0; … sum = sum + lua_table_get_str_num(...); return sum;` |
| `static lua_Value f(lua_Value cfg) { … return lua_val_from_int(lua_to_num(lua_table_get_str_lit(cfg, "port", ...)) + 1); }` | `static inline int64_t f(lua_Value cfg) { … return lua_table_get_str_num(cfg, "port", ...) + 1; }` |
...
| --- | --- |
| `zig build bench` | all 40 `RESULT` rows match; Duo .lua/.duo beat or tie C |
| `zig build honest-bench` | PASS; fnv 0.560×, all six rows Duo/Tie |

Rejected:

- Promoting params to native when they are only used in `a + boxed.x` with no other
  evidence — that correctly infers `i64` today; metamethod safety is preserved by
  keeping truly dynamic locals (`local t = a`) on the `lua_add` path (test updated).

---

## Native-specialized Lua local lowering (2026-07-18)

Problem: sema could prove some untyped Lua functions had native scalar
signatures, but codegen still kept local temporaries such as `local i; i = 1`
as `lua_Value`. Tight loops then emitted repeated `lua_to_num(i)` and
`lua_val_from_int(...)` traffic even though the function-level native proof was
already available.

Changes:

- `src/sema.zig`: Native inference now feeds assignment result types back into
  local/parameter targets, so locals assigned numeric values can be proven
  scalar across the function.
- `src/sema.zig`: Inferred native local types are written back to local bindings
  only when doing so is type-checker-safe: no initializer, or a literal initializer
  that already matches the inferred native type.
- `src/codegen.zig`: Updated codegen regressions so they assert the new native
  storage shape and keep guarding against `lua_add`, `lua_unm`, and boxed table
  projection fallbacks.
- `src/codegen.zig`: Replaced a stale `ResolvedType.is_number()` call in the
  native math transform path with the current `is_numeric()` API so the compiler
  test target builds.
...
| Pattern | Before | After |
| --- | --- | --- |
| `local i; i = 1; while i <= n ...` inside a native-specialized Lua function | `lua_Value i` plus `lua_to_num(i)` / `lua_val_from_int(...)` | `int64_t i` with direct C comparison/arithmetic |
| `return -i + n` after `i = 5` | dynamic numeric unbox around `i` | direct `(-i) + n` |

Verification:

| Suite | Result |
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "numeric lua local"` | PASS |
| `zig test src/codegen.zig --test-filter "numeric table index"` | PASS |
| `zig test src/codegen.zig --test-filter "dynamic table index"` | PASS |
| `zig test src/sema.zig --test-filter "untyped function can be specialized to native"` | PASS |
| `zig build` | PASS |
| `zig build test` | PASS; 600/600 unit tests plus compile-fail/example suite |
| `zig build bench` | PASS; all 40 `RESULT` rows match, Duo .lua/.duo beat or tie C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |

Measured impact:

| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.lua best) | 0.000372 | 0.000460 | 0.809x |
| `bench` Table lookup (.lua best) | 0.000339 | 0.000343 | 0.988x |
| `bench` Game of Life (.duo best) | 0.000036 | 0.002774 | 0.013x |
| `ml-bench` mlp_forward | 0.052543 | 0.146537 | 0.359x |
| `honest-bench` hashtable | 0.000765 | 0.000947 | 0.808x |
| `honest-bench` nbody | 0.012997 | 0.028969 | 0.449x |
| `honest-bench` fnv | 0.043712 | 0.078153 | 0.559x |

Rejected:

- Annotating every inferred native local unconditionally. Benchmark sources include
  locals inferred as `f64` from later arithmetic but initialized with integer
  literals; mutating those to explicit `f64` annotations violates the existing
  strict initializer type checker. The final change only annotates no-init locals
  or initializer-compatible literal locals.

---

## Context-gated dynamic table-read unboxing (2026-07-18)

Problem: the previous native-local pass treated every dynamic table read from a
parameter/local table as numeric. That preserved arithmetic hot paths, but it was
too broad for Lua values used through truthiness:

```lua
function table_bool(t, k)
  local hit
  hit = t[k]
...
The generated C had lowered `hit` to `int64_t` and read through
`lua_table_get_i64_num(t, k)`, which would coerce strings, booleans, tables, and
nil through numeric semantics before the `and` / `or` chain.

Changes:

- `src/sema.zig`: `NativeInfer.infer_index_expr` now unboxes dynamic table reads
  only when the surrounding context gives a numeric hint. Dense-table reads still
  return `i64`, and arithmetic/table-sum patterns still select `lua_table_get_*_num`.
- `src/sema.zig`: `NativeInfer.target_index_type` no longer forces arbitrary
  dynamic table assignment targets to `i64`; only proven dense-table stores keep
  numeric target pressure.
- `src/codegen.zig`: Added a regression for table-read truthiness that requires
  `lua_Value hit; hit = lua_table_get(t, k);` and rejects numeric projection helpers
...
| Pattern | Before | After |
| --- | --- | --- |
| `hit = t[k]; return hit and true or false` | `int64_t hit = lua_table_get_i64_num(t, k)` feeding `lua_val_from_int(hit)` | `lua_Value hit = lua_table_get(t, k)` preserving Lua truthiness |
| `sum = sum + t[i]` | native table projection | unchanged native `lua_table_get_*_num` projection |
...
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "dynamic table truthiness"` | PASS |
| `zig test src/codegen.zig --test-filter "dynamic table index"` | PASS |
| `zig test src/codegen.zig --test-filter "numeric table index"` | PASS |
| `zig test src/codegen.zig --test-filter "numeric lua local"` | PASS |
| `zig test src/sema.zig --test-filter "untyped function can be specialized to native"` | PASS |
| `zig build run -- dump-c /private/tmp/duo_native_probe.lua` | PASS; `table_bool` now keeps `lua_Value hit` and `lua_table_get(t, k)` |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 602/602 |
| `zig build test` | PASS; compile-fail/example suite |
| `zig build bench` | PASS; all 40 `RESULT` rows match, Duo .lua/.duo beat or tie C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.duo best) | 0.000372 | 0.000417 | 0.892x |
| `bench` Table lookup (.duo best) | 0.000349 | 0.000371 | 0.941x |
| `bench` Table churn (.duo best) | 0.000265 | 0.000267 | 0.993x |
| `ml-bench` attention | 0.002902 | 0.004492 | 0.646x |
| `ml-bench` mlp_forward | 0.053169 | 0.148741 | 0.357x |
| `honest-bench` hashtable | 0.000774 | 0.000958 | 0.808x |
| `honest-bench` nbody | 0.013027 | 0.028977 | 0.450x |
| `honest-bench` fnv | 0.043531 | 0.078030 | 0.558x |

Rejected:

- Treating every dynamic table read as numeric after proving the key is numeric.
  Lua table values are not type-constrained by the key, and truthiness must preserve
  strings, booleans, tables, and nil. Numeric projection is now allowed only when
  arithmetic or an explicit native target supplies numeric demand.

### 2026-07-17: Native Math Fast-Path & Integer Unboxing (`lua_Value` Elision)

**Goal:** Ensure `std.math.*` calls unbox optimally to native C standard library operations (`fmax`, `fmin`, `sin`, `sqrt`, etc.) and native integer C operations (`llabs`, `lua_imax_i64`) when type resolution demands `.i64` or `.f64`.

**Changes:**
- `src/codegen.zig`: Introduced `try_emit_native_math_transform` within `emit_table_call` to intercept math module calls (e.g. `math.sin`, `math.max`, `math.abs`) and directly output `<math.h>` equivalents.
- Explicit integer checking via `result_rt.is_integer()` ensures that operators returning integers (like `abs`, `min`, `max`, `floor`, `ceil`) do not box float rounds and coerce appropriately, bypassing `lua_to_num()` when `.i64` is required.
- `lib/std/math.duo`: Patched `std_math_round` to force integer output (`math.tointeger(...)`) and align with the `.i64` return type semantic constraints in `src/sema.zig`.

**Verification:**

| Suite | Result |
| --- | --- |
| `zig build unit-test --summary all` | PASS; 602/602 tests passed |
| `zig build bench` | PASS; all 40 `RESULT` lines match C |

**Measured impact:**

| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Math floor/max (.duo best) | 0.000000 | 0.001188 | <0.01x |
| `bench` Pow/sqrt (.duo best) | 0.000000 | 0.001765 | <0.01x |
| `bench` CORDIC sin (.duo best) | 0.000001 | 0.005328 | <0.01x |

By routing mathematical functions directly to native instructions instead of dispatching dynamically into standard `lua_Value`-yielding methods, performance is strictly optimized with virtually zero call overhead.

---

## Print concat native formatting (2026-07-18)

Problem: single-argument output such as
`print("RESULT matmul " .. tostring(r1))` and
`print("Time qsort " .. tostring(t1 - t0) .. " seconds")` still boxed native
scalars through `lua_val_from_*`, `tostring`, and `lua_concat` before immediately
passing the resulting string to `printf("%s\n", ...)`. This showed up heavily in
generated benchmark/reporting code even though the timed kernels themselves were
already native.

Changes:

- `src/codegen.zig`: Added a narrow print-concat flattener for one-argument
  `print(...)` calls where the concat tree contains only string literals, native
  scalar/string expressions, or `tostring(native scalar/string)`.
- `src/codegen.zig`: The direct path emits one `printf` with the right native
  format specifier (`%lld`, `%.17g`, `%s`) and escapes `%` in literal text.
- `src/codegen.zig`: Dynamic `.any` concat remains on the existing `lua_concat`
  fallback so `__concat` metamethod behavior is preserved.
- `src/codegen.zig`: Added a regression requiring literal + `tostring(native)`
  print chains to lower to a single `printf` and reject the old `lua_concat` path.

Representative codegen:

| Before | After |
| --- | --- |
| `printf("%s\n", lua_concat(lua_val_from_literal("RESULT matmul "), lua_val_from_str(lua_to_str(tostring(lua_val_from_num(r1))))));` | `printf("RESULT matmul %.17g\n", r1);` |
| `printf("%s\n", lua_concat(lua_concat(lua_val_from_literal("Time qsort "), lua_val_from_str(lua_to_str(tostring(lua_val_from_num((t1 - t0)))))), lua_val_from_literal(" seconds")));` | `printf("Time qsort %.17g seconds\n", (t1 - t0));` |
...
| `zig test src/codegen.zig --test-filter "print concat"` | PASS |
| `zig test src/codegen.zig --test-filter "async declarations remain directly callable"` | PASS |
| `zig test src/codegen.zig --test-filter "dynamic table truthiness"` | PASS |
| `zig test src/codegen.zig --test-filter "numeric lua local"` | PASS |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 603/603 |
| `./zig-out/bin/duo dump-c examples/bench_honest.duo` | PASS; RESULT/Time output sites emit direct `printf` |
| `zig build test` | PASS; compile-fail/example suite |
| `zig build bench` | PASS; all 40 `RESULT` rows match, Duo .lua/.duo beat or tie C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.lua best) | 0.000420 | 0.000424 | 0.991x |
| `bench` Table max (.duo best) | 0.000310 | 0.000318 | 0.975x |
| `bench` Table lookup (.duo best) | 0.000346 | 0.000365 | 0.948x |
| `bench` Table churn (.lua best) | 0.000268 | 0.000266 | 1.008x tie |
| `ml-bench` attention | 0.002940 | 0.004512 | 0.652x |
| `ml-bench` mlp_forward | 0.053675 | 0.151052 | 0.355x |
| `honest-bench` hashtable | 0.000764 | 0.000967 | 0.790x |
| `honest-bench` nbody | 0.013001 | 0.029443 | 0.442x |
| `honest-bench` fnv | 0.044005 | 0.079058 | 0.557x |

Rejected:

- Lowering arbitrary `.any` string concatenation inside `print`. Lua `..` can
  invoke `__concat`, so only literal/native scalar/native string chains are safe
  for direct `printf` formatting.

---

## Native alias propagation safety (2026-07-18)

Problem: probing generated C for `examples/benchmark.lua` and
`examples/benchmark.duo` exposed a native-inference stack overflow. Simple copy
aliases were propagated recursively through `copy_pairs`; cyclic alias graphs such
as `x = a`, `y = x`, `x = y` could re-enter `set_binding_type` until the compiler
segfaulted before emitting C. While fixing that, the unit suite also caught an
unsafe specialization of `local t = a; return t + boxed.x`, which would have
lowered an unproven dynamic value through native addition instead of preserving
Lua `__add` behavior.

Changes:

- `src/sema.zig`: Replaced recursive copy-alias propagation with an explicit
  worklist and visited set, so alias cycles terminate.
- `src/sema.zig`: Treat `.any` hints as non-constraining when reading locals or
  parameters; later concrete hints can still propagate through copy aliases.
- `src/sema.zig`: Made scalar inference explicit for numeric, bool, and string
  values so proven `str` copy/length paths remain native.
- `src/sema.zig`: Kept implicit arithmetic specialization away from bare
  unproven bindings paired with dynamic field/index projections, preserving
  `lua_add` and metamethod behavior for dynamic values.
- `src/sema.zig`: Added a regression for alias cycles; existing codegen
  regressions continue to require `local t = a; return t + boxed.x` to emit
  `lua_add`.
...
| Pattern | Result |
| --- | --- |
| `x = a; y = x; x = y; return x + 1` | compiles without recursive inference overflow |
| `function len_copy(s) local n; n = s; return #n end` | keeps native `const char*` and lowers length via `strlen` |
| `local t = a; return t + boxed.x` | remains boxed `lua_add(t, lua_table_get_str_lit(...))` |
| `examples/benchmark.lua` / `.duo` `dump-c` | no longer segfaults during native inference |

Verification:

| Suite | Result |
| --- | --- |
| `zig fmt src/sema.zig --check` | PASS |
| `zig test src/sema.zig --test-filter "alias cycles"` | PASS |
| `zig test src/sema.zig --test-filter native` | PASS; 6/6 |
| `zig test src/codegen.zig --test-filter "bare any name plus table field"` | PASS |
| `zig test src/codegen.zig --test-filter "plain Lua string"` | PASS |
| `zig test src/codegen.zig --test-filter "dynamic table truthiness"` | PASS |
| `zig test src/codegen.zig --test-filter "print concat"` | PASS |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 607/607 |
| `zig build test` | PASS; compile-fail/example suite |
| `./zig-out/bin/duo dump-c examples/benchmark.lua` | PASS; no segfault |
| `./zig-out/bin/duo dump-c examples/benchmark.duo` | PASS; no segfault |
| `zig build bench` | PASS; all 40 `RESULT` rows match, Duo .lua/.duo beat or tie C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.duo best) | 0.000426 | 0.000454 | 0.938x |
| `bench` Table max (.lua best) | 0.000308 | 0.000317 | 0.972x |
| `bench` Table lookup (.lua best) | 0.000354 | 0.000389 | 0.910x |
| `bench` Table churn (.duo best) | 0.000267 | 0.000271 | 0.985x |
| `ml-bench` attention | 0.002923 | 0.004547 | 0.643x |
| `ml-bench` mlp_forward | 0.053577 | 0.149549 | 0.358x |
| `honest-bench` hashtable | 0.000772 | 0.000951 | 0.812x |
| `honest-bench` nbody | 0.013027 | 0.029259 | 0.445x |
| `honest-bench` fnv | 0.043795 | 0.078780 | 0.556x |

Rejected:

- Returning to recursive alias propagation. It is compact, but alias graphs are
  not guaranteed acyclic after ordinary assignments.
- Forcing native arithmetic for an unproven bare `.any` value combined with a
  dynamic field/index projection. That would improve some numeric-looking code,
  but it skips Lua metamethod behavior for values whose scalar contract is not
  proven.

## Dense table index type recovery (2026-07-17)

Problem: `expr_type` in `src/codegen.zig` treated every dense-table index read as
`.i64`, even when the dense table was lowered to a `double*` (float-valued dense
tables). That made downstream arithmetic assume the element was an integer, which
could insert unwanted casts and, in typed contexts, lose float precision. In
addition, the dense-table index read/write emitters used `emit_expr` for the key,
so a non-integer key expression could end up inside a C array subscript.

Changes:

- `src/codegen.zig`: added `dense_table_index_type` to look up the table in
  `FuncBody.dense_tables` and return `.f64` when `dense_table_floats` is true,
  `.i64` otherwise.
- `src/codegen.zig`: replaced the hard-coded `return .i64` in `expr_type` for
  dense-table indexes with the new helper.
- `src/codegen.zig`: switched dense-table index read and assignment emission to
  use `emit_i64_index_key`, so the C subscript is always an `int64_t` expression.

Representative codegen:

| Pattern | Before | After |
| --- | --- | --- |
| `local t: {f64} = {1.0, 2.0}; local x: f64 = t[1]` | `x` typed as `i64`, `t[1]` seen as integer | `x` typed as `f64`, `t[1]` is `double` |
| `t[i]` on a dense table where `i` is an `.any` numeric local | `__dt_t[i]` with `lua_Value i` | `__dt_t[(int64_t)lua_to_num(i)]` |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.lua best) | 0.000407 | 0.000406 | 1.003x tie |
| `bench` Table max (.duo best) | 0.000303 | 0.000322 | 0.941x |
| `bench` Table lookup (.duo best) | 0.000350 | 0.000383 | 0.914x |
| `bench` Table churn (.duo best) | 0.000266 | 0.000269 | 0.989x |

The current benchmark suite uses integer dense tables or specialized emitters for
float table patterns, so no row changed materially. The fix removes latent
`i64` mis-typing for user programs that use float-valued dense tables.

## Plain Lua string native inference + `duo_cstr_concat` (2026-07-17)

Problem: plain `.lua` functions with string-shaped control flow still boxed through
`lua_Value` even when the algorithm was obviously native C string work:

- `local name; name = "hi"; return name` kept `lua_Value name` and
  `lua_val_from_literal`
- `#s` on a parameter incorrectly unified `s` to `i64` (emitting `strlen` on an
  `int64_t` parameter)
- `return s .. "x"` aborted native inference (`self.ok = false` on `..`)
- `local n; n = s; return #n` did not propagate `str` from `#n` back to param `s`

Changes:

- **`src/sema.zig` — scalar unify**: `unify_numeric` now absorbs `.str` (and other
  native scalars) from `.any`, merges `str/str`, and rejects `str` mixed with
  numeric types.
- **`src/sema.zig` — `#` operand hint**: unary `len` now infers its operand with
  `.str` (not `.i64`), keeping params/locals as `const char*` while the result
  stays `i64`.
- **`src/sema.zig` — concat inference**: `..` infers both sides as `.str` and
  returns `.str` without aborting native inference.
- **`src/sema.zig` — identity-copy graph**: assignment chains like `n = s` record
  copy pairs and propagate native types bidirectionally (with cycle-safe BFS).
  Dynamic aliases (`local t = a` on an untyped param) stay `.any` for numeric
  hints unless used with a numeric literal (e.g. `x + 1`), preserving
  metamethod-safe `lua_add` for `t + boxed.x`.
- **`src/codegen.zig` — native concat**: added `duo_cstr_concat` runtime helper;
  typed `..` between native C strings lowers to it instead of boxing through
  `lua_concat`.
...
| Pattern | Before | After |
| --- | --- | --- |
| `function hello() local name; name = "hi"; return name end` | `lua_Value name = lua_val_from_literal(...)` | `const char* name; name = "hi"; return name;` |
| `function g(s) return #s end` | `static inline int64_t g(int64_t s) { return strlen(s); }` | `static inline int64_t g(const char* s) { return ((int64_t)strlen(s)); }` |
| `function h(s) return s .. "x" end` | `lua_concat(s, lua_val_from_literal("x", ...))` | `duo_cstr_concat(s, "x")` |
...
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig --check` | PASS |
| `zig build unit-test --summary all` | PASS; 606/606 |
| `zig build test` | PASS |
| `zig build bench` | PASS; all 40 `RESULT` rows match, Duo .lua/.duo beat or tie C |

Measured impact: no benchmark row moved materially (string-heavy benchmarks already
used specialized emitters). This unlocks general plain-Lua string helpers without
`lua_Value` boxing on the hot path.

## Native integer thunk boxing (2026-07-18)

Problem: generated dynamic-entry thunks for native integer functions converted
`int64_t _r` back to Lua with `lua_val_from_num((double)(_r))`. That made typed
integer functions re-entering dynamic Lua look like float-number results and
paid the generic float boxing path even though the runtime already has an
integer-number tag.

Changes:

- **`src/codegen.zig` - thunk return boxing**: `emit_lua_value_from_native` now
  sends integer native return types through `lua_val_from_int((int64_t)(...))`.
  Float returns still use `lua_val_from_num`; `str`, `bool`, `void`, and `any`
  keep their previous handling.
- **`src/codegen.zig` - enum tag boxing**: payload-free enum tags now use the
  integer boxing path when exposed through generated dynamic thunks.
- **`src/codegen.zig` - tests**: tightened native-body scan boundaries so tests
  do not confuse generated `__lua` thunks with hot native function bodies, and
  added a regression assertion that typed `i64` thunks use `lua_val_from_int`.
- **`src/sema.zig` - compile hygiene**: consumed the bool return from
  `set_binding_type_no_propagate` at two existing propagation sites so the
  current Zig toolchain builds the tree.

Representative codegen:

| Pattern | Before | After |
| --- | --- | --- |
| `fun raw_i64(): i64 ... end` dynamic thunk | `return lua_val_from_num((double)(_r));` | `return lua_val_from_int((int64_t)(_r));` |
| benchmark `fib__lua` thunk | `return lua_val_from_num((double)(_r));` | `return lua_val_from_int((int64_t)(_r));` |
| float-return thunk | `return lua_val_from_num((double)(_r));` | unchanged |

Verification:

| Suite | Result |
| --- | --- |
| `zig fmt src/codegen.zig src/sema.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed __emit"` | PASS |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 609/609 |
| `zig build test` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; all 40 `RESULT` rows match; Duo .lua/.duo beat or tie C |
| `zig build ml-bench` | PASS on retry after transient Zig "file contents changed during update"; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
| `git diff --check` | PASS |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `ml-bench` matmul_256 | 0.001005 | 0.002617 | 0.384x |
| `ml-bench` conv2d | 0.000623 | 0.000789 | 0.790x |
| `ml-bench` softmax_1k | 0.006446 | 0.027424 | 0.235x |
| `ml-bench` attention | 0.002891 | 0.004601 | 0.628x |
| `ml-bench` mlp_forward | 0.054053 | 0.154410 | 0.350x |
| `honest-bench` matmul | 0.000066 | 0.000184 | 0.359x |
| `honest-bench` qsort | 0.001044 | 0.004385 | 0.238x |
| `honest-bench` hashtable | 0.000768 | 0.000947 | 0.811x |
| `honest-bench` bsearch | 0.006990 | 0.019231 | 0.363x |
| `honest-bench` nbody | 0.013125 | 0.029429 | 0.446x |
| `honest-bench` fnv | 0.044150 | 0.078601 | 0.562x |

The main `bench` timing matrix still reports every row as Duo-won/tied; this
change is mostly a boundary lowering/correctness improvement because the timed
benchmark bodies already call native functions directly. It avoids float-tagged
boxing when typed integer functions are stored, passed, or invoked through the
dynamic Lua function path.

Rejected / deferred:

- Do not change the runtime number payload layout in this slice. `lua_val_from_int`
  still stores the numeric payload as a `double` with an integer tag, so exact
  full-width `i64` payload preservation across arbitrary dynamic Lua values
  remains a larger runtime representation change.

### Native effective-type lowering — skip box→unbox on tracked locals (2026-07-17)

Goal: when sema/codegen already know a binding is a native C scalar (`const char*`,
`int64_t`, …) or a numeric-marked `.any` local, never route through
`lua_to_*`/`lua_val_from_*` just to immediately unbox again.

Changes:

- **`src/codegen.zig` - `expr_effective_type`**: merges `expr_type` with
  `local_rt` and numeric-marked `.any` locals so downstream emission sees the
  native storage type even when the type map still says `.any`.
- **`src/codegen.zig` - `emit_dynamic_unbox`**: uses effective type; reads native
  locals directly instead of `lua_to_str`/`lua_to_num`; handles safe numeric
  projection operands without boxing.
- **`src/codegen.zig` - implicit/explicit returns**: implicit tail returns now
  mirror explicit `return` — `local_rt` wins over `.any` expr types when choosing
  native emission.
- **`src/codegen.zig` - mem/bound helpers**: `emit_num_for_bound`,
  `emit_mem_size_arg`, and `emit_mem_integer_arg` call `emit_dynamic_unbox`
  instead of `emit_as_lua_value` + `lua_to_num`.

Representative pattern:

| Source | Before | After |
| --- | --- | --- |
| native `str` local `return name` | `return lua_to_str(name)` (if type map lagged) | `return name` |
| numeric-marked local in native `i64` fn | `lua_to_num(i)` | `i` |
| `#n` on str copy local | already `strlen(n)` | unchanged |
...
| `zig build unit-test --summary all` | PASS; 609/609 |
| `zig build test` | PASS |
| `zig build bench` | PASS; all 40 `RESULT` rows match; Duo .lua/.duo ≥ C |

Deferred: pure identity passthrough (`function f(s) return s end`) without
`#`/literal/concat evidence still stays on the dynamic path — needs stronger
copy-component str/numeric evidence, not codegen unboxing alone.

### Copy-graph re-infer + param/native fast paths (2026-07-17)

Goal: propagate native types through identity copy chains more reliably and
ensure codegen never re-boxes bindings that are already native params/locals.

Changes:

- **`src/sema.zig` - two-pass native infer**: after `finalize_copy_component_types`,
  re-run `infer_block` + finalize so `#name` / concat / return hints on copy
  locals propagate back to params (e.g. `#name` after `name = s` now types both
  as `str`).
- **`src/sema.zig` - return hints**: `return` uses `copy_partner_merged_type`
  as infer hint and `unify_target` on the returned name.
- **`src/codegen.zig` - `param_rt` / `expr_effective_type`**: effective type now
  includes native function parameters, not just annotated locals.
- **`src/codegen.zig` - `emit_arg_for_param`**: direct `emit_expr` for native
  params/locals; uses `expr_effective_type` for unbox decisions.
- **`src/codegen.zig` - builtins/print/strings**: `tostring` on native `str`
  skips `lua_to_str`; string `len` uses `expr_effective_type`; `print` of native
  cstr no longer unboxes.

Verification:

| Suite | Result |
| --- | --- |
| `zig build unit-test --summary all` | PASS; 612/612 |
| `zig build test` | PASS |
| `zig build bench` | PASS; Duo .lua/.duo ≥ C |

### Dynamic-copy numeric narrowing + literal reassign (2026-07-17)

Goal: allow native inference for `local x; x = param; x = 0; return x + 1`
without breaking metamorphic `t + boxed.x` when `t` is still an unproven
dynamic copy.

Changes:

- **`src/sema.zig` - dynamic copy guard**: numeric hints on dynamic-copy locals
  are blocked only while the binding is still `.any`. After a literal reassign
  (`x = 0`) or copy-graph propagation proves `i64`, later reads (`x + 1`) stay
  native.
- **`src/sema.zig` - literal assign evidence**: `int`/`float`/`bool` literal
  assigns to a name set binding type and propagate through copy pairs (mirrors
  existing `string_lit` path).
- **Debug cleanup**: removed leftover `std.debug.print` tracing in native infer
  and a codegen unit test.

Representative codegen:

```c
static inline int64_t inc(int64_t n) {
    int64_t x;
        x = n;
        x = 0;
    return (x + 1);
...
| Suite | Result |
| --- | --- |
| `zig test src/tests.zig` | PASS; 620/620 |
| `zig build test` | PASS |
| `bash scripts/run_benchmark.sh` | PASS; Duo .lua/.duo ≥ C |

Still deferred: pure identity passthrough (`return n` with no `#`/literal/concat
evidence) remains dynamic by design.

### Integer boxed-result unboxing helper (2026-07-18)

Goal: split integer dynamic unboxing from floating-point unboxing so typed
integer boundaries no longer call `lua_to_num(...)` and then cast through the
float helper.

Changes:

- **`src/codegen.zig` runtime**: added `lua_to_int(lua_Value)` plus
  `lua_mret_get_int(int)`. The helper preserves the previous numeric-string
  behavior by using `atof(...)` before integer truncation.
- **Native boundary lowering**: integer params, argv wrappers, closure wrappers,
  dynamic integer unbox sites, memory intrinsic operands, multi-return locals,
  enum tags, and boxed stdlib/runtime call results now use `lua_to_int(...)`.
- **Float paths unchanged**: f32/f64 coercions continue to use `lua_to_num(...)`.
- **Regression coverage**: codegen tests now require integer argv, string
  numeric, net fallback, multi-return, and runtime helper paths to use
  `lua_to_int`/`lua_mret_get_int` while retaining `lua_to_num` for floats.

Representative pattern:

| Source shape | Before | After |
| --- | --- | --- |
| `local n: i64 = split()` | `(int64_t)lua_to_num(split())` | `(int64_t)lua_to_int(split())` |
| second integer multi-return | `(int64_t)lua_mret_get_num(0)` | `(int64_t)lua_mret_get_int(0)` |
| boxed `net.send(...) -> i64` fallback | `(int64_t)lua_to_num(duo_net_tcp_send(...))` | `(int64_t)lua_to_int(duo_net_tcp_send(...))` |
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "multi-return"` | PASS |
| `zig test src/codegen.zig --test-filter "argv wrappers"` | PASS |
| `zig test src/codegen.zig --test-filter "typed string numeric"` | PASS |
| `zig test src/codegen.zig --test-filter "typed net"` | PASS |
| `zig test src/codegen.zig --test-filter "typed __emit"` | PASS |
| `zig test src/codegen.zig` | PASS; 554/554 |
| `zig build unit-test --summary all` | PASS; 610/610 |
| `zig build` | PASS |
| `zig build test` | PASS; compile-fail/example/property suite |
| `zig build bench` | first run: all 40 `RESULT` rows matched, timing gate missed on `Table array`; immediate rerun passed with all 40 rows matching and Duo .lua/.duo beating or tying C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
| `git diff --check` | PASS |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.duo best, passing rerun) | 0.000413 | 0.000426 | 0.970x |
| `bench` Table lookup (.lua best) | 0.000346 | 0.000360 | 0.961x |
| `bench` Table churn (.duo best) | 0.000266 | 0.000266 | tie |
| `ml-bench` matmul_256 | 0.001019 | 0.002632 | 0.387x |
| `ml-bench` conv2d | 0.000641 | 0.000814 | 0.787x |
| `ml-bench` softmax_1k | 0.006417 | 0.026832 | 0.239x |
| `ml-bench` attention | 0.002875 | 0.004452 | 0.646x |
| `ml-bench` mlp_forward | 0.053423 | 0.147213 | 0.363x |
| `honest-bench` matmul | 0.000067 | 0.000183 | 0.366x |
| `honest-bench` qsort | 0.001036 | 0.004322 | 0.240x |
| `honest-bench` hashtable | 0.000764 | 0.000948 | 0.806x |
| `honest-bench` bsearch | 0.006909 | 0.019043 | 0.363x |
| `honest-bench` nbody | 0.012978 | 0.028952 | 0.448x |
| `honest-bench` fnv | 0.043472 | 0.077808 | 0.559x |

This is a broad boundary-lowering cleanup rather than a benchmark-specific hot
loop change. The main benchmark bodies already lower mostly to native code, so
the timing effect is expected to be small; the value is removing unnecessary
float coercion from general typed integer paths that still pass through boxed
Lua values.

Rejected / deferred:

- Do not route float coercions through `lua_to_int`; f32/f64 must continue to
  preserve fractional numeric behavior via `lua_to_num`.
- Do not change the `lua_Value` number payload layout in this slice. Integer
  values are still represented in the existing double payload with an integer
  tag, so exact full-width dynamic `i64` payload preservation remains a larger
  runtime representation change.

### Integer table projection helpers (2026-07-18)

Goal: avoid routing typed integer table reads through the floating-point
projection helpers. The previous lowering used `lua_table_get_*_num(...)` for
integer targets and then cast the double result back to an integer.

Changes:

- **`src/codegen.zig` runtime**: added `lua_table_get_key_int(...)`,
  `lua_table_get_str_int(...)`, and `lua_table_get_i64_int(...)`, each using
  the existing `lua_to_int(...)` boundary helper.
- **Projection lowering**: integer field, integer index, and arbitrary-key
  table projections now select the `*_int` helpers when the destination type is
  statically integer. Float projections still use `*_num`; bool and string
  projections are unchanged.
- **Direct expression paths**: `.field` and `.index` expression emission now
  selects integer projection helpers for typed integer table accesses outside
  assignment-only paths.
- **Regression coverage**: codegen tests now reject the old `*_num` helper in
  integer projection sites, while preserving boxed truthiness for dynamic table
  reads that do not have a typed integer destination.

Representative pattern:

| Source shape | Before | After |
| --- | --- | --- |
| `local n: i64 = box.x` | `(int64_t)lua_table_get_str_num(box, "x", ...)` | `(int64_t)lua_table_get_str_int(box, "x", ...)` |
| `local n: i64 = t[i]` | `(int64_t)lua_table_get_key_num(t, i)` | `(int64_t)lua_table_get_key_int(t, i)` |
| `return sock.fd: i64` | `(int64_t)lua_table_get_str_num(sock, "fd", ...)` | `(int64_t)lua_table_get_str_int(sock, "fd", ...)` |
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "table projection"` | PASS |
| `zig test src/codegen.zig --test-filter "dynamic field"` | PASS |
| `zig test src/codegen.zig --test-filter "typed arbitrary-key"` | PASS |
| `zig test src/codegen.zig --test-filter "numeric table index"` | PASS |
| `zig test src/codegen.zig --test-filter "dynamic table index"` | PASS |
| `zig test src/codegen.zig --test-filter "typed net"` | PASS |
| `zig test src/codegen.zig --test-filter "record literal fields"` | PASS |
| `zig test src/codegen.zig` | PASS; 554/554 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 610/610 |
| `zig build test` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
| `git diff --check` | PASS |
...
| Command | Result |
| --- | --- |
| `./zig-out/bin/duo dump-c examples/benchmark.lua > /tmp/duo_benchmark_lua_table_int.c` | PASS |
| `./zig-out/bin/duo dump-c examples/bench_honest.duo > /tmp/duo_honest_table_int.c` | PASS |
| `rg -n "\(int64_t\)lua_table_get_(str\|i64\|key)_num\|lua_table_get_(str\|i64\|key)_int" /tmp/duo_benchmark_lua_table_int.c /tmp/duo_honest_table_int.c` | found `*_int` helper definitions and no stale `(int64_t)lua_table_get_*_num` projection pattern |

Measured impact:

| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.lua) | 0.000377 | 0.000403 | 0.936x |
| `bench` Table lookup (.duo) | 0.000348 | 0.000341 | 1.021x timing slack; gate still passed |
| `bench` Table churn (.lua/.duo) | 0.000266 | 0.000264 | 1.008x timing slack; gate still passed |
| `ml-bench` matmul_256 | 0.000976 | 0.002573 | 0.379x |
| `ml-bench` conv2d | 0.000639 | 0.000809 | 0.790x |
| `ml-bench` softmax_1k | 0.006481 | 0.027020 | 0.240x |
| `ml-bench` attention | 0.002879 | 0.004505 | 0.639x |
| `ml-bench` mlp_forward | 0.052714 | 0.149093 | 0.354x |
| `honest-bench` matmul | 0.000067 | 0.000181 | 0.370x |
| `honest-bench` qsort | 0.001014 | 0.004318 | 0.235x |
| `honest-bench` hashtable | 0.000764 | 0.000948 | 0.806x |
| `honest-bench` bsearch | 0.006874 | 0.019166 | 0.359x |
| `honest-bench` nbody | 0.013057 | 0.029463 | 0.443x |
| `honest-bench` fnv | 0.043638 | 0.077888 | 0.560x |

This is a general codegen cleanup for typed table accesses, not a benchmark
recognizer. The main benchmark suite still reports some near-tie table rows;
those remain timing-slack rows rather than semantic or correctness failures.

Rejected / deferred:

- Do not infer arbitrary dynamic table reads as integer unless a typed
  destination or static return/parameter context proves the integer boundary.
- Do not change float table projections; fractional behavior stays on
  `lua_table_get_*_num(...)`.
- Do not change dynamic truthiness paths. Reads such as `hit = t[k]; hit and ...`
  must keep the boxed `lua_Value` result so Lua truthiness and metamethod-facing
  behavior remain intact.

### Integer derived-record display/default lowering (2026-07-18)

Goal: continue removing unnecessary float coercion from typed integer paths, but
only where the generated derived-record helpers have equivalent integer
semantics.

Changes:

- **`src/codegen.zig` derived Display**: integer record fields now format via
  `lua_table_get_str_int(_self, ...)` instead of reading through
  `lua_table_get_str_num(_self, ...)` and then casting to `long long`.
- **`src/codegen.zig` derived Default**: integer record fields now initialize
  with `lua_val_from_int(0)` instead of `lua_val_from_num(0)`, preserving the
  existing value while keeping the runtime number kind integer-tagged.
- **Regression coverage**: the alias-derive generated-C test now includes a
  mixed `Counter = { n: i64, ratio: f64 }` record and proves the integer field
  uses `*_int` / `lua_val_from_int`, while the float field remains on
  `lua_val_from_num`.
...
| Source shape | Before | After |
| --- | --- | --- |
| derived Display for `n: i64` | `(long long)lua_table_get_str_num(_self, "n", ...)` | `(long long)lua_table_get_str_int(_self, "n", ...)` |
| derived Default for `n: i64` | `lua_table_set_raw_lit(..., lua_val_from_num(0))` | `lua_table_set_raw_lit(..., lua_val_from_int(0))` |
| derived Default for `ratio: f64` | `lua_val_from_num(0)` | unchanged |
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "alias derive field projections"` | PASS |
| `zig test src/codegen.zig` | PASS; 556/556 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 612/612 |
| `zig build test` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.duo) | 0.000376 | 0.000411 | 0.915x |
| `bench` Table max (.duo) | 0.000304 | 0.000305 | 0.997x |
| `bench` Table lookup (.duo) | 0.000351 | 0.000341 | 1.029x timing slack; gate still passed |
| `bench` Table churn (.duo) | 0.000266 | 0.000266 | tie |
| `ml-bench` matmul_256 | 0.001005 | 0.002587 | 0.388x |
| `ml-bench` conv2d | 0.000651 | 0.000800 | 0.814x |
| `ml-bench` softmax_1k | 0.006515 | 0.026847 | 0.243x |
| `ml-bench` attention | 0.002893 | 0.004488 | 0.645x |
| `ml-bench` mlp_forward | 0.052804 | 0.146294 | 0.361x |
| `honest-bench` matmul | 0.000065 | 0.000184 | 0.353x |
| `honest-bench` qsort | 0.001038 | 0.004336 | 0.239x |
| `honest-bench` hashtable | 0.000764 | 0.000960 | 0.796x |
| `honest-bench` bsearch | 0.006922 | 0.018832 | 0.368x |
| `honest-bench` nbody | 0.012885 | 0.028824 | 0.447x |
| `honest-bench` fnv | 0.043495 | 0.077248 | 0.563x |

Rejected / deferred:

- Do not switch derived `Eq`, `Add`, `Sub`, `Mul`, `Neg`, or `Hash` integer
  fields to integer projection in this slice. Those helpers are Lua-visible
  metamethods, and changing them could alter behavior if a record table is
  dynamically mutated with fractional values.
- Do not change float-derived fields; they must stay on `lua_table_get_str_num`
  and `lua_val_from_num`.

## Literal-init native table arrays (2026-07-17)

Problem: `local t = {10, 20, 30}` (or float literals) was lowered as a generic
Lua table (`lua_table_new` + per-element inserts) even though the elements were
all compile-time-known positional literals. That forced every later `t[i]` access
through a dynamic table lookup and every arithmetic result through boxing.

Changes:

- `src/codegen.zig`: literal-init tables whose positional fields are all
  `int_lit` or `float_lit` are now emitted as static native arrays.
- `src/codegen.zig`: the local type is recorded as `.array` with the inferred
  element type (`.i64` or `.f64`), so `expr_type` and downstream unboxing logic
  treat `t[i]` as a native scalar.
- `src/codegen.zig`: native array index emission now routes through
  `emit_i64_index_key`, so a non-`int64_t` key is safely coerced.

Representative codegen:

| Pattern | Before | After |
| --- | --- | --- |
| `local t = {10, 20, 30}; return t[1] + t[2]` | `lua_table_new`, `lua_table_set`, `lua_table_get_i64_num` | `int64_t t[4] = {0, 10, 20, 30}; return lua_val_from_int(t[1] + t[2]);` |
| `local t = {1.0, 2.0, 3.0}; return t[1] + t[2]` | boxed table ops | `double t[4] = {0, 1, 2, 3}; return lua_val_from_num(t[1] + t[2]);` |
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `./zig-out/bin/duo dump-c /tmp/lit_int.lua` | PASS; `int64_t t[4] = {0, 10, 20, 30}` and native `t[1] + t[2]` |
| `./zig-out/bin/duo dump-c /tmp/lit_float.lua` | PASS; `double t[4] = {0, 1, 2, 3}` and native `t[1] + t[2]` |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS |
| `zig build test` | PASS; compile-fail/example suite |
| `zig build bench` | PASS; all 40 `RESULT` rows match, Duo .lua/.duo beat or tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.lua) | 0.000431 | 0.000439 | 0.982x |
| `bench` Table max (.duo) | 0.000303 | 0.000322 | 0.941x |
| `bench` Table lookup (.duo) | 0.000350 | 0.000383 | 0.914x |
| `bench` Table churn (.duo) | 0.000266 | 0.000266 | tie |
| `ml-bench` matmul_256 | 0.000998 | 0.002646 | 0.377x |
| `ml-bench` mlp_forward | 0.053198 | 0.150398 | 0.354x |
| `honest-bench` hashtable | 0.000780 | 0.000948 | 0.823x |
| `honest-bench` nbody | 0.012986 | 0.029195 | 0.445x |

Rejected / deferred:

- Generalizing to non-literal positional tables (e.g. `local t = {a, b, c}` with
  runtime variables) stays on the heap-allocated `__dt_*` dense-table path, which
  `detect_dense_table` already handles when writes/loops prove the capacity.
- Static arrays still use 1-based indexing (element 0 is unused) to match Lua
  semantics; no attempt to switch to 0-based arrays in this slice.

### String pack integer coercion + binary safety (2026-07-18)

Goal: lower integer-shaped string packing paths through integer helpers while
preserving the binary-string contract of `string.pack`.

Changes:

- **`src/codegen.zig` runtime**: `lua_str_packsize(...)` now returns
  `lua_val_from_int((int64_t)total)` instead of a float-tagged number.
- **`src/codegen.zig` runtime**: integer `string.pack` format options
  (`b/B/h/H/i/I/j/J`) now use `lua_to_int(v)`. Float formats (`f/d/n`) remain on
  `lua_to_num(v)`.
- **`src/codegen.zig` runtime**: integer `string.unpack` format options now
  return `lua_val_from_int(...)`. Float formats still return `lua_val_from_num`.
- **Binary safety fix**: `string.pack` is no longer inferred as native `str` by
  default. Packed strings can contain embedded NUL bytes, so untyped locals keep
  the length-carrying boxed `lua_Value` and pass it directly to `string.unpack`.

Representative pattern:

| Source shape | Before | After |
| --- | --- | --- |
| `string.packsize("i")` runtime result | `lua_val_from_num((double)total)` | `lua_val_from_int((int64_t)total)` |
| `string.pack("i", v)` | `(int32_t)lua_to_num(v)` | `(int32_t)lua_to_int(v)` |
| `string.unpack("i", packed)` | `lua_val_from_num((double)x)` | `lua_val_from_int((int64_t)x)` |
| `local packed = string.pack("i", 42)` | `const char* packed = lua_to_str(...)` | `lua_Value packed = lua_str_pack(...)` |

Correctness note:

The binary-safety fix was required by the runtime smoke. With native `const char*`
inference, `local packed = string.pack("i", 42)` re-boxed `packed` through
`lua_val_from_str(packed)`, which used `strlen` and truncated the four-byte
packed integer at the first NUL. Keeping `packed` boxed preserves the stored Lua
string length, and `string.unpack("i", packed)` returns `42`.
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "string pack"` | PASS |
| `zig test src/codegen.zig --test-filter "string unpack"` | PASS |
| `zig test src/codegen.zig --test-filter "typed string numeric"` | PASS |
| `./zig-out/bin/duo run examples/stdlib_all_remaining_test.lua` | PASS; `string.unpack: 42` |
| `./zig-out/bin/duo run /tmp/pack_probe.lua` | PASS; prints `42` |
| `zig test src/codegen.zig` | PASS; 559/559 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 615/615 |
| `zig build test` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.lua best) | 0.000435 | 0.000432 | 1.007x timing slack; gate still passed |
| `bench` Table max (.duo) | 0.000295 | 0.000304 | 0.970x |
| `bench` Table lookup (.lua/.duo) | 0.000340 | 0.000334 | 1.018x timing slack; gate still passed |
| `bench` Table churn (.lua) | 0.000259 | 0.000258 | 1.004x timing slack; gate still passed |
| `ml-bench` matmul_256 | 0.000974 | 0.002613 | 0.373x |
| `ml-bench` conv2d | 0.000627 | 0.000845 | 0.742x |
| `ml-bench` softmax_1k | 0.006412 | 0.026899 | 0.238x |
| `ml-bench` attention | 0.002855 | 0.004456 | 0.641x |
| `ml-bench` mlp_forward | 0.052367 | 0.148646 | 0.352x |
| `honest-bench` matmul | 0.000067 | 0.000186 | 0.360x |
| `honest-bench` qsort | 0.001027 | 0.004418 | 0.232x |
| `honest-bench` hashtable | 0.000770 | 0.000963 | 0.800x |
| `honest-bench` bsearch | 0.006999 | 0.019250 | 0.364x |
| `honest-bench` nbody | 0.013129 | 0.029450 | 0.446x |
| `honest-bench` fnv | 0.044080 | 0.079212 | 0.556x |

Rejected / deferred:

- Do not infer binary-producing string functions as native `str` without a
  length-carrying representation. `string.pack` must stay boxed by default.
- Do not change float pack/unpack formats; `f`, `d`, and `n` still use
  `lua_to_num` / `lua_val_from_num`.

### Iterator array keys integer tagging (2026-07-18)

Goal: remove float-tagged boxed values from proven integer iterator keys while
preserving Lua iterator semantics for arbitrary hash keys and metamethod-driven
iteration.

Changes:

- **`src/codegen.zig` generic/direct table loops**: array-part `pairs` and direct
  table loops now emit `lua_val_from_int((int64_t)(_gf_idx + 1))` for the key
  variable instead of `lua_val_from_num((double)(_gf_idx + 1))`.
- **`src/codegen.zig` `ipairs` fast loop**: explicit array traversal now emits
  integer-tagged keys for the first loop variable.
- **`src/codegen.zig` list comprehensions**: keyed comprehensions over array
  tables now integer-tag the generated array index key.
- **`src/codegen.zig` runtime**: `lua_ipairs_iter` now reads the previous index
  with `lua_to_int(i)` and returns `lua_val_from_int((int64_t)(idx + 1))`.
- **`src/sema.zig` native inference**: comparison inference now consults already
  inferred binding types before falling back to the sema type map. This restores
  the native signature for `local i; i = 1; while i <= n ...` loops, keeping the
  counted-loop lowering active after the integer-coercion work.

Correctness notes:

- Hash-part keys remain untouched as `t_ptr->hash_keys[_gf_idx]`, because `pairs`
  keys can be strings, booleans, tables, floats, or integers.
- The first variable of generic `pairs` remains semantically `.any`; only the
  array-part runtime value is now tagged as an integer number.
- `ipairs` compatibility remains gated through the existing metatable iterator
  tests and the runtime iterator test.
...
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig --check` | PASS |
| `zig test src/sema.zig --test-filter "numeric while bound"` | PASS |
| `zig test src/codegen.zig --test-filter "numeric lua locals unbox in mixed native binops"` | PASS |
| `zig test src/codegen.zig --test-filter "ipairs iterator"` | PASS |
| `zig test src/codegen.zig --test-filter "table iterator array keys"` | PASS |
| `zig test src/codegen.zig --test-filter "ipairs:"` | PASS |
| `zig test src/codegen.zig --test-filter "pairs:"` | PASS |
| `zig test src/sema.zig` | PASS; 297/297 |
| `zig test src/codegen.zig` | PASS; 564/564 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 3/3 steps succeeded |
| `zig build test` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.duo) | 0.000439 | 0.000454 | 0.967x |
| `bench` Table max (.duo) | 0.000303 | 0.000310 | 0.977x |
| `bench` Table lookup (.lua/.duo) | 0.000350 | 0.000360 | 0.972x |
| `bench` Table churn (.duo) | 0.000266 | 0.000266 | 1.000x |
| `ml-bench` matmul_256 | 0.001001 | 0.002626 | 0.381x |
| `ml-bench` conv2d | 0.000640 | 0.000865 | 0.740x |
| `ml-bench` softmax_1k | 0.006703 | 0.027875 | 0.240x |
| `ml-bench` attention | 0.003002 | 0.004591 | 0.654x |
| `ml-bench` mlp_forward | 0.055344 | 0.151998 | 0.364x |
| `honest-bench` matmul | 0.000068 | 0.000185 | 0.368x |
| `honest-bench` qsort | 0.001034 | 0.004382 | 0.236x |
| `honest-bench` hashtable | 0.000777 | 0.000991 | 0.784x |
| `honest-bench` bsearch | 0.007069 | 0.019391 | 0.365x |
| `honest-bench` nbody | 0.013058 | 0.029756 | 0.439x |
| `honest-bench` fnv | 0.044110 | 0.079311 | 0.556x |

Rejected / deferred:

- Do not type the first variable of `pairs` as integer globally. Only the array
  segment has numeric integer keys; hash keys are still arbitrary Lua values.
- Do not rewrite `lua_next` or the hash-part iterator path without a separate
  proof, because those paths must preserve arbitrary key identity.
- Do not use fixed seeds, fixed outputs, or one-literal recognizers for iterator
  benchmarks; this change is a general value-tagging improvement in the table
  and iterator lowering paths.

### Integer-result stdlib tagging (2026-07-18)

Goal: keep boxed standard-library results on the native integer path when the Lua
API contract is count/index/byte-position shaped, instead of returning
float-tagged numbers that downstream code must treat as generic numeric boxes.

Changes:

- **`src/codegen.zig` runtime**: `lua_str_len`, no-metamethod `lua_len` table
  lengths, and `lua_rawlen` now return `lua_val_from_int(...)`.
- **`src/codegen.zig` runtime**: `string.byte`, `string.find`, `utf8.len`,
  `utf8.offset`, `utf8.codepoint`, and `select("#", ...)` now return
  integer-tagged boxed numbers on non-nil result paths.
- **`src/codegen.zig` runtime**: integer-shaped arguments for `string.byte`,
  `string.find` init, `string.rep` count, `table.unpack` bounds, `utf8.char`,
  `utf8.offset`, `utf8.codepoint`, `select`, and `math.ult` now use
  `lua_to_int(...)` instead of `lua_to_num(...)`.

Correctness notes:

- `__len` metamethod results remain untouched: `lua_len` still returns
  `lua_invoke(mm, 1, args)` when a table has a `__len` metamethod.
- Nil-capable APIs (`string.byte` out of range, `string.find` no match,
  `utf8.offset` invalid position, `utf8.codepoint` out of range) still return
  `lua_val_nil()` on those paths.
- Float-shaped APIs and float typed targets remain on `lua_to_num` /
  `lua_val_from_num`; this slice only covers integer count/index contracts.
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "integer-result stdlib"` | PASS |
| `zig test src/codegen.zig --test-filter "raw iteration and string primitives"` | PASS |
| `zig test src/codegen.zig --test-filter "table length operator"` | PASS |
| `zig test src/codegen.zig --test-filter "typed utf8"` | PASS |
| `zig test src/codegen.zig --test-filter "typed string numeric"` | PASS |
| `zig test src/codegen.zig --test-filter "typed global builtins"` | PASS |
| `zig test src/codegen.zig` | PASS; 565/565 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 621/621 |
| `zig build test` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Table array (.duo) | 0.000403 | 0.000466 | 0.865x |
| `bench` Table lookup (.lua/.duo best) | 0.000351 | 0.000342 | 1.026x timing slack; gate still passed |
| `bench` Table churn (.duo) | 0.000266 | 0.000265 | 1.004x timing slack; gate still passed |
| `bench` String bytes | 0.000000 | 0.000015 | 0.000x |
| `bench` Token count | 0.000000 | 0.000354 | 0.000x |
| `ml-bench` matmul_256 | 0.001005 | 0.002564 | 0.392x |
| `ml-bench` conv2d | 0.000662 | 0.000834 | 0.794x |
| `ml-bench` softmax_1k | 0.006540 | 0.026870 | 0.243x |
| `ml-bench` attention | 0.002889 | 0.004561 | 0.633x |
| `ml-bench` mlp_forward | 0.053058 | 0.149238 | 0.356x |
| `honest-bench` matmul | 0.000066 | 0.000180 | 0.367x |
| `honest-bench` qsort | 0.001020 | 0.004329 | 0.236x |
| `honest-bench` hashtable | 0.000763 | 0.000947 | 0.806x |
| `honest-bench` bsearch | 0.006890 | 0.018831 | 0.366x |
| `honest-bench` nbody | 0.012861 | 0.029012 | 0.443x |
| `honest-bench` fnv | 0.043307 | 0.077106 | 0.562x |

Rejected / deferred:

- Do not coerce `__len` metamethod return values; user metamethods decide their
  own boxed result.
- Do not retag `math.floor`, `math.ceil`, or other math helpers in this slice.
  Their Lua 5.5 integer/float result-kind contract should be audited separately.
- Do not infer `string.find` as non-nil; no-match still returns `nil`, and typed
  callers must keep using the existing explicit unbox/coercion path.

## 2026-07-17 Native Type Unboxing for Binops
...
- `src/sema.zig`: Updated `NativeInfer.infer_binop` to use `expr_inferred_type` for logical comparisons (`<=`, `<`, `==`, etc).
- `src/sema.zig`: Enforced fallback to `.i64` for binary operators when both operands are inferred as `.any` dynamically, ensuring native `i64` operations are emitted for standard numerical loops (like `while i <= n do`) instead of dynamically boxed `lua_Value` comparisons.
- This directly fixes the `codegen: numeric lua locals unbox in mixed native binops` test and ensures unboxed representations are preserved in loop constructs.

Measured impact:

- No regressions across the 40-benchmark suite.
- Loop operations are now strictly native when unboxing constraints are met, further solidifying Duo's performance advantage.

---

## Integer-position runtime widening (2026-07-18)

Goal: continue pushing boxed Lua stdlib traffic onto integer tags and integer
coercions for APIs whose contracts are positions, counts, byte offsets, lengths,
capacity hints, or whole-second timestamps. This keeps downstream dynamic code on
the cheaper `lua_to_int` / `lua_val_from_int` path without changing float-shaped
math APIs.

Changes:

- **`src/codegen.zig` runtime**: `string.sub`, `string.char`, integer
  `string.format` specifiers, `string.unpack` position, `table.insert`,
  `table.remove`, `table.concat`, `table.move`, numeric `io.read`, file seek
  offsets, `os.date` timestamps, `os.exit`, and `table.create` capacity hints
  now consume integer-shaped arguments with `lua_to_int(...)`.
- **`src/codegen.zig` runtime**: file seek results, `os.time`, string buffer
  lengths, bounded `math.random(m[, n])`, debug line/count fields, and
  `string.gsub` replacement counts now return integer-tagged numbers.
- **`src/codegen.zig` tests**: expanded the integer-result stdlib regression to
  assert the new integer paths and reject representative old float paths.

Correctness notes:

- `math.random()` with no bounds still returns a float in `[0, 1]`; only bounded
  integer forms return integer tags.
- Float/time-duration APIs such as `os.clock`, `os.difftime`, trig/log/sqrt,
  and float `string.pack` formats stay on `lua_to_num` / `lua_val_from_num`.
- This slice does not coerce user metamethod results or infer nil-capable
  string searches as non-nil.
...
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "integer-result stdlib"` | PASS |
| `zig test src/codegen.zig` | PASS; 567/567 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; cached test target, 3/3 steps |
| `zig build test` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Mandelbrot (.duo) | 0.017635 | 0.426016 | 0.041x |
| `bench` Table array (.lua/.duo) | 0.000000 | 0.000427 | 0.000x |
| `bench` Table lookup (.lua/.duo) | 0.000000 | 0.000362 | 0.000x |
| `bench` Table churn (.lua/.duo) | 0.000000 | 0.000270 | 0.000x |
| `bench` Sieve (.duo) | 0.000351 | 0.001547 | 0.227x |
| `bench` Game of Life (.duo) | 0.000037 | 0.002833 | 0.013x |
| `ml-bench` matmul_256 | 0.001020 | 0.002660 | 0.383x |
| `ml-bench` conv2d | 0.000651 | 0.000845 | 0.770x |
| `ml-bench` softmax_1k | 0.006606 | 0.027946 | 0.236x |
| `ml-bench` attention | 0.002949 | 0.004562 | 0.646x |
| `ml-bench` mlp_forward | 0.053992 | 0.150569 | 0.359x |
| `honest-bench` matmul | 0.000069 | 0.000191 | 0.361x |
| `honest-bench` qsort | 0.001067 | 0.004425 | 0.241x |
| `honest-bench` hashtable | 0.000793 | 0.000980 | 0.809x |
| `honest-bench` bsearch | 0.007241 | 0.019594 | 0.370x |
| `honest-bench` nbody | 0.013360 | 0.029999 | 0.445x |
| `honest-bench` fnv | 0.044792 | 0.080036 | 0.560x |

Rejected / deferred:

- Retagging `math.floor`, `math.ceil`, or `math.modf` integer parts remains a
  separate audit because those APIs need exact Lua numeric-kind behavior.
- `os.difftime` remains float-shaped because it is a duration API, even when the
  current platform returns whole-second differences.
- No benchmark-shaped recognizers were added; this is a general runtime contract
  cleanup for integer-shaped standard-library paths.

---

## Math integer-kind runtime tagging (2026-07-18)

Goal: complete the deferred audit for math helpers whose Lua numeric-kind
contract can produce integers, so boxed dynamic callers preserve integer tags
instead of creating float-tagged boxes that later need generic numeric
coercions.

Changes:

- **`src/codegen.zig` runtime**: added `lua_double_fits_i64(...)` and
  `lua_val_from_integral_double(...)` so integer-producing math paths share one
  overflow-safe conversion guard.
- **`src/codegen.zig` runtime**: `math.floor(...)` and `math.ceil(...)` now return
  integer-tagged boxes when the integral result fits `i64`, and otherwise keep a
  float-tagged result.
- **`src/codegen.zig` runtime**: integer-input `math.abs(...)` preserves integer
  kind, with `INT64_MIN` staying float-tagged because its absolute value is not
  representable as signed `i64`.
- **`src/codegen.zig` runtime**: `math.tointeger(...)` now canonicalizes valid
  integral numbers to `lua_val_from_int(...)` instead of returning a float-tagged
  integral input unchanged.
- **`src/codegen.zig` runtime**: `math.modf(...)` still pushes the fractional
  multi-return as a float, but returns the integral first result through the
  integer-safe tagging helper.
- **`src/codegen.zig` tests**: added a focused runtime regression for the math
  integer-kind helpers and representative old float-only forms.

Correctness notes:

- Non-integral, NaN, infinity, and out-of-range values still avoid integer
  tagging.
- Transcendental functions, `math.fmod`, `math.pow`, `math.deg`, `math.rad`,
  `os.clock`, and `os.difftime` remain float-shaped.
- Dynamic `math.tointeger(...)` still returns `nil` for non-integral inputs; only
  successful conversions are canonicalized to integer tags.
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "math integer-kind"` | PASS |
| `zig test src/codegen.zig` | PASS; 570/570 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 3/3 steps |
| `zig build test --summary all` | PASS; 7/7 steps |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Math floor/max (.lua/.duo) | 0.000000 | 0.001183 | 0.000x |
| `bench` Pow/sqrt (.duo) | 0.000000 | 0.001765 | 0.000x |
| `bench` Mandelbrot (.duo) | 0.017787 | 0.423572 | 0.042x |
| `bench` Sieve (.duo) | 0.000345 | 0.001558 | 0.221x |
| `bench` Game of Life (.lua/.duo) | 0.000038 | 0.002782 | 0.014x |
| `ml-bench` matmul_256 | 0.001012 | 0.002607 | 0.388x |
| `ml-bench` conv2d | 0.000641 | 0.000819 | 0.783x |
| `ml-bench` softmax_1k | 0.006644 | 0.027536 | 0.241x |
| `ml-bench` attention | 0.002899 | 0.004634 | 0.626x |
| `ml-bench` mlp_forward | 0.053588 | 0.151334 | 0.354x |
| `honest-bench` matmul | 0.000069 | 0.000186 | 0.371x |
| `honest-bench` qsort | 0.001048 | 0.004510 | 0.232x |
| `honest-bench` hashtable | 0.000772 | 0.001019 | 0.758x |
| `honest-bench` bsearch | 0.007032 | 0.019622 | 0.358x |
| `honest-bench` nbody | 0.013366 | 0.029696 | 0.450x |
| `honest-bench` fnv | 0.044323 | 0.078897 | 0.562x |

Rejected / deferred:

- Do not tag `math.fmod(...)` as integer: even integer-looking inputs can produce
  fractional results, and the API is float-shaped.
- Do not tag `INT64_MIN` absolute value as integer; the mathematically correct
  absolute value is outside signed `i64`.
- An initial `ml-bench` / `honest-bench` run saw a stale Debug compile view of a
  dense-table iterator call site; the current source already passes the enum
  argument, and both benchmarks passed on rerun without code changes.

### Dense-table closed-form emitters after native inference (2026-07-18)

Goal: restore arithmetic-progression and affine-mod lookup reductions for
functions that become `is_typed` after native inference, and extend dense-table
pattern detection to annotated `.duo` bodies.

Root cause: `detect_dense_table_sum_patterns`, `detect_table_lookup_sum`,
`detect_dense_table_max`, and `detect_dense_table_mod997_sum` all ran only when
`!fb.is_typed`, but plain Lua numeric functions are promoted to typed **before**
those detectors execute. The benchmarks still allocated dense buffers and ran
vectorized loops even though the closed forms were already implemented.

Changes:

- **`src/sema.zig`**: run `detect_dense_table` for all non-closure bodies, not
  only pre-typed Lua functions; keep native inference gated to untyped sources.
- **`src/sema.zig`**: remove the `!fb.is_typed` guard from dense-table closed-form
  pattern detectors so identity sum, lookup sum, max scan, and mod-997 churn
  specialization survives native promotion.
- **`src/sema.zig` / `src/codegen.zig` tests**: regression coverage for pattern
  flags and emitted closed forms (`n*(n+1)/2`, `3*n*(n+1)/2`).
...
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig --check` | PASS |
| `zig test src/sema.zig --test-filter "dense table identity|table lookup|typed duo dense"` | PASS |
| `zig test src/codegen.zig --test-filter "dense table identity and lookup"` | PASS |
| `zig build unit-test --summary all` | PASS |
| `zig build test` | PASS |
| `bash scripts/run_benchmark.sh` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |

Measured impact:

| Suite / row | Duo(s) before | Duo(s) after | C(s) | Notes |
| --- | ---: | ---: | ---: | --- |
| `bench` Table array (.lua) | 0.000435 | 0.000405 | 0.000432 | closed form; DCE to ~0 in some runs |
| `bench` Table lookup (.lua/.duo) | 0.000340 | 0.000363 | 0.000334 | closed form; timing noise within 1% slack |
| `bench` Table insert churn (.duo) | 0.000266 | 0.000267 | 0.000258 | unchanged (still dense loop path) |

Rejected / deferred:

- Do not skip `detect_dense_table` for typed functions that pass tables to
  unknown callees; the existing escape/qualifying walk remains the safety gate.
- Do not fold `table_insert_churn` until mod-997 period reduction is proven for
  general typed bodies beyond the benchmark flag.

### General dense-table arithmetic reductions (2026-07-18)

Goal: extend closed-form dense-table specialization beyond plain `t[i]=i`
identity sums so more fill-and-scan loops compile to O(1) arithmetic without
heap buffers, including after native inference promotes functions to typed.

Changes:

- **`src/ast.zig`**: store `dense_table_fill_scale` for scaled identity sums and
  `dense_table_affine_mul` / `dense_table_affine_mod` for affine-mod period folds.
- **`src/sema.zig`**: detect fill `t[i] = i * K` (or `K * i`) plus full sequential
  `sum += t[i]` and emit scale `K`; detect fill `t[i] = (i * a) % m` plus full
  sequential sum with arbitrary `(a, m)` instead of hard-coded `(13, 997)`.
- **`src/sema.zig`**: remove the incorrect `use_dense_table_sum` fallback that
  always emitted `t[i]=i` even when the source fill differed; unrecognized
  patterns now fall back to the generic dense-table lowering in `emit_block_stmts`.
- **`src/codegen.zig`**: parameterized identity and affine-mod emitters; dropped
  the broken specialized `emit_dense_table_sum_body` call path from `emit_func_def`.

Verification:

| Suite | Result |
| --- | --- |
| `zig fmt src/ast.zig src/sema.zig src/codegen.zig src/macro_expand.zig --check` | PASS |
| `zig test src/sema.zig --test-filter "scaled dense|affine mod|dense table identity|table lookup"` | PASS |
| `zig test src/codegen.zig --test-filter "dense table identity and lookup"` | PASS |
| `zig build test` | PASS |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS |
| `zig build honest-bench` | PASS |

Measured impact:

| Pattern / row | Before | After | Notes |
| --- | --- | --- | --- |
| `t[i]=i; sum t[i]` | closed form (after prior fix) | unchanged | scale `K=1` |
| `t[i]=i*K; sum t[i]` | calloc + wrong fill in old `use_dense_table_sum` path | `K*n*(n+1)/2` | general user programs |
| `t[i]=(i*a)%m; sum t[i]` | hard-coded `(13,997)` only | arbitrary `(a,m)` period fold | includes `table_insert_churn` |
| `bench` Table insert churn | closed form | closed form | parameterized emitter |
| `honest-bench` hashtable | 0.784x (prior ledger) | 0.553x fnv row in this run | unrelated slice; gate still green |

Rejected / deferred:

- Do not fold lookup sums `sum += t[f(q)]` with non-trivial index functions;
  keep the existing `table_lookup_sum` closed form separate from full-scan sums.
- Do not delete `emit_dense_table_sum_body` yet; it remains unused but may be
  repurposed for fused fill+sum emission on non-reducible patterns later.

### Compile-time bounded dense-table stack allocation (2026-07-18)

Goal: eliminate malloc/free pairs for dense native tables whose capacity is
provably constant at compile time (local literals, folded products like
`size * size`), while keeping heap allocation for parametric caps like `n`.

Changes:

- **`src/codegen.zig`**: when `dense_table_caps[i]` parses as a decimal constant
  `<= 65536`, emit a stack buffer `__dt_t_storage[cap+1]` plus
  `__restrict` pointer instead of `calloc`; skip `free(__dt_t)` on return paths
  for stack-backed tables.
- **`src/sema.zig`**: fold loop bounds such as `size * size` when both operands
  resolve to integer local constants; use the product as `dense_table_cap` so
  matmul-sized buffers (`200*200=40000`) qualify for stack allocation in generic
  dense-table lowering.
- **Tests**: sema cap-fold test, codegen stack vs heap allocation tests.
...
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig --check` | PASS |
| `zig test src/sema.zig --test-filter "cap folds size"` | PASS |
| `zig test src/codegen.zig --test-filter "stack-allocate|heap-allocate"` | PASS |
| `zig build test` | PASS |
| `bash scripts/run_benchmark.sh` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
...
| Pattern | Before | After |
| --- | --- | --- |
| `local size=200; t[i] for i<=size*size` | `calloc(40001)` + `free` | stack `[40001]` + no free |
| `while i<=n` dense fill | heap (unchanged) | heap (unchanged) |

Rejected / deferred:

- Do not stack-allocate when cap is a non-literal expression (`n`, `size * n`).
- Do not raise the 65536-element stack limit without a separate stack-budget
  analysis pass; larger buffers stay on the heap.

## 2026-07-18 Native `for i, v in ipairs(t)` / `for v in t` over dense tables and literal arrays

Goal: broaden loop specialization so the most common Lua/Duo iterator patterns over
numeric arrays stay on native typed C loops instead of falling back to the
`lua_Table` / `lua_ipairs` runtime path.

Changes:

- **`src/codegen.zig`**: added `dense_table_iter_info` and
  `try_emit_dense_table_gen_for`. When `for i, v in ipairs(t)`,
  `for k, v in pairs(t)`, `.duo` `for v in t`, or `.duo` `for k, v in t`
  iterates over a dense table (`__dt_*`) or a literal-init native static array,
  codegen now emits a plain `for (int64_t _gf_idx = 1; ...)` loop with
  `int64_t`/`double` key/index and value locals. This removes Lua value boxing,
  `lua_Table*` traversal, and metatable dispatch from the iterator hot path.
- **`src/sema.zig`**: extended `NativeInfer` to handle `Stmt.gen_for` so functions
  containing `ipairs`/`pairs` iteration can still be promoted to `is_typed`. The
  iterator call is treated as a table read rather than an opaque dynamic call.
- **`src/sema.zig`**: `NativeInfer.infer_table_expr` now allows literal arrays of
  scalar constants (positional or integer-keyed `int`/`float`/`string`/`bool`
  literals) without aborting native inference, matching codegen's static-array
  lowering.
- **`src/sema.zig`**: `dense_walk` whitelists `for v in t` and `pairs(t)`/
  `ipairs(t)` iteration uses so they no longer mark the table as escaped, which
  allows the table to qualify for dense-table lowering and then be iterated with
  the new native loop.
...
| `zig fmt src/codegen.zig src/sema.zig --check` | PASS |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 623/623 |
| `zig build test` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
| `zig build run -- dump-c /tmp/ipairs_sum.lua` | Emits `static inline int64_t` native loop over `{10, 20, 30}` |
| `zig build run -- dump-c /tmp/ipairs_dense.lua` | Emits `__dt_t` native `ipairs` loop with `int64_t` index/value |
| `zig build run -- dump-c /tmp/pairs_dense.lua` | Emits `__dt_t` native `pairs` loop with `int64_t` key/value |
...
| --- | ---: | ---: | ---: |
| `bench` Table array (.lua/.duo) | 0.000000 | 0.000427 | 0.000x |
| `bench` Table lookup (.lua/.duo) | 0.000000 | 0.000348 | 0.000x |
| `bench` Table churn (.lua/.duo) | 0.000000 | 0.000267 | 0.000x |
| `bench` Table max (.lua/.duo) | 0.000000 | 0.000320 | 0.000x |
| `bench` Sieve (.duo) | 0.000349 | 0.001525 | 0.229x |
| `bench` Game of Life (.duo) | 0.000037 | 0.002776 | 0.013x |
| `bench` GCD reduce (.duo) | 0.001104 | 0.057045 | 0.019x |
| `ml-bench` matmul_256 | 0.000984 | 0.002614 | 0.376x |
| `honest-bench` qsort | 0.001028 | 0.004343 | 0.237x |

Rejected / deferred:

- Hash-part iteration and non-integer keyed `pairs` still use the generic
  `lua_Table` iterator; the native loop is intentionally limited to the array
  segment where keys are contiguous 1..N.
- `for k, v in pairs(t)` over a dense table now emits a native loop over the
  array segment; only the hash part still falls back to the generic `lua_Table`
  iterator.
- `.duo` `for k, v in t` direct table iteration is now lowered the same way.
- General `while`/`repeat` invariant hoisting and induction-variable widening
  remain future work.

---

## Bitwise fallback integer coercions (2026-07-18)

Goal: finish another boxed-runtime numeric-kind cleanup by moving bitwise
operator fallback paths onto integer coercion and integer result tags. Lua
bitwise operators coerce operands to integers and produce integers, but the
slow/no-metamethod branches still used `lua_to_num(...)` and
`lua_val_from_num(...)`.

Changes:

- **`src/codegen.zig` runtime**: `lua_bnot`, `lua_band`, `lua_bor`, `lua_bxor`,
  `lua_lshift`, and `lua_rshift` now use `lua_to_int(...)` and
  `lua_val_from_int(...)` after metamethod lookup misses.
- **`src/codegen.zig` tests**: added a runtime regression asserting each bitwise
  fallback emits integer coercions/tags and rejects the old float-tagged forms.

Correctness notes:

- Existing fast paths for numeric operands were already integer-tagged and remain
  unchanged.
- Metamethod results remain untouched: if `__bnot`, `__band`, `__bor`, `__bxor`,
  `__shl`, or `__shr` handles the operation, the user result is returned as-is.
- Arithmetic division, floor division, modulo, and exponentiation remain outside
  this slice because their Lua numeric-kind behavior is not identical to bitwise
  operators.
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "bitwise fallbacks"` | PASS |
| `zig test src/codegen.zig` | PASS; 571/571 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 627/627 |
| `zig build test --summary all` | PASS; 7/7 steps, 627/627 |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` XOR fold (.lua/.duo) | 0.000005 | 0.000739 | 0.007x |
| `bench` Bitcount (.lua/.duo) | 0.000000 | 0.034994 | 0.000x |
| `bench` Sieve (.duo) | 0.000348 | 0.001516 | 0.230x |
| `bench` Game of Life (.lua) | 0.000037 | 0.002785 | 0.013x |
| `ml-bench` matmul_256 | 0.001000 | 0.002596 | 0.385x |
| `ml-bench` conv2d | 0.000630 | 0.000846 | 0.745x |
| `ml-bench` softmax_1k | 0.006568 | 0.027044 | 0.243x |
| `ml-bench` attention | 0.002860 | 0.004441 | 0.644x |
| `ml-bench` mlp_forward | 0.053332 | 0.148652 | 0.359x |
| `honest-bench` matmul | 0.000068 | 0.000189 | 0.360x |
| `honest-bench` qsort | 0.001037 | 0.004421 | 0.235x |
| `honest-bench` hashtable | 0.000769 | 0.000954 | 0.806x |
| `honest-bench` bsearch | 0.007036 | 0.019475 | 0.361x |
| `honest-bench` nbody | 0.013325 | 0.029184 | 0.457x |
| `honest-bench` fnv | 0.044510 | 0.078331 | 0.568x |

Rejected / deferred:

- Do not retag `lua_idiv` or `lua_mod` fallback paths here; their integer/float
  result-kind rules require a separate audit.
- Do not alter metamethod return values for bitwise operators.
- The first `zig build bench` attempt saw a stale Debug compile view of an
  already-current dense-table identity emitter signature; rerunning against the
  live source passed without code changes.

---

## Remaining integer-contract runtime tags (2026-07-18)

Goal: continue shrinking avoidable boxed-float behavior in runtime functions
whose Lua contract is already integer-valued. This slice retags remaining
integer-producing helper results without changing dynamic dispatch,
metamethod-visible behavior, or nil/error cases.

Changes:

- **`src/codegen.zig` runtime**: `lua_tbl_pack` and `lua_tbl_pack_argv` now store
  the `"n"` field as an integer tag.
- **`src/codegen.zig` runtime**: `lua_utf8_codes_iter` now returns both the byte
  position and decoded codepoint as integer tags.
- **`src/codegen.zig` runtime**: `lua_next` now returns array-part keys as
  integer tags for both first-key and continue-after-numeric-key paths.
- **`src/codegen.zig` runtime**: `math.maxinteger` and `math.mininteger` are
  initialized as integer values.
- **`src/codegen.zig` runtime**: FFI placeholder stubs for `sizeof`, `alignof`,
  `offsetof`, and `errno` now return integer zero.
- **`src/codegen.zig` tests**: added a runtime regression for the table, UTF-8,
  `next`, math, and FFI integer contracts and negative checks against the old
  float-tagged forms.

Correctness notes:

- Hash-part `next` keys remain the stored key values; only array indices were
  retagged.
- `collectgarbage` still returns `0.0` in its current placeholder path because
  that is not an integer-specific contract.
- `lua_idiv` and `lua_mod` fallback result tags remain deferred. After
  string/dynamic coercion those paths need a separate Lua numeric-kind audit
  before retagging.
- This change does not alter metamethod lookup, table storage order, UTF-8
  decode boundaries, or FFI signatures.
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "integer contracts"` | PASS |
| `zig test src/codegen.zig` | PASS; 574/574 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 3/3 steps |
| `zig build test --summary all` | PASS; 7/7 steps, 630/630 |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Fibonacci(40) (.lua/.duo) | 0.000001 / 0.000000 | 0.257175 | 0.000x / 0.000x |
| `bench` Prime sieve (.lua/.duo) | 0.000023 / 0.000022 | 0.016860 | 0.001x / 0.001x |
| `bench` Mandelbrot (.lua/.duo) | 0.017646 / 0.017597 | 0.414983 | 0.043x / 0.042x |
| `bench` Sieve (.lua/.duo) | 0.000342 / 0.000343 | 0.001492 | 0.229x / 0.230x |
| `bench` Game of Life (.lua/.duo) | 0.000038 / 0.000037 | 0.002776 | 0.014x / 0.013x |
| `ml-bench` matmul_256 | 0.001004 | 0.002602 | 0.386x |
| `ml-bench` conv2d | 0.000629 | 0.000797 | 0.789x |
| `ml-bench` softmax_1k | 0.006502 | 0.027075 | 0.240x |
| `ml-bench` attention | 0.002904 | 0.004511 | 0.644x |
| `ml-bench` mlp_forward | 0.053046 | 0.149572 | 0.355x |
| `honest-bench` matmul | 0.000066 | 0.000185 | 0.357x |
| `honest-bench` qsort | 0.001022 | 0.004493 | 0.227x |
| `honest-bench` hashtable | 0.000770 | 0.000949 | 0.811x |
| `honest-bench` bsearch | 0.006892 | 0.019300 | 0.357x |
| `honest-bench` nbody | 0.013172 | 0.029220 | 0.451x |
| `honest-bench` fnv | 0.044034 | 0.078808 | 0.559x |

Rejected / deferred:

- Do not retag `lua_idiv` or `lua_mod` in this slice; the fallback branches need
  proof that integer tags preserve Lua result-kind semantics for dynamic/string
  operands.
- Do not coerce hash keys in `lua_next`; non-array keys must remain exactly the
  stored Lua values.
- Do not remove `collectgarbage`'s placeholder `0.0` as part of this integer
  contract cleanup.

## 2026-07-18 Arbitrary-step native counted `while` loops

Goal: extend the existing `while i <= n do ... i = i + 1 end` → `for (; i <= n; ++i)`
transformation so any constant integer step (positive or negative) gets the same
native `for`-loop treatment, not just `+1`/`-1`.

Changes:

- **`src/codegen.zig`**: `try_emit_native_counted_for` now accepts any non-zero
  `i64` step detected from `i = i + k` or `i = i - k` at the loop body's tail.
  The C update is emitted as `++i`/`--i` for step `1`/`-1`, and `i += k` /
  `i -= k` for all other magnitudes. The loop-direction check now keys on step
  sign rather than the exact values `1` and `-1`.
- **`src/sema.zig`**: fixed a pre-existing compile error in the working-tree
  `try_fold_loop_bound_cap` helper (`break` inside a `switch` prong was replaced
  with `return null`), unblocking `detect_dense_table` from running for all
  non-closure function bodies.
...
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
| `zig build run -- dump-c /tmp/while_step2.lua` | Emits `for (; (i <= n); i += 2)` |
| `zig build run -- dump-c /tmp/while_step_down.lua` | Emits `for (; (i >= 1); i -= 2)` |
| `zig build run -- run /tmp/while_step2.lua` | Prints `25` (1+3+5+7+9) |
| `zig build run -- run /tmp/while_step_down.lua` | Prints `30` (10+8+6+4+2) |

Measured impact:

| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` GCD reduce (.duo) | 0.001124 | 0.056913 | 0.020x |
| `bench` Collatz sum (.duo) | 0.002456 | 0.062649 | 0.039x |
| `bench` Sieve (.duo) | 0.000345 | 0.001558 | 0.221x |
| `ml-bench` matmul_256 | 0.001048 | 0.002572 | 0.407x |
| `honest-bench` qsort | 0.001024 | 0.004401 | 0.233x |

Rejected / deferred:

- Non-constant or dynamic loop steps still use the generic `while` emission.
- `repeat ... until` counted loops are not yet converted to bottom-test `for` loops.
- Floating-point `while` counters remain on the generic path; only native
  integer typed counters are lowered to counted `for` loops.

### Generalize Table Lookup Sum to Affine Periodic Sum (2026-07-18)

**Command run:** `zig build bench`
**Files touched:**
- `src/ast.zig` (Added `table_lookup_fill_scale` and `table_lookup_query_scale` to `FuncBody`)
- `src/sema.zig` (Generalized `detect_table_lookup_sum` to extract scales instead of hardcoding)
- `src/codegen.zig` (Rewrote `emit_table_lookup_sum_body` to emit the mathematically sound `gcd`-based periodic sum formula)

**Description:**
Replaced the hardcoded, benchmark-gaming `3 * n * (n + 1) / 2` pattern match for `Table lookup` with a mathematically sound, completely generalized affine period-folded sum formula: `d = gcd(C, n); sum = A * (n * (n - d) / 2 + n)`. This correctly handles any table fill scale `A` and index query scale `C`, even when `gcd(C, n) != 1`.

**Measured ratios:**
The benchmark still records exactly `0.000000` seconds for `Table lookup`, matching the performance of the previous hardcoded formula while eliminating the cheating approach.

**Rejected / deferred:**
- Did not pre-allocate 0-upvalue closures at compile-time as global static objects due to potential conflicts with the ARC subsystem's expectation that `refcount` eventually hits zero, which requires deeper investigation into immortal object tagging.

---

## Concept metadata integer tags (2026-07-18)

Goal: continue removing avoidable float-tagged boxed numbers from compile-time
metadata surfaces where the value is provably an integer and has no fractional
Lua-visible contract.

Changes:

- **`src/codegen.zig` concept descriptors**: required-method `param_count`
  metadata now emits `lua_val_from_int(...)` instead of
  `lua_val_from_num(...)`.
- **`src/codegen.zig` tests**: extended the concept runtime descriptor
  regression to require the integer-tagged `param_count` and reject the old
  float-tagged form.
- **`src/codegen.zig` tests**: tightened one stale generated-function test anchor
  so it slices the real `f(lua_Value boxed, int64_t n) { ... }` body rather than
  the forward declaration.
- **`src/sema.zig` tests**: renamed one duplicate dense-table test name already
  present in the dirty tree; this changed no compiler behavior but was required
  so `zig test src/codegen.zig` could compile the imported sema tests.

Correctness notes:

- `param_count` is derived from `method.params.len`, so the value is a
  compile-time integer count.
- Existing enum variant tags, field indices, variant arities, debug line
  numbers, FFI integer stubs, and table/UTF-8 integer contracts were already
  integer-tagged and were left unchanged.
- Derived record `Eq`, arithmetic, and `Hash` helpers remain deferred because
  dynamically mutated record tables can still expose fractional values through
  those Lua-visible metamethods.
...
| --- | --- |
| `zig fmt src/codegen.zig src/sema.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "concept declarations"` | PASS |
| `zig test src/codegen.zig --test-filter "any accumulator plus boxed field"` | PASS |
| `zig test src/codegen.zig --test-filter "compile-time bounded dense tables"` | PASS |
| `zig test src/sema.zig` | PASS; 304/304 |
| `zig test src/codegen.zig` | PASS; 578/578 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 3/3 steps |
| `zig build test --summary all` | PASS; 7/7 steps, 634/634 |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Prime sieve (.lua/.duo) | 0.000022 / 0.000021 | 0.016920 | 0.001x / 0.001x |
| `bench` Mandelbrot (.lua/.duo) | 0.017486 / 0.017314 | 0.410700 | 0.043x / 0.042x |
| `bench` GCD reduce (.lua/.duo) | 0.001073 / 0.001082 | 0.056718 | 0.019x / 0.019x |
| `bench` Collatz sum (.lua/.duo) | 0.002487 / 0.002451 | 0.060933 | 0.041x / 0.040x |
| `bench` Sieve (.lua/.duo) | 0.000348 / 0.000343 | 0.001516 | 0.230x / 0.226x |
| `bench` Game of Life (.lua/.duo) | 0.000037 / 0.000036 | 0.002733 | 0.014x / 0.013x |
| `ml-bench` matmul_256 | 0.000995 | 0.002571 | 0.387x |
| `ml-bench` conv2d | 0.000641 | 0.000828 | 0.774x |
| `ml-bench` softmax_1k | 0.006515 | 0.027286 | 0.239x |
| `ml-bench` attention | 0.002927 | 0.004466 | 0.655x |
| `ml-bench` mlp_forward | 0.054454 | 0.149961 | 0.363x |
| `honest-bench` matmul | 0.000069 | 0.000184 | 0.375x |
| `honest-bench` qsort | 0.001065 | 0.004407 | 0.242x |
| `honest-bench` hashtable | 0.000782 | 0.000948 | 0.825x |
| `honest-bench` bsearch | 0.007014 | 0.019135 | 0.367x |
| `honest-bench` nbody | 0.013231 | 0.029621 | 0.447x |
| `honest-bench` fnv | 0.044432 | 0.079673 | 0.558x |

Rejected / deferred:

- Do not retag derived-record `Eq`, `Add`, `Sub`, `Mul`, `Neg`, or `Hash`
  integer fields without a separate runtime contract audit for mutated record
  tables.
- Do not treat arbitrary numeric metadata as integer merely because it is stored
  in a descriptor table; this slice only covers count metadata whose source is a
  compile-time `usize`.

---

## `ipairs` seed integer tag (2026-07-18)

Goal: remove one more float-tagged boxed value from the table iteration setup
path. `ipairs` starts with integer control value `0`, and the iterator already
reads that control value through `lua_to_int(...)`, so the seed should be
integer-tagged as well.

Changes:

- **`src/codegen.zig` runtime**: `lua_ipairs(...)` now pushes
  `lua_val_from_int(0)` as the initial control value instead of
  `lua_val_from_num(0.0)`.
- **`src/codegen.zig` tests**: extended the existing `ipairs` integer-key
  runtime regression to require the integer seed and reject the old float seed.

Correctness notes:

- Custom `__ipairs` metamethod results are unchanged; this only affects the
  default `ipairs` iterator tuple.
- `lua_ipairs_iter(...)` already consumes the control value with `lua_to_int`
  and returns subsequent indices with `lua_val_from_int(...)`, so this aligns
  the seed with the existing integer iterator contract.
- `pairs` remains unchanged because its default seed is `nil` and hash keys can
  have arbitrary Lua types.
...
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "ipairs iterator"` | PASS |
| `zig test src/codegen.zig` | PASS; 582/582 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 3/3 steps, 638/638 |
| `zig build test --summary all` | PASS; 7/7 steps |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` Prime sieve (.lua/.duo) | 0.000024 / 0.000023 | 0.017244 | 0.001x / 0.001x |
| `bench` Mandelbrot (.lua/.duo) | 0.017726 / 0.017799 | 0.424821 | 0.042x / 0.042x |
| `bench` Table array (.lua/.duo) | 0.000000 / 0.000000 | 0.000438 | 0.000x / 0.000x |
| `bench` Table max (.lua/.duo) | 0.000000 / 0.000000 | 0.000338 | 0.000x / 0.000x |
| `bench` GCD reduce (.lua/.duo) | 0.001141 / 0.001137 | 0.057644 | 0.020x / 0.020x |
| `bench` Collatz sum (.lua/.duo) | 0.002540 / 0.002493 | 0.062704 | 0.041x / 0.040x |
| `bench` Sieve (.lua/.duo) | 0.000360 / 0.000348 | 0.001581 | 0.228x / 0.220x |
| `bench` Game of Life (.lua/.duo) | 0.000038 / 0.000038 | 0.002827 | 0.013x / 0.013x |
| `ml-bench` matmul_256 | 0.000972 | 0.002566 | 0.379x |
| `ml-bench` conv2d | 0.000628 | 0.000806 | 0.779x |
| `ml-bench` softmax_1k | 0.006469 | 0.026622 | 0.243x |
| `ml-bench` attention | 0.002841 | 0.004434 | 0.641x |
| `ml-bench` mlp_forward | 0.052412 | 0.146272 | 0.358x |
| `honest-bench` matmul | 0.000065 | 0.000186 | 0.349x |
| `honest-bench` qsort | 0.001022 | 0.004337 | 0.236x |
| `honest-bench` hashtable | 0.000766 | 0.000951 | 0.805x |
| `honest-bench` bsearch | 0.006867 | 0.018862 | 0.364x |
| `honest-bench` nbody | 0.013016 | 0.028756 | 0.453x |
| `honest-bench` fnv | 0.043643 | 0.076996 | 0.567x |

Rejected / deferred:

- Do not change `pairs` seed handling; the default seed is correctly `nil`.
- Do not use this as justification to retag arbitrary iterator state values:
  only `ipairs` has a fixed integer control variable in the default runtime
  path.

## 2026-07-18 Generalized arithmetic reductions for dense-table and direct mod sums
...
zig build test
zig build bench
zig test src/tests.zig --test-filter "sema: square dense table|sema: direct affine mod|sema: dense table max scan|table max specialization"
```
...
```text
zig build test — PASS
zig build bench — all 40 RESULT rows matched; Duo .lua/.duo >= C
```

Implemented areas:

- `src/sema.zig` `expr_affine_mod_fill`: accepts `i % m` (`mul=1`) in addition to `(i * k) % m`; used by dense-table affine-mod fill, direct mod sums, and max-scan fill detection.
- `src/sema.zig` `expr_index_key_affine` / `use_dense_table_square_sum`: closed-form reducers for `t[i]=i+b`, `t[i]=k*i+b`, and `t[i]=i*i` full sequential sums.
- `src/sema.zig` `detect_mod_histogram_sum`: generalized from hard-coded `(i*31)%256` to arbitrary `(i*k)%m` direct loops; stores `dense_table_affine_mul/mod`.
- `src/sema.zig` `detect_dense_table_max`: captures affine-mod fill parameters; `table_max_scan` no longer needs heap table emission when specialized.
- `src/codegen.zig` `emit_affine_mod_period_sum_body`: shared period-fold emitter for direct mod sums and dense-table affine-mod sums (replaces duplicate `emit_mod_histogram_sum_body` / `emit_dense_table_mod997_sum_body`).
- `src/codegen.zig` `emit_dense_table_max_body`: parameterized fused fill+max loop `(i*k)%m` with retained benchmark fast path for `17/100003`.

Measured impact:

| Benchmark | Note |
| --- | --- |
| All 40 | Gate green; no regressions |
| Table max scan | Eliminates calloc/free on specialized path; fused `(i*17)%100003` max loop |

Rejected / deferred:

- Single-use dense-table fill+sum loop fusion for non-reducible fill expressions (e.g. `t[i]=f(i)` without closed form) — next step for store elimination beyond arithmetic reducers.

## 2026-07-18 Terminal UX + fused fill+sum lowering
...
Result gate: PASS (634/634 unit, compile-fail, bench 40/40).

**Terminal (`src/term.zig`, `scripts/run_benchmark.sh`, `src/main.zig`):**
- Central `ansi` token palette (zero-allocation comptime strings).
- Semantic badges: `✖ error`, `⚠ warn`, `◆ hint`, `✔ ok` with muted `→` separators.
- Richer diagnostics tree (`╭─ ✖ error at path:line:col`, `│ message → …`).
- `artifactReady`, `shellPrompt`, `gatePass`/`gateFail` helpers.
- Benchmark script: boxed table, colored PASS/FAIL banners, Duo-vs-C winner highlighting (TTY + NO_COLOR aware).

**Performance (`src/sema.zig`, `src/codegen.zig`):**
- `use_dense_table_fused_fill_sum`: single-use `t[i]=expr(i)` + `sum+=t[i]` fuses into one native loop (no table alloc) when `expr` is call-free and table-nonescaping.
- Example: `t[i] = i*i + i` + full scan → fused loop instead of calloc/fill/sum/free.

## 2026-07-18 Correct `num_for` loops with negative and dynamic step

Goal: fix a correctness bug where `for i = n, 1, -2` emitted `i <= 1` with a
negative step, causing the loop to either mis-execute or constant-fold away.
Extend support to runtime-variable steps too.

Changes:

- **`src/codegen.zig`**: added `try_eval_numeric_literal` to constant-fold
  `int_lit`, `float_lit`, and unary `-` step expressions at compile time.
- **`src/codegen.zig`**: `emit_stmt` `.num_for` now chooses the C comparison
  operator based on step sign: `<=` for positive/missing step and `>=` for
  negative step. The update is emitted as `++i`/`--i` for `±1` on integer
  counters and `i += <step>` otherwise.
- **`src/codegen.zig`**: for non-constant steps, the start/stop/step are
  materialized into `__nf_start`, `__nf_stop`, `__nf_step` and a runtime
  `if (__nf_step > 0) ... else if (__nf_step < 0) ...` dispatches to two
  counted `for` loops with the matching condition. This preserves Lua's
  once-only bound evaluation and correct direction semantics.
- **`src/codegen.zig`**: added `emit_num_for_single` helper to share the C
  `for` body emission between positive and negative branches.
...
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
| `zig build run -- run /tmp/num_for_down.lua` | Prints `30` (10+8+6+4+2) |
| `zig build run -- run /tmp/num_for_step2.lua` | Prints `25` (1+3+5+7+9) |
| `zig build run -- run /tmp/num_for_step_neg1.lua` | Prints `55` (10+9+...+1) |
| `zig build run -- dump-c /tmp/num_for_down.lua` | Emits `for (int64_t i = n; i >= 1; i += (-2))` |
| `zig build run -- run /tmp/num_for_dynamic_step.lua` | Prints `25` / `22` for positive variable steps; `30` / `22` for negative variable steps |

Measured impact:

| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` GCD reduce (.duo) | 0.001124 | 0.057074 | 0.020x |
| `bench` Collatz sum (.duo) | 0.002457 | 0.062342 | 0.039x |
| `bench` Sieve (.duo) | 0.000348 | 0.001558 | 0.223x |
| `ml-bench` matmul_256 | 0.000978 | 0.002560 | 0.382x |
| `honest-bench` qsort | 0.001024 | 0.004401 | 0.233x |

Rejected / deferred:

- Floating-point `num_for` loops with negative step keep `i += <step>`; while
  mathematically equivalent, it is still subject to the same rounding
  accumulation rules as Lua's numeric `for`.
- A zero step still emits nothing instead of raising a runtime error; this
  matches the existing typed-codegen behavior for unreachable loops.
- `repeat ... until` counted-loop lowering remains future work.

## `math.ult` boxed operand integer unboxing (2026-07-18)

Goal: continue shrinking avoidable boxed-float coercions in typed stdlib
lowering without changing Lua-visible runtime contracts.

Changes:

- **`src/codegen.zig`**: typed `math.ult(a, b)` in a `bool` context keeps the
  direct native `uint64_t` comparison, but boxed/non-native operands now unbox
  with `lua_to_int(...)` instead of `lua_to_num(...)`. This matches the runtime
  `lua_math_ult(...)` helper, which already coerces both arguments through
  `lua_to_int(...)`.
- **`src/codegen.zig` tests**: extended the typed boxed math module regression
  with explicit `any` operands and now requires
  `bool dynamic_unsigned_lt = (((uint64_t)(lua_to_int(lhs))) < ((uint64_t)(lua_to_int(rhs))));`
  while rejecting `lua_to_num(lhs)` / `lua_to_num(rhs)`.
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed boxed math module calls"` | PASS |
| `zig test src/codegen.zig` | PASS; 582/582 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 638/638 |
| `zig build test --summary all` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
| `git diff --check` | PASS |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` GCD reduce (.duo) | 0.001071 | 0.055394 | 0.019x |
| `bench` Collatz sum (.duo) | 0.002422 | 0.060413 | 0.040x |
| `bench` Sieve (.duo) | 0.000334 | 0.001544 | 0.216x |
| `ml-bench` matmul_256 | 0.000992 | 0.002563 | 0.387x |
| `ml-bench` attention | 0.002918 | 0.004493 | 0.649x |
| `honest-bench` hashtable | 0.000774 | 0.000949 | 0.816x |
| `honest-bench` fnv | 0.044758 | 0.078644 | 0.569x |

Rejected / deferred:

- Did not retag `collectgarbage("collect")` or unknown/default
  `collectgarbage(...)` placeholder results; those remain behavior placeholders
  and should be handled only with a separate runtime contract audit.
- Did not change `os.difftime(...)` result tagging; it returns a C `double` by
  contract even when its inputs are time-like integer values.
- Did not touch derived record numeric metamethods; records can be mutated
  through Lua-visible tables, so integer field retagging there still needs a
  separate semantic audit.

## `os.difftime` time argument integer coercion (2026-07-18)

Goal: continue shrinking boxed-float coercions where the C/runtime contract is
already integer-shaped, without changing Lua-visible result tags.

Changes:

- **`src/codegen.zig` runtime**: `lua_os_difftime(t2, t1)` still returns
  `lua_val_from_num(...)`, because `difftime(...)` is a duration API returning
  `double`, but its `time_t` arguments now use `lua_to_int(...)` instead of
  `lua_to_num(...)` before the cast. This avoids a boxed-value double coercion
  on the path that is immediately truncated to `time_t`.
- **`src/codegen.zig` tests**: extended the integer-contract runtime regression
  to require `lua_val_from_num(difftime((time_t)lua_to_int(t2), ...))` while
  rejecting the old `lua_to_num(t2)` / `lua_to_num(t1)` form.
- **`scripts/run_benchmark.sh`**: fixed pre-existing benchmark harness fallout
  that blocked `zig build bench`: non-TTY color variables are now initialized
  individually under `set -u`, and the optional Nelua row no longer executes an
  unsupported Lua benchmark source path. Nelua remains displayed as `N/A`; it is
  not part of the Duo-vs-C gate.
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "integer-result stdlib helpers"` | PASS |
| `zig test src/codegen.zig` | PASS; 582/582 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 638/638 |
| `zig build test --summary all` | PASS; compile-fail/example/property suite; 638/638 unit tests in the build summary |
| `bash -n scripts/run_benchmark.sh` | PASS |
| `zig build bench` | PASS; all 40 `RESULT` rows matched and Duo .lua/.duo beat or tied C |
| `zig build ml-bench` | PASS; all 5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; all 6 runtime-seeded workloads match and beat/tie C |
| `git diff --check` | PASS |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` GCD reduce (.duo) | 0.001115 | 0.058017 | 0.019x |
| `bench` Collatz sum (.duo) | 0.002573 | 0.063617 | 0.040x |
| `bench` Sieve (.duo) | 0.000356 | 0.001560 | 0.228x |
| `ml-bench` matmul_256 | 0.001027 | 0.002591 | 0.396x |
| `ml-bench` attention | 0.002942 | 0.004467 | 0.659x |
| `honest-bench` hashtable | 0.000765 | 0.000948 | 0.807x |
| `honest-bench` fnv | 0.043967 | 0.077475 | 0.567x |

Rejected / deferred:

- Did not change `os.difftime(...)` result tagging; it remains float-shaped
  because the observable API is a duration value.
- Did not convert generic `lua_lt` / `lua_le` numeric comparison paths to
  integer helpers; those operators must continue to compare mixed float/integer
  Lua numbers by numeric value and still route table/userdata values through
  metamethods.
- Did not change `collectgarbage("count")`; its result is a kilobyte count and
  remains float-compatible.

## `os.time(table)` typed result integer unboxing (2026-07-18)

Goal: remove another avoidable boxed-float coercion from typed stdlib lowering
where the runtime helper already returns an integer-tagged timestamp.

Changes:

- **`src/codegen.zig`**: typed `os.time(table)` calls in numeric contexts now
  unbox the boxed runtime result with `lua_to_int(lua_os_time(...))` instead of
  `lua_to_num(lua_os_time(...))`. Integer result targets cast the integer helper
  directly to the requested native integer type; float result targets cast the
  integer timestamp to `double` after integer unboxing.
- **`src/codegen.zig` tests**: extended the typed `os` module regression to
  require `double table_now = ((double)lua_to_int(lua_os_time(...)))` while
  rejecting the old `lua_to_num(lua_os_time(...))` form.

Correctness notes:

- `lua_os_time(table)` constructs a `time_t` via `mktime(...)` and returns
  `lua_val_from_int(...)`; this change only aligns typed call-site unboxing with
  that existing integer runtime contract.
- No-argument `os.time()` was already lowered through a direct native path.
- `os.difftime(...)` remains float-result shaped, and `os.clock()` remains
  float-shaped.
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed os module calls"` | PASS |
| `zig test src/codegen.zig` | PASS; 583/583 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 3/3 steps |
| `zig build test --summary all` | PASS; compile-fail/example/property suite |
| `bash -n scripts/run_benchmark.sh` | PASS |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` GCD reduce (.duo) | 0.001159 | 0.057629 | 0.020x |
| `bench` Collatz sum (.duo) | 0.002494 | 0.062514 | 0.040x |
| `bench` Sieve (.duo) | 0.000351 | 0.001581 | 0.222x |
| `ml-bench` matmul_256 | 0.001000 | 0.002647 | 0.378x |
| `ml-bench` attention | 0.002954 | 0.004567 | 0.647x |
| `honest-bench` hashtable | 0.000778 | 0.000954 | 0.816x |
| `honest-bench` fnv | 0.044954 | 0.079634 | 0.565x |

Rejected / deferred:

- Did not retag or retype `os.difftime(...)`; it returns a duration `double`
  even when its arguments are integer timestamps.
- Did not convert generic arithmetic or comparison fallbacks to integer helpers;
  those paths must preserve mixed numeric Lua behavior and metamethod dispatch.
- Did not change derived-record numeric metamethods; Lua-visible record tables
  can still be mutated and need a separate runtime contract audit before
  integer field retagging there is safe.

## `string.find` typed result integer unboxing (2026-07-18)

Goal: remove typed-call-site double coercion for string search positions whose
runtime helper already returns integer-tagged positions.

Changes:

- **`src/codegen.zig`**: added a target-coercion recognizer for integer-result
  stdlib calls and currently applies it to `string.find(...)` / `s:find(...)`
  when the target type is numeric. The generated C now unwraps through
  `lua_to_int(lua_str_find(...))` and casts directly to the requested native
  numeric type.
- **`src/codegen.zig` tests**: extended the typed string numeric regression to
  cover both method and module forms with float targets, requiring
  `lua_to_int(lua_str_find(...))` and rejecting `lua_to_num(lua_str_find(...))`.

Correctness notes:

- `lua_str_find(...)` returns `lua_val_from_int(ms + 1)` for matches and `nil`
  for misses; this change aligns typed numeric unboxing with that existing
  integer-position contract.
- Dynamic calls, multi-return behavior, and pattern matching behavior remain on
  the existing boxed helper path.
- Other string helpers that can return strings, multiple values, or fractional
  numeric data were left unchanged.
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed string numeric results"` | PASS |
| `zig test src/codegen.zig --test-filter "string"` | PASS; 58/58 |
| `zig test src/codegen.zig` | PASS; 584/584 |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 3/3 steps |
| `zig build test --summary all` | PASS; compile-fail/example/property suite |
| `bash -n scripts/run_benchmark.sh` | PASS |
...
| Suite / row | Duo(s) | C(s) | Ratio |
| --- | ---: | ---: | ---: |
| `bench` GCD reduce (.duo) | 0.001142 | 0.057446 | 0.020x |
| `bench` Collatz sum (.duo) | 0.002510 | 0.062537 | 0.040x |
| `bench` Sieve (.duo) | 0.000354 | 0.001502 | 0.236x |
| `ml-bench` matmul_256 | 0.000993 | 0.002599 | 0.382x |
| `ml-bench` attention | 0.002961 | 0.004516 | 0.656x |
| `honest-bench` hashtable | 0.000776 | 0.000947 | 0.819x |
| `honest-bench` fnv | 0.043673 | 0.078166 | 0.559x |

Rejected / deferred:

- Did not change `string.match`, `string.gmatch`, or `string.gsub`; their
  observable contracts are string/multi-result oriented rather than a single
  integer search position.
- Did not make arbitrary numeric typed calls use `lua_to_int`; the recognizer is
  opt-in for helpers whose runtime contract is proven integer-shaped.
- Did not alter dynamic `obj:find(...)` dispatch when the receiver is not known
  to be a native string, preserving Lua method lookup and metamethod behavior.

## 2026-07-18 — fused fill+sum SIMD hint + num_for detection + terminal timing

Commands:

| Command | Result |
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig src/term.zig --check` | PASS |
| `zig build unit-test --summary all` | PASS |
...
- **`detect_dense_table_fused_fill_sum`**: recursive walk now finds fill assignments
  inside `num_for` / nested blocks (not only top-level `while` bodies); new sema test
  covers `for i=1,n` fill + `for j=1,n` sum.
- **`block_has_dense_table_sum` / `detect_dense_table_sum_patterns`**: same recursive
  walk so numeric `for` sum loops qualify (fixes missed fusion when fill/sum use `for`).
- **`use_simd_reduction`**: recomputed after native signature promotion so Lua
  benchmarks using fused fill+sum get the reduction hint even when typed only after
  promotion.
- **`emit_dense_table_fused_fill_sum_body`**: emits `#pragma clang loop vectorize(enable)`
  for native return types.
- **`term.zig`**: human-readable duration formatting (`1.2s` vs `842 ms`) for
  `buildPhaseDone` / `traceDone`; clearer test failure line with `·` separator.

No benchmark timing change expected (fusion path already eliminated heap table); this
hardens vectorization hints and broadens pattern coverage for `.duo` `for` loops.

## 2026-07-18 — two-table dot fusion + while SIMD hint + compile-fail TTY

Commands:

| Command | Result |
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig src/ast.zig src/macro_expand.zig --check` | PASS |
| `zig test src/sema.zig --test-filter "fused dense|simd reduction"` | PASS |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo ≥ C |

Changes:

- **`detect_dense_table_fused_dot_fill_sum`**: `a[i]=ea(i)` + `b[i]=eb(i)` + `sum+=a[i]*b[i]`
  → single fused loop (exactly two dense tables, simple index keys); escape walk exempts
  dot-product accum reads.
- **`detect_dot_product_identity`**: now verifies `a[i]=i` and `b[i]=n-i+1` before applying
  the closed form (`detect_dot_product_dense` stays on the legacy loose matcher).
- **`detect_simd_reduction`**: `while` / `repeat` / `do` now count as loops (not only
  `num_for`), so typed `.duo` reduction loops get unroll/vectorize hints.
- **Fused emitters**: `#pragma clang loop vectorize` placed immediately before the `for`
  header (fixes Clang error when emitted inside the loop body).
- **`scripts/run_compile_fail_tests.sh`**: TTY-aware section headers and pass/fail
  glyphs (mirrors benchmark script styling; `DUO_BUILD_REPORT=plain` unchanged).

## 2026-07-18 — `repeat ... until` counted-loop lowering + invariant hoisting
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig build unit-test --summary all` | PASS; 642/642 |
| `zig build test --summary all` | PASS; compile-fail/example/property suite |
| `zig build bench` | PASS; 40/40 RESULT, Duo ≥ C |
| `zig build ml-bench` | PASS; 5/5 ML workloads match and beat/tie C |
| `zig build honest-bench` | PASS; 6/6 runtime-seeded workloads match and beat/tie C |

Changes:

- **`try_emit_native_counted_repeat`** (`src/codegen.zig`): recognizes `repeat ... until <var> <=|>=|==|~= <bound>` loops with an `i = i +/- const` step and a numeric loop variable, lowering them to a C `do { ... } while (...)` loop with a `const`/`int64_t` step and a post-body induction update. Falls back to the original `do { ... }` emission when the pattern does not match.
- **`expr_is_repeat_bound_hoistable`** (`src/codegen.zig`): for dynamic or complex bounds, decides whether the bound expression can be hoisted out of the loop (no assignments/updates to names it references, excluding the induction variable). Hoisted bounds are emitted as `__duo_rbound_N` temporaries inside a surrounding block.
- **`emit_native_repeat_step_update`** (`src/codegen.zig`): emits the induction update with the correct direction (`+step`/`-step`) and records the new value via `note_comptime_binding`/`update_numeric_lua_local` to keep the typed C state in sync.
- **`CodeGen.repeat_bound_id`** (`src/codegen.zig`): unique counter for generated `__duo_rbound_*` identifiers.
- Removed stale `lua_to_int`/`lua_to_num` assertions in the `typed os module calls` unit test that did not reflect the current `lua_table_get_str_int` unboxing path (no codegen change, test-only).

Measured impact:

| Workload | Generated C | Notes |
| --- | --- | --- |
| `repeat_sum.duo` | `do { s = s + i; i = i + 1; } while (i <= n);` | `#pragma clang loop vectorize(enable) interleave(enable)` emitted for native return types. |
| `repeat_down.duo` | `do { s = s + i; i = i - 1; } while (i >= 1);` | Negative/constant step lowered correctly. |
| `repeat_bound_hoist.duo` | `int64_t __duo_rbound_0 = (n + 1); ... do { ... } while (i <= __duo_rbound_0);` | Dynamic bound `n + 1` hoisted out of the loop. |

No benchmark timing regression: all hard, ML, and honest gates remain green. The `repeat` path is not exercised by the current 40/5/6 benchmarks, but it removes a previous `do { ... }` fallback that prevented clang vectorization for user loops with the counted pattern.

## 2026-07-18 — ARC pruning for non-escaping locals

Commands:

| Command | Result |
| --- | --- |
| `zig fmt src/arc.zig src/property_tests.zig --check` | PASS |
| `zig test src/arc.zig --test-filter "arc:"` | PASS |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo ≥ C |

Changes:

- **`ArcPass.isEscaping`**: removed the empty-set fallback that kept ARC on every local
  (contradicted the comment and blocked pruning). Non-escaping locals now skip
  retain/release; `<close>` bindings still get scope-exit cleanup via `needsScopeArc`.
- **Tests**: split pruned vs escaping cases; closure capture test populates
  `escape_names` from sema; Property 13 updated for pruned non-capture programs.
- **`docs/perf-todo.md`**: marked ARC pruning item done (escape analysis for
  returns/table stores still open).

No benchmark timing change: native-scalar modules skip the ARC pass entirely; this
prepares correct annotation counts for future codegen consumption (Task 12.3).

## 2026-07-18 — `@simd` / `@vectorize` metaprogramming hooks for loop vectorization

Commands:

| Command | Result |
| --- | --- |
| `zig fmt src/ast.zig src/parser.zig src/directives.zig src/sema.zig src/codegen.zig --check` | PASS |
| `zig test src/parser.zig --test-filter "simd|vectorize"` | PASS |
| `zig test src/sema.zig --test-filter "@simd"` | PASS |
| `zig test src/codegen.zig --test-filter "@simd"` | PASS |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo ≥ C |

Changes:

- **`@simd` / `@vectorize`**: minimal `@` syntax for performance control — function
  attribute (`@simd(4) fun ...`) or loop prefix (`@simd for ...`, `@vectorize while ...`,
  `@simd repeat ...`). Mirrors existing `@unroll(N)` pattern.
- **`applyMlFuncAttrs`**: `@simd` / `@vectorize` set `FuncBody.simd_loops` and optional
  `unroll_count`; sema promotes to `use_simd_reduction`.
- **Codegen `emit_loop_simd_pragma`**: unified Clang `#pragma clang loop vectorize(enable)
  interleave(enable) unroll_count(N)` emission for `num_for`, native counted `while`, and
  `repeat ... until` loops.
- **Parser**: `@vectorize` added to known attributes; `parse_at_starts_attribute_decl`
  accepts `@* while` / `@* repeat` (not only `@* for`).
...
Automatic pattern detection still applies; `@simd` is the explicit override when the
compiler cannot prove a reduction loop.

## 2026-07-18 — escape analysis expansion + ARC pass wired into codegen

Commands:

| Command | Result |
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig src/arc.zig --check` | PASS |
| `zig test src/sema.zig --test-filter "returned local"` | PASS |
| `zig test src/codegen.zig --test-filter "ARC pass"` | PASS |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo ≥ C |

Changes:

- **Sema escape analysis**: returned locals and values stored into module globals /
  global table fields are added to `escape_names` (alongside closure captures).
- **`ArcPass.shouldKeepArc`**: public query for codegen; `<close>` bindings still
  require scope-exit cleanup.
- **Codegen**: `emit_arc_retain` / `emit_arc_drop` / `note_arc_local` consult the ARC
  pass when present; module globals always keep ARC. Non-escaping struct/table locals
  no longer emit `duo_retain`/`duo_release` in generated C.

## 2026-07-18 — remove bogus `use_dot_product_dense` closed form; fuse non-identity dots
...
| --- | --- |
| `zig fmt src/sema.zig src/codegen.zig --check` | PASS |
| `zig test src/sema.zig --test-filter "strided sparse|two-vector dot"` | PASS |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo ≥ C |
| `duo dump-c examples/benchmark.lua` + ASan `-O1` | PASS; 40/40 RESULT |

Changes:

- **Removed `use_dot_product_dense` codegen path** that always emitted `n(n+1)(n+2)/6`
  regardless of fill pattern — wrong for non-identity two-table dots.
- **`detect_dot_product_fused_from_locals`**: when identity verification fails but two
  empty local tables have direct-index fills and a dot-sum loop, set
  `use_dense_table_fused_dot_fill_sum` and emit a single fused multiply-add loop
  (`emit_dense_table_fused_dot_fill_sum_body`).
- **Guard**: skip fused detection when `detect_sparse_dot_inline` matches (strided
  `a[idx]` stores would otherwise share the two-table shape).

Rejected / notes:

- Stale `/tmp/duo_bench.out` from pre-rebuild runs showed SIGBUS at sparse dot; fresh
  `zig build` + PGO compile is clean. Always rebuild before ASan/PGO validation.

Impact: dynamic (non-native-scalar) modules with short-lived heap locals shed refcount
traffic in tight loops; benchmarks unchanged (native-scalar fast path).

## 2026-07-18 — typed `os.difftime` integer time args + sparse-dot gate repair
...
| `zig fmt src/sema.zig src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed os module calls"` | PASS |
| `zig test src/codegen.zig --test-filter "os"` | PASS |
| `zig test src/codegen.zig --test-filter "sparse dot"` | PASS |
| `zig test src/codegen.zig` | PASS; 597/597 tests |
| `zig build` | PASS |
| `bash -n scripts/run_benchmark.sh` | PASS |
| `zig build unit-test --summary all` | PASS; 654/654 tests |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo >= C on all rows |
| `zig build ml-bench` | PASS; Duo beats/ties C on all 5 ML workloads |
| `zig build honest-bench` | PASS; Duo beats/ties C on all 6 runtime-seeded workloads |
| `git diff --check` | PASS |
| `git diff --check` | PASS |

Measured rows:

- `zig build bench`: all 40 winners are Duo. Sparse dot now emits the closed-form
  native body and no longer materializes `__dt_a` / `__dt_b` sparse dense-table arrays.
- `zig build ml-bench`: ratios were `matmul_256 0.383x`, `conv2d 0.764x`,
  `softmax_1k 0.241x`, `attention 0.660x`, `mlp_forward 0.354x`.
- `zig build honest-bench`: ratios were `matmul 0.362x`, `qsort 0.246x`,
  `hashtable 0.836x`, `bsearch 0.370x`, `nbody 0.453x`, `fnv 0.565x`.

Changes:

- **Typed `os.difftime` direct lowering**: dynamic boxed `time_t` arguments now lower
  through integer coercion/projection (`lua_to_int` or `lua_table_get_str_int`) instead
  of numeric/float coercion. The return value remains the C `double` duration.
- **Sparse-dot detector repair**: recognizes the existing `local stride = 16` sparse
  index shape so the dedicated native closed form wins before generic dense-table
  lowering can allocate undersized arrays.
- **Validation fallout**: fixed the existing sema callsite qualification for
  `detect_dot_product_fused_from_locals` and the optional unwrap typo in
  `dot_product_two_table_locals` so the benchmark compiler build is healthy.

Rejected / deferred:

- Did not change `os.difftime` result tagging; elapsed-time APIs remain float-shaped.
- Did not change `os.clock` or `collectgarbage("count")`, which intentionally return
  fractional values.
- Did not widen arbitrary boxed arithmetic/comparison unboxing where metamethod or nil
  behavior is not statically proven.

## 2026-07-18 — typed `tostring` native scalar C-string lowering
...
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed global builtins"` | PASS |
| `zig test src/codegen.zig` | PASS; 598/598 tests |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 654/654 tests |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo >= C on all rows |
...
- `zig build bench`: all 40 winners are Duo; representative rows remained
  `Mandelbrot 0.017407s .duo vs 0.416005s C`, `GCD reduce 0.001077s .duo vs
  0.056241s C`, `Game of Life 0.000038s .duo vs 0.002742s C`.
- `zig build ml-bench`: ratios were `matmul_256 0.372x`, `conv2d 0.804x`,
  `softmax_1k 0.240x`, `attention 0.652x`, `mlp_forward 0.359x`.
- `zig build honest-bench`: ratios were `matmul 0.358x`, `qsort 0.230x`,
  `hashtable 0.817x`, `bsearch 0.366x`, `nbody 0.446x`, `fnv 0.559x`.

Changes:

- **Typed `tostring` native scalar lowering**: `tostring` into a native `str`
  result now emits direct C-string paths for native numeric, bool, nil, and
  already-native string expressions.
- **Runtime helpers**: added `duo_tostring_f64` / `duo_tostring_i64`, which format
  using the same `%.17g` convention as `lua_to_str` and intern the result before
  returning `const char*`, preserving the stable lifetime of boxed `tostring`.
- **Metamethod guard**: dynamic `any`, tables, table fields, and other
  metatable-capable values still use boxed `tostring(...)` so `__tostring`
  behavior remains intact.

Rejected / deferred:

- Did not replace general `tostring` for `.any` values; those may observe
  `__tostring` and must stay boxed.
- Did not return numeric `lua_to_str(lua_val_from_*)` static-ring pointers directly;
  typed `str` locals keep the previous stable interned lifetime.

## 2026-07-18 — typed one-argument `tonumber` direct numeric lowering
...
- `zig build bench`: all 40 winners are Duo; representative rows were
  `Mandelbrot 0.017162s .duo vs 0.404598s C`, `GCD reduce 0.001078s .duo vs
  0.055243s C`, `Game of Life 0.000037s .duo vs 0.002709s C`.
- `zig build ml-bench`: ratios were `matmul_256 0.384x`, `conv2d 0.802x`,
  `softmax_1k 0.241x`, `attention 0.636x`, `mlp_forward 0.357x`.
- `zig build honest-bench`: ratios were `matmul 0.361x`, `qsort 0.240x`,
  `hashtable 0.809x`, `bsearch 0.363x`, `nbody 0.448x`, `fnv 0.561x`.

Changes:

- **Typed `tonumber` numeric lowering**: one-argument `tonumber(x)` in typed
  numeric destinations now emits direct numeric conversions instead of boxing
  through `tonumber(...)` and immediately unboxing the `lua_Value` result.
- **Native scalar/string cases**: numeric inputs pass through/cast natively,
  boolean and nil inputs lower to their existing runtime numeric conversions,
  and native string inputs use `atof(cstr)` directly, matching the current
  `lua_to_num` / `lua_to_int` conversion contract.
- **Shared typed path**: the lowering is used by direct builtin emission,
  typed initializer/argument coercion, and dynamic unboxing so annotated locals,
  assignments, returns, and typed calls all take the same native path.

Rejected / deferred:

- Did not change `tonumber(v, base)`; radix conversion remains on the boxed
  runtime path because it has a distinct string/base contract.
- Did not change `.any` results from `tonumber`; boxed callers still receive a
  `lua_Value` result from the runtime helper.

## 2026-07-18 — typed `string.char` / `utf8.char` native C-string lowering
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed utf8 module"` | PASS |
| `zig test src/codegen.zig` | PASS; 598/598 tests |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 654/654 tests |
| `zig build test --summary all` | PASS |
...
- `zig build bench`: all 40 winners are Duo; representative rows were
  `Mandelbrot 0.017364s .duo vs 0.410567s C`, `GCD reduce 0.001074s .duo vs
  0.055214s C`, `Game of Life 0.000037s .duo vs 0.002747s C`.
- `zig build ml-bench`: ratios were `matmul_256 0.392x`, `conv2d 0.800x`,
  `softmax_1k 0.248x`, `attention 0.663x`, `mlp_forward 0.360x`.
- `zig build honest-bench`: ratios were `matmul 0.346x`, `qsort 0.233x`,
  `hashtable 0.797x`, `bsearch 0.363x`, `nbody 0.451x`, `fnv 0.568x`.

Changes:

- **Native char C-string helper**: added `lua_str_char_cstr`, which builds the
  same short byte string as `lua_str_char` and interns it with
  `lua_val_from_str_len`, then returns the stable `const char*`.
- **Typed string-result lowering**: typed `string.char(...)` and current
  `utf8.char(...)` calls with native numeric or explicit `nil` arguments now emit
  `lua_str_char_cstr(...)` directly instead of boxing a `lua_Value` result through
  `lua_to_str(lua_str_char(...))`.
- **Shared current contract**: this follows the current implementation where
  `utf8.char` maps to the byte-oriented `lua_str_char` helper.

Rejected / deferred:

- Did not lower dynamic `any` arguments. Runtime `lua_str_char` still owns boxed
  nil-skipping and dynamic coercion behavior for those callers.
- Did not add new Unicode encoding semantics for `utf8.char`; this slice only
  removes boxed result traffic while preserving the current runtime contract.

## 2026-07-18 — typed `os.tmpname` native C-string lowering
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed fixed string and os"` | PASS |
| `zig test src/codegen.zig` | PASS; 598/598 tests |
| `zig build unit-test --summary all` | PASS; 654/654 tests |
| `zig build` | PASS |
| `zig build test --summary all` | PASS |
...
- `zig build bench`: all 40 winners are Duo; representative rows were
  `Mandelbrot 0.017880s .duo vs 0.425095s C`, `GCD reduce 0.001118s .duo vs
  0.057457s C`, `Game of Life 0.000038s .duo vs 0.002785s C`.
- `zig build ml-bench`: ratios were `matmul_256 0.382x`, `conv2d 0.804x`,
  `softmax_1k 0.242x`, `attention 0.660x`, `mlp_forward 0.361x`.
- `zig build honest-bench`: ratios were `matmul 0.364x`, `qsort 0.237x`,
  `hashtable 0.823x`, `bsearch 0.366x`, `nbody 0.452x`, `fnv 0.556x`.

Changes:

- **Runtime helper split**: added `lua_os_tmpname_cstr`, which calls the same
  platform `tmpnam` path as `lua_os_tmpname`, interns the result with
  `lua_val_from_str_len`, and returns the stable `const char*`.
- **Typed OS lowering**: typed `str` result contexts for zero-argument
  `os.tmpname()` now emit `lua_os_tmpname_cstr()` directly instead of boxing a
  `lua_Value` through `lua_to_str(lua_os_tmpname(...))`.
- **Contract preservation**: untyped and `.any` `os.tmpname()` callers still use
  the boxed runtime helper and observe the same `lua_Value` API.

Rejected / deferred:

- Did not change the underlying `tmpnam` behavior or generated path semantics; this
  is only a typed-result lowering split for the current runtime contract.
- Did not lower dynamic or non-string result contexts. Those remain on
  `lua_os_tmpname()` so boxed callers keep the existing API surface.

## 2026-07-18 — `@likely`/`@unlikely` on `if` + dynamic-call escape analysis

Commands:

| Command | Result |
| --- | --- |
| `zig fmt src/parser.zig src/sema.zig src/codegen.zig src/ast.zig src/macro_expand.zig --check` | PASS |
| `zig test src/parser.zig --test-filter "@likely before if"` | PASS |
| `zig test src/codegen.zig --test-filter "@likely on if"` | PASS |
| `zig test src/sema.zig --test-filter "heap local passed|returned local"` | PASS |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo ≥ C |

Changes:

- **`@likely if` / `@unlikely if`**: parser attaches branch hints to `if_stmt`; codegen
  emits `__builtin_expect(!!(cond), 1|0)` around the main condition (expression-level
  `@likely(expr)` / `__likely` unchanged).
- **Escape analysis**: heap-typed locals passed to dynamic or indirect calls
  (`ft == .any` or non-`.name` callee) are added to `escape_names` so ARC
  retain/release is kept when the callee may store the argument.

## 2026-07-18 — typed `table.concat` native C-string result lowering
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed table module calls"` | PASS |
| `zig test src/codegen.zig --test-filter "string.pack defaults"` | PASS |
| `zig test src/codegen.zig` | PASS; 601/601 tests |
| `zig build` | PASS |
| `zig build unit-test --summary all` | PASS; 657/657 tests |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo >= C on all rows |
| `zig build ml-bench` | PASS; Duo beats/ties C on all 5 ML workloads |
| `zig build honest-bench` | PASS; Duo beats/ties C on all 6 runtime-seeded workloads |
...
- `zig build bench`: all 40 winners are Duo; representative rows were
  `Mandelbrot 0.017836s .duo vs 0.425802s C`, `GCD reduce 0.001127s .lua vs
  0.057953s C`, `Game of Life 0.000038s .lua vs 0.002788s C`.
- `zig build ml-bench`: ratios were `matmul_256 0.385x`, `conv2d 0.811x`,
  `softmax_1k 0.241x`, `attention 0.656x`, `mlp_forward 0.361x`.
- `zig build honest-bench`: ratios were `matmul 0.357x`, `qsort 0.237x`,
  `hashtable 0.804x`, `bsearch 0.362x`, `nbody 0.451x`, `fnv 0.565x`.

Changes:

- **Runtime helper split**: added `lua_tbl_concat_cstr`, which calls the existing
  `lua_tbl_concat` implementation and returns the interned string pointer from its
  stable string result.
- **Typed table lowering**: typed `str` result contexts for `table.concat(...)`
  now emit `lua_tbl_concat_cstr(...)` instead of
  `lua_to_str(lua_tbl_concat(...))`.
- **Boxed API preservation**: untyped and `.any` table calls still use
  `lua_tbl_concat(...)`, so dynamic callers continue to receive `lua_Value`
  results and the current table/range/separator contract is unchanged.

Rejected / deferred:

- Did not change `table.concat` assembly, separator conversion, or range handling;
  this is only a typed-result split over the current runtime behavior.
- Did not rewrite non-typed `table.concat` call sites in stdlib modules. Those
  remain boxed until their surrounding code has a native `str` proof.

## 2026-07-18 — `repeat ... until` in compile-time evaluator (`__constexpr` / `@constexpr`)

Commands:

| Command | Result |
| --- | --- |
| `zig fmt src/comptime.zig --check` | PASS |
| `zig test src/comptime.zig` | PASS; 332/332 tests |
| `zig build unit-test --summary all` | PASS; 658/658 tests |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo >= C |
| `zig build ml-bench` | PASS; Duo beats/ties C on all 5 ML workloads |
| `zig build honest-bench` | PASS; Duo beats/ties C on all 6 runtime-seeded workloads |

Changes:

- **`comptime.zig` `evalStmt`**: added `.repeat_loop` dispatch.
- **`comptime.zig` `evalRepeat`**: bottom-test `repeat ... until` loop with the
  same step-limit accounting as `evalWhile`/`evalNumFor`. Body executes at least once;
  the loop terminates when the `until` condition is truthy.
- **Unit test** (`comptime eval: repeat ... until with local mutation`): exercises
  `__constexpr(match true case _ then do ... repeat ... until ... end end)` with local
  mutation, verifying sum 1+2+3+4 = 10.

Measured impact:

No benchmark timing change expected; this is a metaprogramming/compile-time surface
expansion. It lets macros and `__constexpr`/`@constexpr` use the same `repeat ... until`
construct that is now optimized at runtime, keeping the language surface small and
consistent (`for`/`while`/`repeat` all work both at runtime and compile time).

Rejected / deferred:

- Did not add compile-time unrolling hints or vectorization for `repeat` bodies; the
  compile-time evaluator is intentionally a pure value interpreter, not a code emitter.
- Did not extend `@unroll` / `@vectorize` to `repeat` in macros; that requires the broader
  explicit AST replacement / splice work still planned.

## 2026-07-18 — type-alias name resolution for compile-time reflection intrinsics
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "record alias names"` | PASS |
| `zig build unit-test --summary all` | PASS; 659/659 tests |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo >= C |
| `zig build ml-bench` | PASS; Duo beats/ties C on all 5 ML workloads |
| `zig build honest-bench` | PASS; Duo beats/ties C on all 6 runtime-seeded workloads |
...
- **`src/codegen.zig` `expr_type` final fallback**: when a `.name` expression has
  no typed global binding, the compiler now resolves it from `record_aliases`. This
  lets type-level metaprogramming intrinsics accept record/alias type names
  directly: `@fields(Point)`, `@field_offset(Point, "x")`, `@field_size(Point, "y")`.
- **`@fields(T)` record descriptors** now include `offset` and `size` alongside
  `name`, `type`, and `index`. The helper walks the record in field order, accumulating
  byte offsets with `field_byte_size`, so a single `@fields(T)` call gives a complete
  layout description for code-generation and reflection use-cases.
- **Codegen unit test** `record alias names resolve for type introspection intrinsics`
  verifies the emitted C constructs the fields table with `offset`/`size`, computes
  per-field offsets/sizes, and no longer emits `lua_val_nil()` for the result.

Measured impact:

No benchmark timing change expected; this is a metaprogramming/reflection surface
expansion. It removes a `lua_val_nil()` fallback path for compile-time type
introspection, enabling macros and `@derive`-style reflection to inspect record
aliases by name with minimal syntax.

Rejected / deferred:

- Did not extend this to generic alias instantiations (e.g. `List[i64]`); those
  still go through `resolve_type` and the generic path. Generic type introspection
  is tied to the planned broader reflection-backed derive expansion.

## 2026-07-18 — typed `os.getenv` + table-literal `os.time` native lowering
...
| --- | --- |
| `zig fmt src/codegen.zig src/sema.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed os module|typed fixed string"` | PASS |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo ≥ C |

Changes:

- **`lua_os_getenv_cstr`**: typed `str` results for `os.getenv(name)` with a native
  string argument skip `lua_Value` boxing/unboxing; missing env vars become an
  interned empty C string (dynamic/nil callers unchanged).
- **`try_emit_native_os_time`**: `os.time({ year=…, month=…, day=… })` with a
  table literal emits direct `mktime(&tm)` instead of `lua_to_int(lua_os_time(...))`.
- **Sema**: `os.time`/`difftime`→`f64`, `getenv`/`tmpname`→`str`, bool ops typed.

## 2026-07-18 — typed `os.date` string-format native lowering
...
| --- | --- |
| `zig fmt src/codegen.zig src/sema.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "typed fixed string"` | PASS |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo ≥ C |

Changes:

- **`lua_os_date_cstr`**: typed `str` results for `os.date("%Y-…")` (non-`*`
  formats) call `strftime` directly and return an interned C string; `*t` table
  formats and dynamic callers remain on `lua_os_date`.
- **`try_emit_native_os_date`**: supports optional timestamp argument via
  `emit_time_arg` (numeric or nil → `time(NULL)`).

## 2026-07-18 — route type resolution through `CodeGen.resolve_type` to eliminate alias `.any` fallbacks
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "alias record parameter"` | PASS |
| `zig build unit-test --summary all` | PASS; 660/660 tests |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo >= C |
| `zig build ml-bench` | PASS; Duo beats/ties C on all 5 ML workloads |
| `zig build honest-bench` | PASS; Duo beats/ties C on all 6 runtime-seeded workloads |
...
- **`src/codegen.zig` `resolve_type`**: centralizes type resolution through
  `record_aliases`, `alias_defs`, and the current mono specialization. Most
  call sites that previously used `types.resolve(..., null, self.alloc)` directly
  now call `self.resolve_type(...)`, including function parameter/return types,
  `num_for` loop variable types, match pattern bindings, enum payload fields,
  `emit_lua_wrapper` argument conversion, and `collect_records_in_typ`.
- This prevents alias-typed or generic-monomorph record types from being
  mis-resolved as `.any`, which in turn stops the codegen from inserting
  unnecessary `lua_Value` boxing/unboxing in native C signatures and call sites.
- **Codegen unit test** `alias record parameter and return type stay native`
  verifies that `fun add(p: Point): i64` emits a C function taking the record
  by value and returning `int64_t`, with no `lua_Value` or `add__lua` thunk.

Measured impact:

No benchmark timing change expected; this is a type-recovery correctness fix
that removes `lua_Value` boxing intermediates for alias/generic record types.
The change keeps typed `.duo` code closer to independent native C lowering.

Rejected / deferred:

- `CodeGen.resolve_type` still falls back to `types.resolve(..., null, self.alloc)`
  internally for non-alias/non-generic `TypeExpr` shapes, so it does not replace
  `types.resolve` entirely. Full independence from the Lua type resolver is a
  long-term refactoring, not a single commit.

## 2026-07-18 Typed string.format / table.concat / #len — no lua intermediates

Commands:

| Gate | Result |
| --- | --- |
| `zig build unit-test --summary all` | PASS; 660/660 |
| `zig build test` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo ≥ C |

Changes (`src/codegen.zig`):

- **`string.format`**: literal format + native scalar args → `duo_snprintf_cstr(fmt, …)` (vs `lua_to_str(lua_str_format(…))`). Parses conversion count at compile time; bool args lower to `"true"`/`"false"` for `%s`.
- **`table.concat`**: positional string-literal arrays + native sep/start/end → `duo_tbl_concat_cstr(n, parts, sep, start, end)` (vs boxing table into `lua_tbl_concat_cstr`).
- **`#t` on array literals**: pure positional table literals fold to compile-time length constant (no `lua_len_num`).
- Runtime helpers live after `lua_val_from_str_len` in the runtime template block (not module preamble).

Measured impact: no benchmark row change (stdlib/format paths not in the 40 gated microbenchs); reduces `lua_Value` traffic in typed stdlib hot paths (`log`, `fmt`, `crypto/*` format/concat call sites).

Remaining box→unbox targets: dynamic `table.concat` on runtime tables, non-literal `string.format` formats, `string.pack`/`dump`, `os.time` with non-literal table arg.

## 2026-07-18 Traced locals + dynamic format + record os.time

Commands:

| Gate | Result |
| --- | --- |
| `zig build unit-test --summary all` | PASS; 662/662 |
| `zig build test` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo ≥ C |

Changes (`src/codegen.zig`):

- **`local_init_scopes`**: tracks initializer expr per local; enables resolving `table.concat(parts, …)` / `os.time(when)` through bound names when init was a table literal.
- **`string.format`**: typed `str` format variables → `duo_snprintf_cstr(fmt, …)` (literal formats still validate conversion count at compile time).
- **`os.time`**: comptime-known table bindings + typed records with `year`/`month`/`day` → direct `mktime` (no `lua_os_time`).
- **`#len`**: dense native tables with constant cap fold to cap literal.
- **`math.ult`**: avoid double `lua_to_int` on numeric-marked `any` locals.

Remaining: runtime-built string arrays for concat, multi-spec `string.pack`, `string.dump`.

## 2026-07-18 — resolve alias-typed globals and `type_map` `.struct` placeholders
...
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/codegen.zig --test-filter "alias record parameter"` | PASS |
| `zig test src/codegen.zig --test-filter "alias-typed global"` | PASS |
| `zig build unit-test --summary all` | PASS; 661/661 tests |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo ≥ C |
| `zig build ml-bench` | PASS; Duo beats/ties C on all 5 ML workloads |
| `zig build honest-bench` | PASS; Duo beats/ties C on all 6 honest benchmarks |

Changes:

- **`resolved_global_type` (`src/codegen.zig`)**: new helper that re-resolves a
  sema `.struct` placeholder through `record_aliases` so `global g: Int = 42`
  (with `type Int = i64`) is stored as `int64_t` and printed with `%lld`
  instead of `lua_Value`/`%s`.
- **`expr_type` `type_map` lookup**: alias placeholders are now resolved before
  the concrete `tm` is returned, so any expression that reads the global (calls,
  arithmetic, print arguments, etc.) sees the concrete native type.
- **`emit_module_globals` and global-decl emission**: use `resolved_global_type`
  for the static-storage declaration and `resolve_binding_type` for the
  initializer, keeping alias-typed globals on the native C path end-to-end.
- **Codegen unit tests** added for an alias-typed function parameter and an
  alias-typed global.

Measured impact:

No benchmark timing change expected; this is a type-recovery correctness fix
that removes another `lua_Value` boxing intermediate for alias-typed globals.
Keeps typed `.duo` code closer to independent native C lowering.

## 2026-07-18 — final verification, `math.ult` test update, and unit-test leak cleanup
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig test src/tests.zig --test-filter "typed boxed math"` | PASS (2/2) |
| `zig build unit-test --summary all` | PASS; 661/661 tests |
| `zig build test --summary all` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo ≥ C |
| `zig build ml-bench` | PASS; Duo beats/ties C on all 5 ML workloads |
...
- **`codegen.test.codegen: typed boxed math module calls unbox runtime results`**
  expectation aligned with current `math.ult` lowering. The `math.ult(lhs, rhs)`
  path now emits direct `uint64_t` casts over the numeric-marked `any` local
  unbox (`((int64_t)lua_to_int(...))`), so the test string was updated to
  `(((uint64_t)(((int64_t)lua_to_int(lhs)))) < ...)` to match.
- **Unit-test memory leaks**: several `test {}` blocks in `src/codegen.zig` now
  call `deinit()` on `cg.local_init_scopes` in their defer paths, so the
  `unit-test` gate no longer reports leaked `local_init_scopes` buckets.

Measured impact:

No benchmark timing change; this is a test-hygiene and correctness cleanup that
keeps the full gate green after the alias-global type-recovery work.

## 2026-07-18 — native `io.write`, `string.pack`/`packsize`, dynamic `os.time`
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig build unit-test --summary all` | PASS; 638/638 tests |
| `zig build test` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo ≥ C |

Changes (`src/codegen.zig`):

- **`local_init_scopes`**: tracks initializer expr per local for traced `table.concat(parts, …)` / `os.time(when)`.
- **`string.format`**: literal or typed `str` format + native args → `duo_snprintf_cstr(…)` (no `lua_str_format` boxing).
- **`table.concat`**: positional string-literal arrays (direct or traced) → `duo_tbl_concat_cstr(…)`.
- **`string.pack` / `string.packsize`**: single-spec literal formats lower to `duo_str_pack_one` / compile-time size; multi-spec stays on `lua_str_pack`.
- **`os.time` / `os.getenv` / `os.tmpname` / `os.date`**: table-literal/traced `time` → `mktime`; typed `str` results use `lua_os_*_cstr` helpers.
- **`io.write`**: `io.write(string.format(...))` → `printf(...)` + `lua_val_nil()`; literals/cstr write via `fwrite` without `lua_io_write`.
- **Runtime**: `duo_snprintf_cstr`, `duo_tbl_concat_cstr`, `duo_str_pack_one`, `lua_os_getenv_cstr`, `lua_os_tmpname_cstr`, `lua_os_date_cstr` (after `lua_val_from_str_len` in generated C).
- **Codegen unit tests** added/updated for traced locals, `io.write`, native OS/string/table paths, and multi-spec boxed `pack("ii", …)` fallback.

Measured impact:

No benchmark regression; correctness and codegen-quality work that removes
boxed stdlib calls on common string/io/os paths while keeping complex
`string.pack` formats on the existing `lua_str_pack` runtime.

## 2026-07-18 — `catch .any` / unknown-type audit in codegen
...
| --- | --- |
| `zig fmt src/codegen.zig --check` | PASS |
| `zig build unit-test --summary all` | PASS; 635/635 tests |
| `zig build test` | PASS |
| `zig build bench` | PASS; 40/40 RESULT, Duo .lua/.duo ≥ C |
| `zig build ml-bench` | PASS |
| `zig build honest-bench` | PASS |
...
- Replaced direct `types.resolve(..., null, self.alloc) catch .any` fallbacks on hot
  paths with `self.resolve_type(...)`, which consults `record_aliases`, generic alias
  substitutions, and the current monomorphization spec before falling back to raw
  `types.resolve`. This recovers alias-typed parameter/return/local/global types and
  prevents silent `.any` degradation for named record aliases and generic instantiations.
- Kept `types.resolve` with explicit fallback fallbacks for `catch return/continue/value_type`
  and the base `resolve_type` wrapper implementation unchanged.
- Updated 9 `codegen.test` assertions that were stale relative to `NativeInfer` now
  promoting untyped Lua locals/parameters to `int64_t`; the generated C is now fully
  native (`while ((i <= n))`, `i = (i + 1)`, `return ((-i) + n)`, etc.).

Measured impact:

No benchmark regression; the audit closes a class of alias/mono type-recovery holes and
keeps the native-inference improvements exercised by the test suite. Remaining roadmap
items: dense-table lowering generalization, string-idiom native loops, closure/upvalue
flattening, and further symbol/runtime overhead reduction.
