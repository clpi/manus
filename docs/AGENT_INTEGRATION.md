...
**Redirect:** This file is maintained for historical reference. The canonical location
is [`.agents/AGENT_INTEGRATION.md`](.agents/AGENT_INTEGRATION.md) which includes
this content plus the latest updates.

## Quick Reference for All Agents

### At session start:
1. Call `duo_agent_session_start(agent_id="your-id")` (duo-bench MCP)
2. Read [`.agents/AGENT_COORDINATION.md`](.agents/AGENT_COORDINATION.md)

### Before editing:
- Claim files via `duo_coordination_update(action="claim")`

### Before building:
- Check `scripts/duo_lock.sh status`
- Use `scripts/duo_lock.sh -- ./zig-out/bin/duo run scripts/agent_smoke.duo` for tier-0

### MCP Servers:
- **duo-lsp** — diagnostics, completions, hover, definition
- **duo-bench** — benchmarks, build gates, coordination
- **zls** — Zig language support for compiler development
...
| Kimi Code | `kimi` | `~/.kimi/config.toml` | ✅ (brew kimi-code installed) |
...
5. **When discovering gaps:** Append to `.agents/AGENT_COORDINATION.md#cross-agent-gap-buffer`
