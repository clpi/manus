...
| # | directive |
|---|---|
| 1 | Truncation is **toward zero**, not floor: `-2 0 7` -> `-2`, so negatives need `math.ceil`. |
...
| # | directive |
|---|---|
| 1 | `f64.nearest` is **roundTiesToEven**, not `floor(x + 0 0 5)`: 2.5 -> 2 but 3.5 -> 4. |
...
| # | directive |
|---|---|
| 1 | **Return-type handling is FIXED** (`find_run_body(... , 2)` reports the target's declared result type; `run_body` no longer masks with `M32`; the entry formats f64 by bitcasting the bits back). `wart_f64_bench` went `1929046707` -> `1000100019002752` against wasmtime's `1000100048462729 0 9` — the magnitude is now right, so what remains is an **accumulation difference**, not truncation. |
