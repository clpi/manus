# Platform shim: ARM64 + Windows. STATUS: planned (stub).
#
# Backend needed: PE/COFF object emission for ARM64 in lib/compiler/.
# Implements the bench/platforms/lib.sh interface as stubs (return 3).
# Source this file, do not execute it.
#
# To enable (backend agent):
#   1. Implement ARM64+PE/COFF object emission in lib/compiler/
...
#   4. Remove the host abort in bench/run.sh for the ARM64-Windows uname pair.

plat_triple="arm64-windows"
plat_status="planned"

plat_idol_object() {
  echo "not implemented: arm64-windows plat_idol_object" >&2
  return 3
}

plat_link() {
  echo "not implemented: arm64-windows plat_link" >&2
  return 3
}

plat_c_exe() {
  echo "not implemented: arm64-windows plat_c_exe" >&2
  return 3
}
